...
- [ ] 免除（自律クラフト：<理由>）
- recon: <!-- docs/handoff/<仕事名>/recon.md（実ファイルが必要） -->
- 設計: <!-- docs/handoff/<仕事名>/design.md（実ファイルが必要） -->
- 対象ADR: <!-- design.md の ADR 参照と同じ文字列にする -->
- 設計仕様書（あるべき姿）: <!-- 参照/新規作成した docs/specs/<領域>/README.md へのリンク。索引: docs/specs/README.md。該当が無い開発は「対象外」 -->
- 触るファイル: <!-- 半角コロンと同じ行にカンマ区切り。改行して箇条書きにしない。例: backend/app/a.py, docs/handoff/x/recon.md -->
- 削除するファイル: <!-- 1行でも変更・削除した行があるファイルをすべて列挙。追加のみなら「なし」 -->
