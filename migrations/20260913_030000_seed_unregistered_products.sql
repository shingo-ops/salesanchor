...
BEGIN
    ...
    IF v_schema IS NOT NULL THEN
        EXECUTE format('SELECT id FROM %I.tcg_series WHERE code = $1', v_schema)
            INTO v_db_work_id USING 'IP003';
        EXECUTE format('SELECT id FROM %I.tcg_series WHERE code = $1', v_schema)
            INTO v_op_work_id USING 'IP002';
    END IF;

    -- work_id NOT NULL 制約が存在するのにシリーズが見つからない場合はスキップ
    IF (v_db_work_id IS NULL OR v_op_work_id IS NULL) AND EXISTS (
        SELECT 1 FROM pg_attribute
        WHERE attrelid = 'public.products'::regclass
          AND attname = 'work_id' AND attnotnull
    ) THEN
        RAISE NOTICE 'work_id NOT NULL active but Dragon Ball or ONE PIECE series not found — skipping seed';
        RETURN;
    END IF;
    ...
    ('DB-FB09', 'ブースターパック DUAL EVOLUTION', 'Booster Pack -DUAL EVOLUTION-',
     ...
     'スリーブ,プレイマット,カードケース,シングル,中古,オリパ,ワンピース,ワンピースカード,ポケモン,ポケモンカード,遊戯王,デュエマ,ヴァイス,バトスピ,ヒーローズ,SDBH,ダイバーズ,超カードゲーム,ガンダム', v_db_work_id),
    ...
    ('DB-STBR01', 'STORY BOOSTER 01', 'STORY BOOSTER 01',
     ...
     'スリーブ,プレイマット,カードケース,シングル,中古,オリパ,ワンピース,ワンピースカード,ポケモン,ポケモンカード,遊戯王,デュエマ,ヴァイス,バトスピ,ヒーローズ,SDBH,ダイバーズ,超カードゲーム,ガンダム,ST-01,ST01', v_db_work_id),
    ...
    ('DB-DIVERS', 'ドラゴンボールスーパーダイバーズ', 'Dragon Ball Super Divers',
     ...
     'フュージョンワールド,FW,DBFW,FB,FS,ブースターパック,スタートデッキ,ワンピース,ワンピースカード,ポケモン,ポケモンカード,遊戯王,デュエマ,ヴァイス,バトスピ,ヒーローズ,SDBH,ガンダム,スリーブ,プレイマット', v_db_work_id),
    ...
    ('OP-PCC-DAY24', 'プレミアムカードコレクション -ONE PIECE DAY''24-', 'Premium Card Collection -ONE PIECE DAY''24-',
     ...
     'DAY25,DAY26,DAY''25,DAY''26,ドラゴンボール,ポケモン,ポケモンカード,遊戯王,デュエマ,ヴァイス,バトスピ,ガンダム,デジモン,ホロライブ,ロルカナ,ユニオンアリーナ,SDBH,ヒーローズ,スリーブ,プレイマット,カードケース,シングル,中古,オリパ', v_op_work_id),
    ...
    ('OP-PCC-NIKE', 'プレミアムカードコレクション -Nikeコラボエディション-', 'Premium Card Collection -Nike Collaboration Edition-',
     ...
     'ドラゴンボール,ポケモン,ポケモンカード,遊戯王,デュエマ,ヴァイス,バトスピ,ガンダム,デジモン,ホロライブ,ロルカナ,ユニオンアリーナ,SDBH,ヒーローズ,スリーブ,プレイマット,カードケース,シングル,中古,オリパ', v_op_work_id),
    ...
    ('OP-CHP01', 'ONE PIECE CHOPPER''s 1 プロモカード', 'ONE PIECE CHOPPER''s 1 Promo Card',
     ...
     'EB-02,EB02,アニメ25th,ドラゴンボール,ポケモン,ポケモンカード,遊戯王,デュエマ,ヴァイス,バトスピ,ガンダム,デジモン,ホロライブ,ロルカナ,ユニオンアリーナ,SDBH,ヒーローズ,スリーブ,プレイマット,カードケース,シングル,中古,オリパ', v_op_work_id),
    ...
    ('OP-2ANNY-GD', '2nd ANNIVERSARY COMPLETE GUIDE', 'ONE PIECE CARD GAME 2nd ANNIVERSARY COMPLETE GUIDE',
     ...
     '2nd ANNIVERSARY SET,2ANNY,ドラゴンボール,ポケモン,ポケモンカード,遊戯王,デュエマ,ヴァイス,バトスピ,ガンダム,デジモン,ホロライブ,ロルカナ,ユニオンアリーナ,SDBH,ヒーローズ,スリーブ,プレイマット,カードケース,シングル,中古,オリパ', v_op_work_id),
