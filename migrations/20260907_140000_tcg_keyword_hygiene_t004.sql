...
BEGIN
    IF NOT EXISTS (
        ...
        RAISE NOTICE 'migration 20260907_140000: schema % does not exist, skipping', _schema;
        RETURN;
    END IF;
    ...
    IF v_count != 0 THEN
        RAISE EXCEPTION '20260907_140000: 削除対象が残っています: %', v_count;
    END IF;
    ...
    $q$, _schema, _schema) INTO v_count;
    IF v_count != 7 THEN
        RAISE EXCEPTION '20260907_140000: 追加が7本ではありません: %', v_count;
    END IF;
    ...
    $q$, _schema, _schema) INTO v_count;
    IF v_count != 1 THEN
        RAISE EXCEPTION '20260907_140000: PM0071 の綴り修正が1件ではありません: %', v_count;
    END IF;
    ...
    IF v_count != 1 THEN
        RAISE EXCEPTION '20260907_140000: PM0146 の無効化が反映されていません: %', v_count;
    END IF;

    RAISE NOTICE '20260907_140000: 完了（削除5・追加7・修正1・無効化1）';
END
$body$;
