...
BEGIN
    ...
    LOOP
        ...
        -- ビューが deals に LEFT JOIN するため deals なしでは CREATE VIEW が失敗する。
        -- 後続の 20260612_120000 が deals 非依存版を作成するため chain は破綻しない。
        IF NOT EXISTS (
            SELECT 1 FROM pg_tables
            WHERE schemaname = schema_rec.nspname AND tablename = 'deals'
        ) THEN
            RAISE NOTICE 'migration 20260604_100000: skipping schema %: deals table does not exist', schema_rec.nspname;
            CONTINUE;
        END IF;
