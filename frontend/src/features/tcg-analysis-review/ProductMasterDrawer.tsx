    ...
    <>
      ...
      {status === 'loading' ? (
        ...
        <>
          ...
          <section className="drawer-result">
            <h4>登録内容確認</h4>
          ...
          {candidates.length > 0 && (
            <section className="drawer-warning">
          ...
          {error && <p className="error">登録エラー: {error}</p>}
  ...
  return (
    <>
      ...
      {candidates.length === 0 && !searching && (
        <p className="drawer-note">検索結果なし</p>
      )}
      {candidates.map((c) => (
        <div
          key={c.product_id}
          className={`search-candidate${selected?.product_id === c.product_id ? ' search-candidate--selected' : ''}`}
        >
...
export function ProductMasterDrawer({ item, onClose }: { item: AnalysisReviewItem; onClose: () => void }) {
  return (
    <>
      ...
      <aside className="drawer" aria-label="商品マスタメンテナンス">
