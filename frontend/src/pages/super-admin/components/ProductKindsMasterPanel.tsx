/**
 * ProductKindsMasterPanel — 大分類マスタパネル（AnalysisRulesPage の hub-content 内で使用）
 *
...
export function ProductKindsMasterPanel() {
  const { t } = useTranslation();
  const f = "productKindsMaster";

  const [items, setItems] = useState<ProductKind[]>([]);
  const [error, setError] = useState("");
  const [showForm, setShowForm] = useState(false);
  const [form, setForm] = useState<KindFormState>(emptyForm);
  ...
  const submit = async (e: FormEvent) => {
    ...
    try {
      if (editId) {
        await api.patch(`/super-admin/product-kinds/${editId}`, payload);
      } else {
        await api.post("/super-admin/product-kinds", payload);
      }
      setShowForm(false);
      setForm(emptyForm);
      setEditId(null);
  ...
  const confirmDelete = async () => {
    ...
    setError("");
    try {
      await api.delete(`/super-admin/product-kinds/${confirmDeleteId}`);
  ...
  const columns: DataTableColumn<ProductKind>[] = [
    ...
    },
    {
      ...
      renderCell: row => (
        <HeaderButton
          variant="secondary"
          data-testid={`product-kind-edit-${row.id}`}
    ...
    },
    {
      ...
      renderCell: row => (
        <HeaderButton
          variant="secondary"
          data-testid={`product-kind-delete-${row.id}`}
