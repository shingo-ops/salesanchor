...
function NoteMasterImportPanel({ onDone }: { onDone: () => void }) {
  ...
  async function doPreview() {
    ...
    try {
      setPreview(await api.postForm<PreviewResponse>("/super-admin/note-master/import/preview", form));
    } catch (e) {
  ...
  async function doCommit() {
    ...
    try {
      setResult(await api.postForm<CommitResponse>("/super-admin/note-master/import/commit", form));
      setPreview(null);
    } catch (e) {
...
export default function NoteMasterImportPage() {
  const { t } = useTranslation();
  const navigate = useNavigate();
  const { isSuperAdmin, loading } = useSuperAdmin();
  return (
    <PageLayout titleText={t("noteMasterCsv.importTitle")}>
      {loading ? <p>{t("common.loading")}</p> : !isSuperAdmin ? <p role="alert">{t("productCsv.denied")}</p> : <NoteMasterImportPanel onDone={() => navigate("/super-admin/analysis-rules")} />}
      {!loading && !isSuperAdmin && <HeaderButton variant="secondary" onClick={() => navigate("/super-admin/analysis-rules")}>{t("noteMasterCsv.backToList")}</HeaderButton>}
    </PageLayout>
  );
}
