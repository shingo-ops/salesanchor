...
function StatusMasterImportPanel({ onDone }: { onDone: () => void }) {
  ...
  async function doPreview() {
    ...
    try {
      setPreview(await api.postForm<PreviewResponse>("/super-admin/status-master/import/preview", form));
    } catch (e) {
  ...
  async function doCommit() {
    ...
    try {
      setResult(await api.postForm<CommitResponse>("/super-admin/status-master/import/commit", form));
      setPreview(null);
    } catch (e) {
...
export default function StatusMasterImportPage() {
  const { t } = useTranslation();
  const navigate = useNavigate();
  const { isSuperAdmin, loading } = useSuperAdmin();
  return (
    <PageLayout titleText={t("statusMasterCsv.importTitle")}>
      {loading ? <p>{t("common.loading")}</p> : !isSuperAdmin ? <p role="alert">{t("productCsv.denied")}</p> : <StatusMasterImportPanel onDone={() => navigate("/super-admin/analysis-rules")} />}
      {!loading && !isSuperAdmin && <HeaderButton variant="secondary" onClick={() => navigate("/super-admin/analysis-rules")}>{t("statusMasterCsv.backToList")}</HeaderButton>}
    </PageLayout>
  );
}
