...
| Advisor Phase 1 W-2① 属性別成約率API | Agent | `backend/app/routers/analytics.py` に `/analytics/conversion-by-attribute` を追加し、`channel_type / country / sales_form / temperature / response_speed` の team/mine 別成約率を `n / conversions / raw_rate / smoothed_rate` で返す read-only 集計 API を実装中 | `backend/tests/test_analytics.py` に team / mine / shrink の pytest を追加し、PG/RLS 実走を確認して branch → push → CI へ進める | `backend/app/routers/analytics.py` / `backend/tests/test_analytics.py` | 2026-06-22 |
