...
| Supply Chain PR-1/2/3 condition正典地盤 | Agent | `backend/app/services/condition_vocab.py` を新設し、parser / LLM parser / ParseReviewPage / ja.json / en.json を正典マスタに寄せた。`scripts/check-condition-vocab.js` と `.github/workflows/condition-vocab-check.yml` を追加し、temp copy での意図的違反で CI 赤、実体で CI 緑を確認した。`recon_ro` は prod1 側で後始末済み、`migrations/20260622_020000_add_inventory_raw_condition.sql` と `scripts/run_all_migrations.sh` への登録で raw_condition の段階1 migration も runner に載せた。加えて `scripts/check-migration-runner-coverage.sh` と `.github/workflows/migration-runner-coverage.yml` を追加し、repo audit で 088/089/20260621 の現行-era 未登録候補を確認、PR 差分では dummy migration で赤→緑を実機確認済み | design / recon / migration runner / gate / tests の証跡を揃えた。残ブロッカーは `process-artifacts gate` の GO 記録未反映 | `backend/app/services/condition_vocab.py` / `backend/app/services/inventory_parser.py:107-108,223-237,511-518,577-672,1097-1111` / `backend/app/services/inventory_parser_llm.py:64-81,102-125,159-180,299-317` / `frontend/src/pages/super-admin/ParseReviewPage.tsx:125-131,585-602` / `frontend/src/locales/ja.json:2233-2243` / `frontend/src/locales/en.json:2233-2243` / `scripts/check-condition-vocab.js:8-116` / `.github/workflows/condition-vocab-check.yml:1-21` / `migrations/20260622_020000_add_inventory_raw_condition.sql:1-6` / `scripts/run_all_migrations.sh:165-173` / `scripts/check-migration-runner-coverage.sh` / `.github/workflows/migration-runner-coverage.yml` / `pytest -q --no-cov backend/tests/test_inventory_parser_rule.py backend/tests/test_inventory_parser_llm.py` / `node scripts/check-condition-vocab.js` / `gh run view 27932790162 --log-failed` / `gh run view 27932819190 --json conclusion,url,headSha` | 2026-06-22 |
