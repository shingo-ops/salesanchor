...
|------|------|------|---------|-----|------|
| QA Smoke Suite #2346 psql 経路自己探索化 | CI失敗 | `tests/qa-smoke/utils/db-assert.ts` は host psql 一本化済み。`tests/qa-smoke/utils/real-backend.ts` の診断ログ付き run `27886759302` では scene-01 は Firebase auth の `identitytoolkit.googleapis.com` への `CORS policy` ブロックと `[reqfailed] net::ERR_FAILED` を記録し、`scene-09` でも Dashboard 未到達と `lead_sales_form_selections` 0 件で fail | Firebase auth の egress/CORS 要因を切り分けて修正方針を確定し、`gh pr checks 2346` が全 SUCCESS かつ `mergeable=MERGEABLE` になるまで再 dispatch | `.github/workflows/qa-smoke.yml` / `tests/qa-smoke/utils/db-assert.ts` / `tests/qa-smoke/utils/real-backend.ts` / `gh run view 27886759302 --log-failed` / `gh pr view 2346 --json headRefOid,mergeable` | 2026-06-21 |
| Advisor Phase 1 PR-5 W-1b 今やること UI | Agent | `frontend/src/pages/dashboard/DashboardPage.tsx` の `FunnelSection` 直下に `WeeklyAdvisorSection` を差し込み、`/analytics/weekly-advisor-defensive` を `scope=mine` で表示する UI を実装中。ローディング / 空状態 / 意味ベース色 / dark mode / Playwright まで確認済み | `docs/handoff/advisor-weekly/` の recon/design を追加し、required checks と screenshot baseline の最終確認後に branch → push → PR へ進める | `frontend/src/pages/dashboard/DashboardPage.tsx:487-493` / `frontend/src/pages/dashboard/WeeklyAdvisorSection.tsx` / `frontend/src/pages/dashboard/WeeklyAdvisorSection.css` / `frontend/tests-e2e/scene1-dashboard.spec.ts` / `frontend/tests-e2e/funnel-dashboard.spec.ts` | 2026-06-21 |
...
| タスク | 完了日 | PR |
|------|------|---|
| Discord ticket translation A2 Meta 受信 即時二言語翻訳 | 2026-06-20 | #2397 / #2393 |
