"""
TCG Analysis System — SQLAlchemy Models (v1.0)
...
class ProductsLogistics(Base):
    ...
    product_id = Column(UUID(as_uuid=True), ForeignKey("tcg_products.id", ondelete="CASCADE"),
                         primary_key=True)
    # 将来列: boxes_per_case, packs_per_box, box_weight_g, case_weight_g,
    #         volume_weight, jan_code, official_url, purchase_price など
    # 現時点では器のみ作成。実データ投入時に ALTER TABLE で列追加。
    created_at = Column(DateTime(timezone=True), nullable=False, server_default=func.now())
