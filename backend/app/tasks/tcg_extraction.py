"""
MIG-04 Phase 3: Celery タスク — Gemini 抽出 + 照合。
...
1 つの source_message に対して:
  ...
  6. status='done' なら analyze_extraction_job を呼び出して analysis_results を生成
...
def _run_extraction(session: Session, source_message_id: str) -> dict:
    ...
    session.execute(
        text(
            "UPDATE extraction_jobs SET status = 'running' WHERE id = :ej_id"
        ),
