...
def apply_unit_recovery_for_job(
    ...
    処理順序:
      1. E3a: unit_resolved=FALSE・raw_unit='' の行を商品名末尾の単位語で復旧 → DB UPDATE
      2. E5:  E3a 復旧行のうち condition_basis='R4:単位既定:単位不明' の行で condition 再計算 → DB UPDATE

    Returns:
        {e3a_recovered: int, e5_changed: int, aborted: bool, message: str}
    """
    unit_terms = build_unit_recovery_terms()
    if not unit_terms:
        return {
    ...
    if len(e3a_updates) > E3A_MAX_RECOVER:
        ...
        return {
            ...
            "aborted": True,
            "message": (
                f"E3a safety abort: {len(e3a_updates)} rows exceed limit {E3A_MAX_RECOVER}"
            ),
        }
    ...
    if len(e5_changes) > E5_MAX_ROWS:
        ...
        return {
            ...
            "message": (
                ...
                f"E5 safety abort: {len(e5_changes)} changes exceed limit {E5_MAX_ROWS}"
            ),
        }
    ...
    return {
        ...
        "aborted": False,
        "message": (
            f"E3a: {len(e3a_updates)} recovered, E5: {len(e5_changes)} conditions changed"
        ),
    }
