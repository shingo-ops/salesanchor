...
async def commit_update(db: AsyncSession, raw: bytes, filename: str, executed_by: str, confirmed_digest: str) -> dict:
    try:
        ...
        prior = await db.execute(
            text(f"SELECT id FROM public.tcg_product_import_jobs WHERE raw_sha256=:digest"),
        ...
        job = await db.execute(
            text(
                f"INSERT INTO public.tcg_product_import_jobs "
        ...
        for plan in plans:
            ...
            await db.execute(
                text(
                    f"INSERT INTO public.tcg_product_import_rows "
