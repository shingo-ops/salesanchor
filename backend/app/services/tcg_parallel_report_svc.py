...
async def _load_lookup_maps_async(db: AsyncSession) -> tuple[dict, dict, dict, dict, dict]:
    ...
    # 商品コード → UUID
    rows = (
        await db.execute(text("SELECT code, id FROM tcg_products WHERE is_active = TRUE"))
    ).fetchall()
    ...
    rows = (
        await db.execute(
            text(
                ...
                JOIN tcg_products p ON p.id = psk.product_id
    ...
    rows = (
        await db.execute(
            text(
                ...
                FROM product_exclude_keywords pek
                JOIN tcg_products p ON p.id = pek.product_id
                WHERE p.is_active = TRUE
    ...
    rows = (
        await db.execute(
            text(
                ...
                JOIN extraction_jobs ej ON ei.extraction_job_id = ej.id
                JOIN source_messages sm ON ej.source_message_id = sm.id
                JOIN supplier_channels sc ON sm.supplier_channel_id = sc.id
                JOIN tcg_suppliers ts ON sc.supplier_id = ts.id
