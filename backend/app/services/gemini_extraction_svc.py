...
PROMPT_VERSION = "raw-extraction-v3-work-p2"
...
PROMPT_TEXT = (
    ...
    "\n"
    "抽出の保持規則:\n"
    "商品に対応する発送日・発送時期・発送条件（例: 9/18（金）発送、発売日発送、届き次第発送、発送日要相談）は、原文表記のまま必ずRAW_MEMOへ残せ。商品名と数量価格の間の独立行、数量価格の後の補足行も確認せよ。日付や曜日を計算・補正・推測するな。\n"
    "同じ商品に価格・数量・発送条件の異なる行がある場合は別明細とし、その行に対応する発送条件だけを付けよ。次の商品へ前の発送条件を引き継ぐな。対象を明示した共通見出しは、その明示された商品のみへ適用せよ。帰属が不明な語を別商品へコピーするな。\n"
    "当該商品の状態（シュリンク有無、開封、箱の凹み・破れ等）は否定や程度も省略せずRAW_STATEへ残せ。「凹みなし」の「なし」を落とすな。送料・配送後の免責を商品の現状の破損と扱うな。\n"
    "RAW_SOURCE_LINE_SPANは当該商品の名前・数量・価格・状態・補足の根拠を含める。出力前に各明細の原文を読み返し、対応する発送情報と状態の肯定/否定が欠落していないか、別商品の情報が混じっていないかを点検せよ。既定の9列ヘッダーと出力禁止事項を維持せよ。\n"
)
