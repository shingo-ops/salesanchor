...
"""
spec.md v1.1 F4 / Sprint 4: LLM フォールバック解析 (Gemini 3.1 Flash-Lite)。

F3 ルールベース解析 (`inventory_parser.parse_raw_content`) が unparsed[] を
返した行のみを Gemini 3.1 Flash-Lite に投げ、構造化抽出して items[] にマージする
ための薄い service レイヤ。
...
async def parse_with_gemini(
    ...
    *,
    ...
    """unparsed 行を Gemini 3.1 Flash-Lite で再解析する。
    ...
    Args:
        ...
        model_name: 'gemini-3.1-flash-lite' (default、別モデルは別 ADR)
