...
def test_v3_format_failure_saves_no_partial_items(monkeypatch, response):
    """全行が失敗 or ヘッダーなし → status=error, items=[]。"""
...
def test_v3_partial_save_success_and_error_rows(monkeypatch):
    """成功行と失敗行が混在 → status=done, 成功行のみ items に含む。"""
    mixed = _V3_HEADER + "\n商品A｜1｜100｜BOX｜｜｜L0001｜｜\n壊れた｜行"
    monkeypatch.setattr("app.services.gemini_extraction_svc.call_gemini_extraction",
...
def test_v3_product_span_never_clamped(span):
    # Invalid spans are now collected as parse_errors (row-level skip) instead of whole-batch failure.
...
def test_v3_work_evidence_roundtrip_without_repair():
    ...
    items2, _ = parse_extraction_response(_VALID_RESPONSE, "a\nb\nc")
    assert items2[0]["raw_work_name"] is None
