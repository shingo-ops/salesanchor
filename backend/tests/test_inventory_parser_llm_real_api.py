...
feedback_evaluator_gap_2026_05_15.md の「SQLite モック禁止条項」遵守:
  ...
  実 Gemini 呼び出しは有料枠では利用token量に応じて課金されるため、
...
async def test_real_gemini_call_returns_structured_items() -> None:
    ...
    try:
        ...
        warnings.warn("Real Gemini validation skipped: configuration unavailable", RuntimeWarning)
        pytest.skip("GEMINI_API_KEY 不正により skip")
    except LLMParseError as exc:
        exc_str = str(exc)
        if "API_KEY_INVALID" in exc_str or "API key not valid" in exc_str:
            warnings.warn("Real Gemini validation skipped: invalid API key", RuntimeWarning)
            pytest.skip(
    ...
    cost = calculate_cost(result.input_tokens, result.output_tokens, model=result.model)
    print(
