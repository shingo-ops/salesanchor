...
async def _ensure_tenant_006_schema(
    ...
    async with admin_engine.begin() as conn:
        ...
        """))
        await conn.execute(text("""
            CREATE TABLE IF NOT EXISTS tenant_006.tenant_sales_form_options (
                id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
        ...
        """))
        await conn.execute(text("""
            CREATE TABLE IF NOT EXISTS tenant_006.lead_sales_form_selections (
                id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                lead_id INTEGER NOT NULL REFERENCES tenant_006.leads(id) ON DELETE CASCADE,
                option_id INTEGER NOT NULL REFERENCES tenant_006.tenant_sales_form_options(id) ON DELETE RESTRICT,
                other_text TEXT,
                created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
