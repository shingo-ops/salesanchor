...
def test_postgres_analysis_replay_and_distribution(pg, monkeypatch):
    ...
    with connection.cursor() as cur:
        for i, unit in enumerate(recovery._UNIT_MASTER_ROWS, 1):
            # Insert with explicit id matching _UNIT_MASTER_ROWS.unit_id (9–16) so that
            # apply_unit_recovery_for_job() FK references resolve correctly.
            # public.units uses GENERATED BY DEFAULT AS IDENTITY, allowing explicit id.
            cur.execute('INSERT INTO public.units(id,code,canonical,kubun,is_active) VALUES (%s,%s,%s,%s,true) RETURNING id',
        ...
        cur.execute(f"""INSERT INTO public.tcg_status_master
            (status_id,canonical,search_pattern,exclude_pattern,priority,match_type,effect,enabled)
            SELECT status_id,canonical,search_pattern,exclude_pattern,priority,match_type,effect,enabled
