...
【非稼働・履歴保存用】2026-06-28 に run_all_migrations.sh から呼び出しを停止。
  停止理由:
    - メンバー→CS 移行・メンバー削除は全テナント完了済み（役目終了）
    - 毎デプロイ全テナント seed が DEFAULT_ROLES 変更のたびに旧新ロールの重複を引き起こす構造的欠陥
    - 新テナントの初期ロール seed は create_tenant_schema / setup_tenant.py / setup_review_tenant.py が個別に担保済み
    - 将来の既存テナントへのロール展開は --tenant-id 必須の明示スクリプト（migrate_*_stage_*.py）で実施する
  本体はコード履歴・経緯参照のため削除せず残置。削除は別便で検討。
...
変更履歴:
  2026-04-16: 初版作成
  2026-06-28: run_all_migrations.sh からの呼び出しを停止（役目終了）
"""
