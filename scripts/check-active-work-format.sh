...
if [ "$ERRORS" -gt 0 ]; then
  ...
  echo ""
  echo "   状態の種類: IN_PROGRESS / REVIEW / BLOCKED / DONE"
  echo "   列の意味:"
  echo "     状態=IN_PROGRESS + main空欄 → 開発中"
  echo "     状態=REVIEW + PR# + main空欄 → PRレビュー中"
  echo "     状態=DONE + PR# + main空欄  → develop反映済み・本番未反映"
  echo "     状態=DONE + PR# + main日付  → 本番反映済み"
  echo ""
  echo "   修正方法: 各行に main 列(空欄可)を追加してください"
