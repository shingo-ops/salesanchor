...
# 教訓の還流先: generatorの指示違反が起きたら、本ファイルの実行ルール節に禁止例を追記する便を立てる。
# 矛盾の再発防止: Plannerカードと本プロンプトの食い違いを発見したら、実行者に賭けさせず本ファイルを直す。
# 関連正本: .claude/agents/generator.md / docs/agents/generator.md（実行規律の正本・PR #2714で整備）
