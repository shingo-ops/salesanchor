...
変更後の正規表現（案）:
```
^(\+\d{6,15}|\d{10,15}|0\d{9,10})$
```

| パターン | 対象 | 根拠 |
|---|---|---|
| `\+\d{6,15}` | 新規入力（国番号コンボ結合後・E.164範囲） | 本ADR §4 国番号コンボ導入 |
| `\d{10,15}` | 旧データ互換（+なし10〜15桁） | 旧Googleフォーム時代の既存DB値（例: 15551234567）。スタッフ側 AddressInput 編集時に既存値が弾かれないよう維持 |
| `0\d{9,10}` | 日本国内形式（0始まり10〜11桁） | 従来通り |

> `\+?\d{6,15}` に一本化しない理由：+なしで6桁まで緩めると明らかな入力ミス（123456等）まで通るため、+なし下限は旧仕様の10桁を維持する。
...
- [ ] +なし10〜15桁の既存形式（例: 15551234567）がバリデーションを通過する（旧Googleフォーム時代のDB値互換）。
