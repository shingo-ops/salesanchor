...
- **社内ロールのみアクセス可。** `tenant_admin` / `tenant_staff` ロールのみスコア閲覧・上書き可能。顧客・外部ユーザーはアクセス不可（既存 auth middleware の `role_required` に準拠）。
- **社内専用。** 顧客には見せない。
- **不確実性に正直。** 確信度・母数を必ず添え、データ不足では断定しない。
- **説明可能。** 根拠は"実際に数えた信号"に紐づく（作文しない）。
- **定期再較正と自己成就防止。** スコアは静的な判決ではなく定期的に更新される。スコアが実際の成約率から乖離した場合は較正リセットを提案し、高スコア客だけを追う偏りを防ぐ。
...
2. **状態検証スクリプト（定期バッチ）:** 以下をすべて点検する
   > **較正クエリは inbound ラベルのみ使用（自己成就回避の設計根拠）**
   > `calibrate_tenant_weights()` は `WHERE ml.direction = 'inbound'` でフィルタリングし、
   > outbound（営業の返答）は較正に含めない。理由：outbound を客の採点に使うと、
   > 「スコアが高い客に多く話しかける → 自動的に成約率が上がる」という自己成就
   > バイアスが較正データに混入し、スコアが実際の顧客ポテンシャルではなく
   > 営業活動量を反映してしまう。inbound（顧客側の行動）のみ使うことで
   > 顧客自身の購入意欲を独立変数として計測できる。
   - 較正の鮮度（最終較正日 ＋ 蓄積件数）
   - 入力データ量（テナントごとのメッセージ数・成約/失注件数）
   - 確信度分布（低確信度割合が閾値超なら警告）
   - スコア異常（分布が極端に偏っていないか）
   - **ドリフト検知:** 直近 30日のスコア分布と前期比較。特定ティア（要観察/有望/最有望）への集中率が前期比 ±20pp 超過で警告
   - **自己成就監視:** テナントの「最有望スコア客の実際成約率」と「要観察スコア客の実際成約率」の差が縮小していないか追跡。差が期待値の 50% 未満に収束したら較正リセットを提案
   - **定期再較正サイクル:** 月次バッチ実行（月初）または新規成約/失注が累計 10 件追加されたタイミング（いずれか先に到達した方）
3. **監視・通知:** 以下を Discord 通知（`discord_notifier.py` 流用）
   - データ不足（テナントのメッセージ数 < 20 または成約/失注件数 < 5）
   - 較正失敗
   - スコア急変（個別顧客のスコアが 1 ティア以上変動）
   - ドリフト閾値超過（②で検知）
   - 自己成就警告（②で検知）
