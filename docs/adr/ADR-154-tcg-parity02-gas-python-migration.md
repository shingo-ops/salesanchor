...
### Why追補: 試行記録の構造検証と容量超過メタデータ（2026-09-14）

固定e594d3efの実コードと隔離PG16.15で、同名CHECK(true)・接続列違いFKを既存検証が受け入れること、parsed JSONが8,388,609bytesのとき失敗は保存されてもparsed_bytesがNULLであることを再現した。サイズ保存候補は8,388,608bytesの成功を維持し、1byte超過を失敗のまま既存行へ実測値保存。構造候補は正常7受入・不正8拒否・未知等価式1停止、比較前後の試行行不変16/16を確認した。本番READ ONLYでは2schema各9制約が正規期待値と一致し、実不正構造の修復が必要とは確認されていない。

そのため新しい診断表を追加せず、既存extraction_attempts.parsed_bytesと正規DDLの構造検証を補強する。未知の構造は自動修復せず停止し、過去の欠落値を推測補完しない。候補試験を正式実装や本番反映の合格に転用しない。設計・受入・試験限界は docs/handoff/pmg-import-delivery-ssot/design.md の限定是正追補、証跡は reports/pr3494-evidence-20260914/。本追補は既存の試行記録SSOTを維持する限定修正の理由であり、新たなPO GOを代筆するものではない。
