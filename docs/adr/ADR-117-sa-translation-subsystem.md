...
| 送信ガード | **英訳プレビュー経路（`/outbound-preview` → `/outbound-confirm`）を使った送信のみ確認ゲート対象。日本語直送り（既存送信ボタン）はゲート対象外** |
| 商品名原語保持 | 商品マスタ seed 時は `target_text = NULL`（訳さない）で登録。英語商品名は注文・在庫の一意キーのため翻訳しない。`name_ja` は `notes` に退避（参照用） |
...
3. 送信フロー（固定）: 日本語下書き → 英訳プレビュー → 人が編集可 → 「確認して送信」のみ。**英訳プレビューを使った送信のみ確認ゲート対象。日本語直送りはゲート対象外**（テストで担保）
4. 受信は原文 + 和訳を併記表示
