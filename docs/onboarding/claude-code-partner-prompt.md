...
- **初回だけ手動 pull が必要**（母艦＝Claude Code を起動するディレクトリ）。これでルール本体とフックが入る:
  ...
  git checkout main          # main にいることを確認
  git pull --ff-only origin main
  ```
- **以後のタスクは worktree で作れば自動的に最新**。`bash scripts/new-worktree.sh release/<topic>` は毎回 origin/main を fetch して最新から枝を切るため、worktree 内の CLAUDE.md/SOP/FEATURE-INDEX/フックは常に最新。
...
- **作業は必ず worktree で**: `bash scripts/new-worktree.sh release/<topic>` で独立ディレクトリを作ってから着手（毎回最新 origin/main から枝が切られる＝中身は自動で最新）。`develop` / `main` に直接コミット・push しない。
...
5. **検証ゲート**: Reviewer＋Evaluator（＋CI 必須チェック）を通して main へマージ。
...
# 3. ブランチ / PR / マージ
- `release/<英語で簡潔>` を main から作成（worktree）。
- 完了後 `gh pr create`（--base 省略で main が自動設定）。PR 本文に **Merge stage 宣言**＋`### 標準ワークフロー確認`（対象 ADR / recon パス / 設計パス）を必ず入れる。
- **release/* → main のマージは人間ゲート（PO）**。**main へは必ず merge commit**（squash 禁止・ADR-050）。
...
- 関連: `docs/meetings/2026-06-11-claude-code-dev-best-practices.html`（MTG 資料 A〜D）、ADR-050（リリース運用）、ADR-056（develop 自動 merge・#2715 で役目終了）、ADR-042（運用ガードレール）、ADR-121（process-artifacts gate）。
