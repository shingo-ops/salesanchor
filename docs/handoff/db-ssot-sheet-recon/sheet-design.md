...
| Q11 費用と通貨 | まとめ仕入・まとめ出金の実費配分基準 | 個別特定を優先し、共通分は**均等（注文数で等分）**で配分（2026-09-18確定） | P6-原価、C35/C39/C49。配分方法を確定 |
...
| Q14 現物の識別 | 不具合品と良品の取り違え防止方法 | **1個ずつ管理**。発注ID→在庫ID→販売ID→返品ID→検品IDをIDで紐づけ。在庫テーブルが状態のSSOT（コピー禁止）。移動履歴で経路を追跡。状態変更記録は「いつ・誰が・何を・どこで・どれぐらい・どうやって・なぜ（状態変更が必要だった理由）・結果」の8項目（2026-09-18確定） | C14/C17/C25/C26の識別方法を確定 |
| Q15 費用と通貨 | 仕入元残額を異通貨仕入に使う対象に含めるか | 仕入れは全て日本円のみ。外貨での仕入れはない。異通貨設計は仕入側に不要 | C50は顧客側のみ適用。仕入側の異通貨設計を対象外に確定 |
| Q16 待機期間 | 入荷待ち・返事待ちが長期化した場合の自動終了 | 自動終了しない。**初期値1日**で担当者に通知。通知期間設定はフロントエンドで人間が追加・編集・ON/OFF切替可能、設定値はDBに保存（2026-09-18確定） | C28の待機期限を解消。通知設定をDB後続化 |

この16項目はEV104の8分野を具体化した判断票。PO回答により**16件全て解消**（2026-09-17に13件、2026-09-18に残り3件を確定）。UIアイデア（Q11費用配分モックアップ4版）は本採用ではなく参考記録として別途保管。
