...
| 引用先 | 確認内容 |
|---|---|
| `scripts/check-process-artifacts.js:42` | DOCS_PATTERNS の定義。拡張子 md のファイルが docs 区分に落ちる |
| `scripts/check-process-artifacts.js:65` | DANGEROUS_PATTERNS。標準ワークフロー正本と関所自身は既に自己保護済み |
| `scripts/check-process-artifacts.js:101` | classifyChanges。hasDocsOnly の算出箇所 |
| `scripts/check-process-artifacts.js:104` | hasDocsOnly は危険もコードも無い時に true になる |
...
| `scripts/tests/test-process-artifacts.js:613` | 柱2-欠落版。宣言なしで正本を触ると fail |
...
| # | 不明点 | 解消方法 | 状態 |
|---|---|---|---|
| 1 | 拡張子 md を一律で判定すると台帳を巻き込まないか | 台帳パスが正本4パターンのいずれにも該当しないことを実測 | 解消済み |
| 2 | 正本の実数と網羅範囲 | git ls-files で実測。あるべき姿42・KGI41・エージェント正本12・全体正本2 = 97件 | 解消済み |
...
- 過去の書類のみPR5件（3012・3005・3004・3002・2991）に本ガードを当てた試算では、5件とも宣言完備で pass 想定、fail 想定は0件。
