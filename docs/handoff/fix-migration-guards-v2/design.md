...
## 対象ADR
- ADR-082: scripts/run_all_migrations.sh 冪等性要件（docs/adr/ADR-082-deploy-skip-migrations-on-frontend-only.md）
...
既存テスト test_migration_partial_structure_rejects_before_insert の動作は変わらない。
...
守り手: migration-test.yml が CI で冪等実行テストを行う（tenant_004 削除後の再実行シナリオ含む）。今後のマイグレーションで dropped テーブルを参照する場合は同様のテーブル存在ガードを必須とする。

- to_regclass() / information_schema.tables によるテーブル存在確認
- 既存テスト: test_migration_partial_structure_rejects_before_insert （部分削除は依然 EXCEPTION）
