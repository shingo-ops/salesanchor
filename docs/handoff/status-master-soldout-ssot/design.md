...
### C. scripts/run_all_migrations.sh
...
内部 SSOT 統合のため、外部事例は該当なし。既存の resolve_status_v2 ロジックは GAS の SystemResolverV2.gs と対称設計になっており（docs/adr/ADR-154-tcg-parity02-gas-python-migration.md 参照）、除外パターンの仕組みも GAS 側で実績がある。今回はその対称性を Python 側に補完する。

## 維持の仕組み

守り手: backend/tests/test_tcg_completion_safety.py（既存テスト・resolve_status_v2 動作確認）、本番マッチングテスト（実運用データで exclude_pattern 効果を確認）

## ADR 参照

- docs/adr/ADR-154-tcg-parity02-gas-python-migration.md — tcg_status_master SSOT方針
