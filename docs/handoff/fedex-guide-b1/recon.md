...
| `docs/adr/FEATURE-INDEX.md:17` | FedEx / shipping 関連ADR一覧（ADR-103 / ADR-123 / ADR-128 / ADR-125 / ADR-129）|
...
| `frontend/src/pages/integrations/CarrierIntegrationPage.tsx:420` | 連携ガイドタブから FedexLabelValidationTab を呼び出す箇所 |
...
| `frontend/src/pages/integrations/FedexLabelValidationTab.tsx:153` | lv-wizard ルート要素 |
| `frontend/src/pages/integrations/FedexLabelValidationTab.tsx:154` | 現行タイトル（lvTitle キー使用、置き換え対象）|
...
| `frontend/src/locales/ja.json:249` | lvTitle: 現行タイトル（置き換え対象）|
| `frontend/src/locales/en.json:226` | tabIntegrationGuide: "Integration guide" — タブ名（英語）|
| `frontend/src/locales/en.json:249` | lvTitle: 現行タイトル（英語、置き換え対象）|
...
- ADR-129（fedex-label-validation-wizard）: J4 で Label Validation 申請支援タブとして実装済み。B-1 ではこのタブを「連携ガイド」全体として2部構成に拡張する
