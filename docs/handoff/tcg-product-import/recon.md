...
PR3489初回CI（HEAD0a4a7bfc、base313d7796）：process gateは番号GO欄不足で停止。親が後段の設計/維持/引用validatorを個別実行し全配列空、PR設計パスの末尾§21は別行へ修正。これは正式GO検査の合格ではない。secret scan job103738549601はd56649c5のroundtrip-design-evidence.json13行、frontend/src/lib/api.tsをキーにしたSHA256をgeneric-api-keyとして検出。親が元baseの実ファイルを再計算し10/10一致、秘密ではないことを確認。証拠JSONを明示的なpath/sha256レコードへ変更し、値10件を保持。gitleaks8.30.1を既存.gitleaks.tomlで当該文書へ実行、exit0/no leaks。検査設定/allowlist/承認条件は変更しない。

誤検出はgit履歴の古い記録を対象にしている（CIの実git log範囲d56649c5^..0a4a7bfc）。追記commitだけでは消えないため、旧branch/PRの証拠を保持し、origin/mainから正式な新規作業場所へ検収済み13製品の同一内容と修正済み9文書を保存してPRを作り直す。既存branchのforce-push/履歴上書きはしない。新しいPRでも通常secret scan/実PG等を実行する。旧PRは新PRの作成確認後に置換先を記録して閉じる。実装再設計/本番データ操作ではない。

### DETAIL-01 GO受領後の環境復旧・CSV統合（2026-09-14 09:39 JST転記）

PO原文「進めるGO #3492」を受領後、ローカル起動ガード欠落で全操作が停止。POから「復旧したので再開してくれ」を受領し、executor-preflight exit0を確認して再開。ガードの再作成/変更は本セッションでは未実施。PRはOPEN、HEAD0f38bbf3、停止中にmain ae5248f4（PR3490 CSV往復）が追加され、両機能を保持して統合。台帳/仕様の双方の追記も保持した。
英語名/型番検索は一覧だけ拡張済みだったため、roundtripのCSV検索条件も一致させた。既存詳細PG試験へ英語名/型番出力2件、CSVと詳細の古い版の相互拒否2件を追加。追加製品差分は検索条件1箇所のみ。新たなAPI形式や本番データ操作なし。統合後の検証はPR3492の最新checksを正本とし、過去CI成功で代替しない。
...
PO原文: 「合意した、この切り替えを推測は禁止して事実確認を怠らずに確実性を重視して最も効果があり、現状把握の粒度が細く、精度が高いエビデンスを確立して安全に進めてくれ、確立したなら進めたいが依頼してもいいか？」
CSVを日常更新に使い、DB構造変更と初期登録を分ける方向への合意。以下の具体方式・受入条件は設計草案であり、実装承認の代筆ではない。

### 観測と根拠

1. 実配備: PR3492はmerge e39fa65abb83837a7909290b993a8cd363a11ef9。配備34794455633は旧bundle migrationでPM0264 identity mismatchとなり失敗。さらにmain59f644cdの配備34795225002も2026-09-14T01:16:26Zに同じ理由で失敗した。後者の232番略語追加には到達していない。手動適用の有無はこのログだけでは確定できない。URL: https://github.com/shingo-ops/salesanchor/actions/runs/34795225002 。前便の詳細結果は未マージのPR3496に分離保存しており、本便で上書きしない。
2. 実CSV履歴: job 9f74c3c8-443b-4069-9d48-e260a63cf643、2026-09-14 07:44:18 JST、297行中作成0・変更なし285・更新12。履歴の変更34項目と10:37:49 JSTの現在値を照合し、一致24・差10。差はPM0276–PM0284の検索語9項目、PM0263の除外語1項目。商品名の変更は保持。今回編集していない英語名や型番の実被害は主張しない。
3. 追加される経路: `20260905_010000_tcg_pokemon_master_batch1_t004.sql:89` 以降の検索語追加は9商品の「カードセット …」を再投入する。`20260913_200000_tcg_cardset_exclusion.sql` はPM0263に「カードセット」を再投入する。現在の10差分と一致する機構が存在し、失敗前にこれらの登録順を通る。各行の変更時刻・操作者の独立監査記録は未取得であり、排他的な人物特定はしない。
4. 再実行機構: `scripts/run_all_migrations.sh:49` のrun_pyと61のrun_sqlは毎回実行し、適用済み履歴の照会をしない。232登録（SQL206/Python26）。途中失敗で全ファイルを一括rollbackする仕組みではない。成功表示も末尾16登録より前にある。DDL187/189は同一ファイルの重複登録。
5. 対象表: 本番tenant_004は商品297・検索語676・除外語285、参照4表は3/11/5/2件。QA tenant_001は商品/検索/除外0、参照4表は3/11/5/2件。10:39 JSTの14表の全行指紋をJSONへ保存。10:46 JSTに両schemaの対象7表の非内部トリガー0を確認。すべてPGOPTIONSでtransaction_read_only=onのSELECT/SHOW。本番書込0。
6. CSV契約: `backend/app/services/tcg_product_roundtrip_svc.py:22` はproduct_code/revisionと既存10列の12列。94のsnapshotは7表、259以降のロック/再照合、300以降の更新、320以降の履歴を同時確定する。旧新規CSVは `backend/app/services/tcg_product_import_svc.py:17` のとおりid/code/category_class/is_activeを入力しない。CSV出力をUUID・参照関係まで復元するDBバックアップとみなせない。
7. 自動検査: `.github/workflows/migration-test.yml:1297` と1333以降の2周検査はSQLファイル名を `0*` / `20260604–09*` / `2026061*` / `2026062*` に限定し、今回の202609ファイルを含めない。CSV編集を間に挟む処理もない。既存TCG PG個別テストは存在するが、この本番経路の保全証明と同一ではない。

実データ証跡: [migration-separation-evidence.json](migration-separation-evidence.json)。
登録順・ソースSHA256・一致行: [migration-separation-inventory.json](migration-separation-inventory.json)。検索対象7表名の文字列一致は21登録/20固有ファイル。動的SQL・Python経由の間接呼出全体を証明するものではない。

### 分離候補の分類

| 登録順 | ファイル識別部 | 観測した責務・扱い案 |
|---|---|---|
| 187/189 | 20260831_110000 | 18表の構造作成を維持。重複登録の整理は別途差分根拠が必要 |
| 192 | 20260902_110000 | 分類4表の構造と21参照値の初期登録。既存値の上書きは禁止 |
| 193 | 20260902_110100 | 4外部キー制約追加と268商品の分類書換えが混在。構造を残して過去補完を分離 |
| 198 | 20260903_180000 | mark/english_title追加と268商品の無条件書換えが混在。構造を残して過去補完を分離 |
| 204/205/206 | 20260904_160000 / 20260905_010000 / 020000 | 既存名等の書換え・3件/25件登録・語追加。通常配備からの退役候補 |
| 210 | 20260906_120000 | QAの27表と分類21件・supplier/channel初期値。構造・空環境初期値を維持 |
| 214 | 20260907_140000 | 商品無効化・語修正と退避表作成が混在。業務データ整備として退役候補。単なるDDL-onlyではない |
| 215 | 20260906_130000 | CSV履歴2表の構造作成を維持 |
| 218/223/224 | 20260908_170000 / 20260910_160100 / 170000 | 語の置換/追加/削除、可変商品名による停止。通常配備からの退役候補 |
| 230/231 | 20260913_200000 / 210000 | 語追加・12商品の固定名検査・PM0297登録。通常配備からの退役候補 |
| 232 | 20260914_080000 | 新しい略語追加。最新配備で未到達。適用済みと偽って記録しない |
| 63/99/114/157 | 旧TCG/属性/public商品関連 | 文字列一致の雑音。今回の商品CSVが触るtenant商品表と区別 |

`scripts/migrate_inventory_sprint1.py:53` の間接SQLリストも確認した。文字列一致候補から外れる全呼出先をまだ網羅したとは宣言しない。public.productsを使う227–229の別商品登録も今回のtenant商品CSVとは別物。

### 7観点と未解決

全体入口（配備→runner→SQL→tenant表→CSV/詳細/解析）、共通部品（構造/参照表/既存CSV）、個別処理（上表）、規則（ADR045/082/113、商品マスタ親仕様、data-durability）、機械的守り（CIの対象漏れ）、理想との差（編集保持と固定名検査の衝突）、雑音/境界（public.products・別台帳・トリガー）を整理した。

残る確認は、間接呼出の網羅性、空環境で必要な初期投入経路、隔離DBでの復元成功、CSV編集後2回実行の実測、232番を含む切替時データ差分の扱い。台帳の「未着手」等は現在の完了事実と断定しない。バックアップ生成成功だけを復元成功とは扱わない。実装カードは未発行。

外部企業事例は本件固有の再実行経路の安全性を証明できないため採用しない。数値根拠は当該DB/コード/実配備ログから得た。Context7は利用可能ツールに見つからず、新規ライブラリ/API方式を根拠未確認で採用していない。

文書検証: check-task-state終了0、git diff --check終了0、design/親READMEの節番号検査成功。reconの既存節1–7重複で見出し検査は終了1。同基点HEADを/tmpへ取り出して同じ7重複・同じ行番号を再現し、本追補起因ではないことを確認した。既存記録を変更して検査を隠さない。7文書を専用作業台へ保存、commit/push/PR提出は未実施。

### 2026-09-14 分離調査の追加照合（11時台に再開できる保存点）

作業基点は59f644cd。preflight終了0。最終照合でorigin/mainが進んでいることを検出し、以下の末尾へ差分を追補した。7文書の前便未保存変更を保持した。以下は本担当が直接実行した読み取り確認であり、実装役からの成功報告の転記ではない。

- **間接呼出**: 登録Python26本のASTからSQLファイル引数32本/32固有を抽出し、SHA256とimportを既存inventory JSONへ追記した。直登録SQL205固有との和集合は237SQL。アプリimportは `country_codes.parse_country_code` だけで、ヘルパー本体は国名正規化でありDB操作をしない。動的なschema/tableの組立ても確認し、036はdeals/orders/quotes/invoicesの前提検査、20260602_010000はquote_items/invoice_items/purchase_order_itemsと旧public商品への処理だった。静的依存範囲の根拠が増えたが、未登録の手動操作や全runner実行結果までは保証しない。
- **固定名検査の全対象**: 本番10:57:48 JST、bundle migrationのVALUES12行をそのままSELECTの比較表として使い、現名と一致1・不一致11を確認。PM0264/0265とPM0276–PM0284。不一致先頭1件だけ緩める案では残り10件に対応できない。
- **混在migrationの実値比較**: 本番10:56:24 JST、193/198のVALUESを実物から抽出しSELECTで比較。分類268件全件照合でPM0267の品目参照1差。型番/英語268件全件照合でPM0190の型番/英語、PM0200の型番、PM0267の英語の計4差。合計3商品5項目。後続204/206が再訂正する箇所を含むため、これは「単独処理が上書きする値」の投影であり、全runner終了後に5差残るという主張ではない。DMLは実行していない。
- **構造との照合**: 本番10:59:11 JST、PostgreSQL16.13、対象7表と商品参照元の制約22件がvalidated=true。分類欠損0。商品の参照元は語2表に加えanalysis_resultsとproducts_logisticsもある。復元時は7表の件数比較だけで終えず、これらの参照も検査する。
- **新規環境の追加問題**: 本番には `uq_sk_product_keyword` / `uq_ek_product_keyword` のUNIQUE(product_id,keyword)が存在する。初期DDL187とQA DDL210には同制約がなく、migrations/scripts/backend/tests/本handoffの名前検索も0件。これだけで誰がいつ追加したかは確定しない。205/232のON CONFLICT(product_id,keyword)はこの差に依存する。Context7ツール0件を再確認し、[PostgreSQL16公式INSERT仕様](https://www.postgresql.org/docs/16/sql-insert.html#SQL-ON-CONFLICT)を直接確認した。該当unique indexを推論できなければエラーになる仕様。空環境での実エラーは未実行であり、仕様とDDLから導く予測として区別する。
- **訂正**: 前便reconの193を「4参照列追加」と表現した箇所は不正確だった。4列は187のDDLに既にあり、193は**4外部キー制約の追加**と268件の補完。下記分類表の記載を訂正した。誰かの改変と断定せず、自分の記述の精度を上げる。
- **復元手順の不一致**: `docs/B-09_restore_test_procedure.md` の接続は旧IP・myapp_user/myapp_db、同一ホストrestore_test作成/削除で、現行の接続と隔離を満たすと確認できない。data-durability-design/design.md:79はtenant_006をQAと記載するが、現行design-partnerではMeta Review専用で利用不可。この旧手順をそのまま実行しない。復旧演習の台帳記載だけから現在の完了/未完了を断定しない。
- **実行環境**: docker infoはsocket不在で終了1。ローカルPostgreSQL16.15のバイナリは存在するが、起動/初期化していない。既存PG fixtureは本物のCI、localhost、jarvis_test_dbを必須とする。GITHUB_ACTIONSの偽装やガード解除は行わない。

最新配備は34795225002 failureのまま。上記原データと観測日時はmigration-separation-evidence.jsonのfollowup_20260914に保存。根拠が確立したのは固定名検査の11不一致・混在処理の5差・間接依存32SQL・本番と初期DDLの制約差まで。隔離DBでの候補処理実行と復元は未実施であり、切替安全性の確立とは区別する。

最終main追補: origin/main e69da6ed7945a5fa8c16b32b89b02d8cfb7967ea（PR3494）が入り11commit進んだ。登録は233（SQL207/Python26）、固有SQLは間接込み238。追加233番はextraction_attempts構造作成であり通常配備に残す。既存PG fixtureにもこのDDLとschema切替対象追加があるため、旧fixtureの固定SHAを再利用しない。最新配備34797490804は10:58:57 JSTに231/233の同じPM0264検査で失敗、232/233は当該配備で未到達。手動適用有無は未確定。新しい制約・コード差を検証便の基点へ取り込むことが必要。元の232登録の観測は旧基点として保持し、inventoryのlatest_main_deltaへ追加SHAと責務を保存した。

### 2026-09-14 検証便の委任・PR3500追従

直前に提示した「隔離DBでの検証用テスト作成・CI実行だけを別の実装担当へ委任（マージ/本番配備は含まない）」に対し、PO原文「推測は禁止して事実確認を怠らずに確実性を重視して最も効果があり、現状把握の粒度が細く、精度が高いエビデンスを確立して安全に進めてくれ、確立したなら\n報告」を受領。この限定検証への進行指示として扱い、製品切替GOには拡張しない。

公式専用worktree release/product-migration-verification をorigin/main 70d145f090e122dd36e4a39b4928e13cc0dae613から作成、開始時clean/preflight成功。親は設計8文書、委任先migration_verifierは新規backend/tests/test_tcg_migration_separation_pg.pyだけを所有。正式カードはcard-migration-verification.md、初版lint違反0。担当追加/本番接続/既存コード/runner/migration/CI変更は禁止。

最新PR3500は2026-09-14T03:54:17Zにmerge、配備34804164057成功。実ログで231/232/233の実行到達（03:57:09–10Z）を確認。商品名ガードは除去され、型番/英語はCOALESCE、名前訂正は旧パターン条件付きへ変わった。V3は旧失敗期待を捨て、現名変更を許容する現行SQLを検証する。V2はNULLを明示したCSV編集が再充填されるか追加検証する。

13:06:03 JSTに同じCSV履歴34項目を本番readonlyで再照合し、一致23/差11、履歴job総数1。従前の検索語9/PM0263除外1に加え、PM0264除外「カードセット」が現在存在する。bundle SQLにも当該追加があり、今回231が通ったログと整合する。行ごとの排他的変更主体を特定したとは主張しない。配備復旧とCSV保全の完成は別であり、後者は未達。
