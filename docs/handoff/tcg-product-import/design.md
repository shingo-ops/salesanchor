...
## CSVとmigrationの責務分離案（2026-09-14・草案）

### 目的・合意と方式の区別

POはCSVで今後も商品を更新する方向に合意。日常の変更を次回配備で戻さず、必要なDB構造変更を引き続き届ける。具体方式のPO承認、実装GO、マージ・本番GOは本便では未受領。
根拠はreconの同日分離調査と2証跡JSON。これはPlanner作成後に同一AIがArchitectを順に担当する自己審査であり、独立レビューではない。

### 推奨する変更前後と対象

現状は配備ごとに構造変更と過去の商品登録/修正を再実行する。推奨案は、構造変更は通常配備に残し、過去の商品データ整備を通常配備から外す。分類の空環境初期登録、通常の新規商品CSV、更新用CSV、復元をそれぞれ明示する。

対象は商品ページのtenant_004の商品/検索語/除外語および参照4表、QA tenant_001とこれらの初期化経路。既存の一覧・詳細編集・CSV形式・認証は維持。全232migrationを別フレームワークへ移すこと、他業務のpublic商品管理、本番データの自動修復、designサイト復旧は本便の対象外。

実装範囲候補はrunner、recon分類表の混在migration、専用の隔離PG回帰試験、既存migration CIの対象選定、既存テストの登録順契約、正式説明文書。まだ確定カードではない。deploy.ymlや本番scriptsの変更は設計相談の許可で実施しない。

### Why・代替案

- 全migration停止はADR045が守る列・表不足の防止を失うため不採用。
- 今回のPM0264比較だけ緩めても、英語名/分類の上書き、語の再追加・置換が残るため不採用。
- 既存履歴を一律「適用済み」にする方式は232番が配備で未到達という事実に反するため不採用。
- 全体のmigration管理置換は232登録に範囲が広がり、今回の保全目的に対する変更量が大きい。まず商品データの反復処理を限定分離する。
- 採用案はCSVのrevision/全行同時確定/履歴を再利用し、日常更新の入口を維持する。代償は、古い初期商品投入を通常配備に頼れなくなること。空環境初期化と復元の実証なしには切替を合格にしない。

ADR082の「冪等」は同じ入力の繰返しに関する設計で、POが編集した状態の保持とは同義でない。268行の無条件書換えと実CSV10差分がこの区別の根拠になる。正式採用時はADR082のWhy/適用範囲へ追記する案とし、既存決定を本草案だけで上書きしない。ADR113のhandoff形式に従う。

### 分離・導入の契約案

1. 固定した基点の全登録・間接呼出を照合して、残す構造、空環境初期値、退役する業務データ処理、未到達処理を列挙する。履歴保管と通常実行対象を区別し、ファイル削除で済ませない。
2. 混在する193/198では構造作成を残す。既存商品を固定表へ戻すDMLと固定名検査を通常配備から除く。192/210の初期値は既存行不変、新規環境で参照解決可能なことを試験する。
3. 過去処理の退役記録は「再実行対象から除外」と記し、「過去に全件実行済み」と捏造しない。基点SHA・対象ファイルSHA・根拠・未到達処理・切替時snapshot・日時を残す。232番は独立した現値との差分として扱い、配備復旧に便乗して投入しない。
4. 空の新規環境は構造/参照値を作り、新規CSVで商品を登録できることを確認する。既存環境の復元はID/FK/履歴を保持するDBバックアップと正式復元手順を使う。更新CSVをDB復元の代用品にしない。
5. 現在の10差分を元へ戻す操作は分離後の別データ変更。最新CSV履歴とその後の正当な編集を再照合して差分を提示する。古いCSVをそのまま再送して上書きしない。
6. 検証対象の抽出元を通常実行対象と一致させる。今回の202609ファイルが検査に含まれた件数・対象名を出力し、対象0/予期しないskipを成功にしない。既存の必須チェックを外さない。
7. 本番切替はレビュー済み設計・正式カード・明示実装承認の後に準備し、当該PRのGOと最新CI確認を経る。切替直前のbackupと隔離復元結果がなければ停止。旧runnerへ戻すだけでは再びデータを書換えるため、失敗時の自動再実行を復旧策としない。

### 受入条件案と検証方法

| ID | 判定できる完成条件 | 検証 |
|---|---|---|
| M1 | CSV編集後、通常migrationを2回実行して商品・語・参照4表の意図しない差分0、両回終了0 | PostgreSQL16隔離DBで実export→編集→commit→runner相当の実対象2回→全列比較 |
| M2 | 日本語/英語/型番/発売日/4分類/両語の10編集項目が保持 | 268行補完対象、PM0264/0265、9カードセット商品、PM0263、既存語置換12商品を含むfixture |
| M3 | 別tenant/未対象商品/未公開列・有効状態・UUID・参照関係の意図しない変更0 | 前後snapshot/参照整合照合。語の意図した更新以外は行ID/位置も比較 |
| M4 | 空環境の構造/参照作成後に新規CSV登録・更新往復が成功 | 正式migrationを使用。別コピーDDLをfixture正本にしない |
| M5 | 固定backupを隔離復元でき、商品/語/参照/関連FK/履歴の照合不一致0 | data-durabilityの手順と接続制限を使用。所要時間・対象・結果を記録 |
| M6 | stale CSV/不正分類/途中例外で意図しない書込0 | 既存CSV PG試験維持＋分離後の再実行回帰 |
| M7 | CI対象漏れ0、必要PG試験skip0、未到達migrationの虚偽完了記録0 | CI抽出対象と実行manifest照合、旧事故fixtureが旧実装で失敗することも確認 |
| M8 | 切替時の当該HEAD/backup/検査/配備成功と編集保持を照合可能 | 承認後の正式配備ログ、実DB読取比較、アプリ公開確認。healthだけで合格にしない |

### 影響・維持担当

PO/利用者はCSVと詳細編集の変更が配備後も残ることを確認する。実装役は固定カード範囲のみ実施し、Reviewerは間接呼出と新規環境を確認する。機械は対象漏れを検出するCIを維持する。データ面は7表と参照ID/履歴、運用面は配備失敗時の途中確定と復元、外部面は解析結果に使う検索/除外語の保持が接点となる。新規外部サービスは導入しない。

維持責務の案: 商品機能担当が編集保持fixture、配備担当が実行対象/退役記録、data-durability担当が隔離復元証跡を維持する。実名担当の指名済みとは主張しない。今後商品データを通常migrationに追加するPRはこの契約との整合を審査する。

### Architect自己審査

判定 **REVISE（修正必要・実装カード未発行）**。原因機構と全体停止を避ける判断には実測根拠がある。一方、間接呼出網羅・空環境初期化・隔離復元・編集後2回実行は未実証で、確実に安全に切替可能とはまだ認定しない。

自力で続ける次の調査は登録Python26本の呼出先と初期化の依存照合、既存隔離DB試験で再現できる固定fixture/実行範囲の確定。以後、審査合格と正式カードチェックを経て実装担当へ渡す。事業判断として保留なのは未到達の略語追加と10差分修復の具体内容であり、本切替に自動で含めない。本便の調査・文書化を再承認させない。

### 分離案の再審査と検証便の設計（2026-09-14追補）

再審査は **REVISE維持**。単一商品名の検査修正では12対象中11不一致が残り、初期DDLと本番には語のunique制約2件の差がある。従来案のM4/M5を「既存手順を使う」で済ませられない。設計の193は4列追加ではなく4外部キー追加に訂正する。

追加の必須確認:
- 本番のunique制約2件は維持する。初期環境へ同等制約を設ける場合は重複行を勝手に除去せず、重複があれば停止する契約を設計へ含める。今回の通常再実行廃止と、構造差の正式な補完の範囲をカードで固定する。
- analysis_results/products_logisticsの参照保持をM3/M5へ明記する。CSV往復だけで全DB復元可能とはしない。
- 旧復元手順の接続先/DB名/QA用途は転記しない。独立した使い捨てDBで、人工データによる機構試験と実バックアップの復元実証を分ける。

#### 検証便の具体範囲（PO委任待ち・実行カードではない）

目的は、設計の残る前提を実DBで試すこと。製品切替とは別の検証便として準備する。担当案はPOが明示委任する検証専用の実装役。設計担当は成果物を読み取り確認し自己審査へ戻す。委任も起動もまだ行っていない。

成果物候補は `backend/tests/test_tcg_migration_separation_pg.py` の新規1ファイルと、本handoff内の検証結果JSON/追記。実装役が専用worktreeで作成し、製品migration/runner/CI/サービスコードの変更は禁止。既存CI PostgreSQL16と `test_tcg_work_matching_integration.py` の一時DB fixtureを再利用する。CI条件をローカルで偽装しない。テスト追加PRで実CIを走らせる場合もマージ・配備は行わない。

| 検証 | 入力・操作 | 合格に必要な観測 |
|---|---|---|
| V1 | 実物DDLから空schemaを作り、参照4表と商品列/外部キー/語uniqueを列挙 | 本番証跡22制約との差を名前でなく定義で説明できる。既存DDLにない制約をある扱いにしない |
| V2 | 人工商品にCSV相当の10項目を保存し、193/198の構造部分だけを抽出した候補を2回実行 | 編集全列の差0、2回終了0。抽出は元SHA/境界を検査し、元ファイルを変更しない。全runner検証とは呼ばない |
| V3 | 既存bundle fixtureの12商品を本番照合の現名へ変更し旧bundle SQLを実行 | PM0264で失敗し当該transactionの前後差0。11不一致の一覧と整合する |
| V4 | 固定した人工fixtureで205/230の語追加を実行→CSV相当で対象語を外す→再実行 | 検索語9/除外語1が戻る機構を再現。外部入力に依存しない値/件数と全SQLハッシュを出す |
| V5 | uniqueなしの初期DDLと、本番同等unique付きfixtureを別DBで比較 | 205/232の明示conflict targetの成否を実測し、同じfixtureで混同しない |
| V6 | 人工DBのdumpを別の一時DBへ復元 | ID/全列/語順/参照先/履歴不一致0。人工データの復元であり、本番backup復元成功とは報告しない |

人工fixtureの構築に業務データの自由な創作を混ぜない。値は「検証専用」と明示し、固定されたmigrationのVALUESと既存fixture・公開済み本照合値から最小限を構成する。未知の268商品の現値を模造して「本番同等297商品」と報告しない。元の全文runnerは固定コンテナ名・本番設定を読むため、この検証で直接起動しない。

停止条件は、接続先がCI一時DB以外、元SHA不一致、fixture安全条件不成立、範囲外ファイル変更、期待結果不一致、実バックアップやsecretsが必要となった場合。結果を読取確認してから切替方式・正式実装範囲を再審査する。V1–V6の成功はM1–M8の全達成ではなく、本番切替GOでもない。最終の全実行対象2回/実backup復元は後続の正式計画に残す。

この検証計画に対する自己審査は **APPROVE（検証便の範囲のみ）**。製品方式はREVISEのまま。POが委任を承認した後、担当・作業台・対象HEADを固定した正式カードを作成しcard-lintを通す。現時点で実装可能な切替カードを発行しない。

検証便の基点更新条件: 最新main e69da6edでは233番extraction_attempts構造と既存PG fixtureが更新されている。検証用worktreeはその時点のorigin/mainから作り、最低限この追加を保持する。旧232件manifestを最新の全件と扱わない。商品退役候補以外の新構造を停止しない。切替の全runner実証は233番を含む実行対象全件が条件。

### 検証委任後の現行仕様への追従

PR3500/70d145fで停止原因の名称比較が除去され、配備34804164057成功を確認した。V3は名称変更成功・構造破損rollbackの検証へ変更する。V2に現行COALESCEの非NULL保持/明示NULL再充填の比較を加える。V4は元の9+1の再現に加え、bundleでPM0264除外語が再追加される現行経路を測る。これは検証計画の改訂であり製品変更ではない。自己審査APPROVE（検証限定）。切替REVISEを維持する。

本検証便の対象ADR: ADR-045（構造不足の防止）、ADR-082（通常migration経路）、ADR-113（設計持込と根拠照合）。

### 実CIと切替判定の境界（PR #3502）

初回実PGは1失敗を検出し、検証SQLのbindと観測出力だけを修正した。再試験34807228083/9e954af8の実ログを確認するまで検証完了としない。

V1–V6が成功しても、M1–M8を一括合格にしない。M1の正式全実行対象2周は未実施。M2は人工1商品10項目保持と個別上書き経路の再現まで。M3は限定fixtureのID/参照保持まで。M4は元SQL構造断片からの初期化までで実CSV登録往復は未実証。M5は人工dump/restoreまでで本番backup復元は未実証。M6は既存CI試験との整合確認対象。M7は新規9ケースの実行を観測するが全runnerの対象漏れ検査を未変更。M8の本番切替は未着手。

次の設計便は、今回実証した構造/業務データの境界を233登録・間接呼出のmanifestへ適用し、退役候補の厳密な実行対象と初期投入手順を確定する。実装前に全対象2回・実CSV往復・既存ID/関連参照・本番backup隔離復元の実施手順をカードへ固定する。これは本便の検証合格から本番GOを自動発行する手順ではない。
