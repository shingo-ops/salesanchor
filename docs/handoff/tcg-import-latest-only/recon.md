...
実行: `/private/tmp/line-restock-probe.py`（ローカル検証ハーネス）。実コードの関数ASTだけを読んで、`parse_extraction_response` → `build_note_ja` と `resolve_status_v2` を実行した。原文全体のAI抽出ではなく、原文から切り出した備考を手動で9列応答へ入れた。商品名は商品Aへ置換。状態も手動ラベル。実DBのマスタは未照会・未変更。
