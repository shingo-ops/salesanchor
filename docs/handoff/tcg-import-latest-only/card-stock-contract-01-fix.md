...
# CARD-LINE-STOCK-CONTRACT-01-FIX
...
許可: 前便で作成した以下4ファイル内の限定修正、同ファイルの部品試験、読み取り調査。実装は手順4で指定する範囲に限り、実装方法の選択は設計契約内で行う。
...
手順2: 作業ブランチと未保存変更を確認
    ...
    期待する出力: 上記ブランチで、前便の新規4ファイルと設計担当の本追加カードのみが未保存。その他の変更があれば停止して全文報告する。
...
手順3: 正式設計の内容一致を確認
    ...
    期待する出力: be63630b27e47200e0edc23a01e9103cd9af487112fc29405ed22d7db78dd311。一致した場合のみ次へ。

手順4: 限定修正
    tcg_stock_evidence.pyの_shapeは辞書を検証後に返す型付き関数とし、最上位parsedをその戻り値で受け直す。
    原文一致・例外・全件拒否の契約は維持し、type: ignoreなどの検査抑制を追加しない。
    test_stock_evidence.pyの既存6試験群内に、範囲内なのに逆順の2行と重複範囲を拒否する例を追加する。
    逆順はsource=A改行B、raw=B改行A、spans=2行目→1行目。重複はsource=ABC、raw=AB改行BC、spans=[0,2],[1,3]。
    7raw項目とclausesについて、空でない原文一致の正常例と各項目の不一致拒否例を追加する。
    数量部品の仕様変更、既存処理への接続は禁止。他者の変更を戻さない。

手順5: 部品テスト
    cd /Users/tanizawashingo/worktrees/salesanchor/release-line-stock-message-design/backend && python3.12 -m unittest discover -s tests/stock_contract -p 'test_*.py' -v
    期待する出力: 0件収集でなく、両モジュールの6試験群が収集され全成功。全backend試験やAI精度の合格とは報告しない。

手順6: 新規部品の型検査
    cd /Users/tanizawashingo/worktrees/salesanchor/release-line-stock-message-design/backend && /private/tmp/line-stock-check-py312/bin/mypy --cache-dir /private/tmp/line-stock-worker-mypy app/services/tcg_stock_evidence.py app/services/tcg_stock_quantity.py
    期待する出力: 新規2モジュールの型エラー0。親がPython3.12の隔離環境に既存固定版を用意する。実在しない場合は停止。
    全体make lint-ciの再実行と安全性検査は設計担当が行う。前便の失敗を成功へ読み替えない。
...
手順8: 範囲検査
    ...
    期待する出力: 前便の指定4ファイルと設計担当の本追加カードだけが新規。その他の差分0。新規ファイル全文と試験出力を設計担当へ返して停止する。
...
実行履歴: このカードは実施済み。実行時の設計hashはprobe-20260913.jsonへ保存。上記hashは検収状態追記後の現行設計に追従したもの。新規再実行・範囲拡大の許可ではない。
