...
| 残課題 | 未達箇所 | file:line |
|--------|---------|-----------|
| R1 | write_conversation_log() が contact_id を INSERT しない | `backend/app/services/conv_log_writer.py:29-42`（引数に contact_id なし）/ `backend/app/services/conv_log_writer.py:64-91`（INSERT に contact_id なし） |
