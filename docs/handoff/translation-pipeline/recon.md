...
`check_translation_health_task`（`translation.py:133-137`）も同一構造のため同じ修正が必要。
...
| ファイル | line | 内容 |
|----------|------|------|
| `backend/app/services/message_translator.py` | 95 | `key = os.getenv("GEMINI_API_KEY", "").strip()` → 空なら `LLMConfigError` |
| `backend/app/services/inventory_parser_llm.py` | 207 | `key = os.getenv("GEMINI_API_KEY", "").strip()` → 空なら `LLMConfigError` |
...
**手動記録時の別経路**（場面3）: `conv_logs.py:314` の `_fire_translation()` は Celery を経由せず HTTP リクエスト内で即時実行（FastAPI 非同期コンテキスト、EventLoop 問題なし）。
...
`send_lead_message` → プラットフォーム別送信（`_send_discord_message` `leads.py:1474` / Meta 送信 `leads.py:1249,1264`）→ **翻訳フックなし**。送信前に自動で `generate_outbound_draft()` を呼ぶコードは存在しない。
...
- 保存後に `_fire_translation(db, tenant_id, log_id, body.content_text)` を即時発火（`conv_logs.py:314`）
- `_fire_translation()` は `translate_inbound()` を呼び、結果を `conversation_logs.translated_text` に書き戻す（`conv_logs.py:127-172`）
...
注: direction チェックなし。outbound 記録も `translate_inbound()` を呼ぶ（`conv_logs.py:137-151` に direction 分岐なし）。意図通りかは PO に確認推奨。
...
**判定**: **顧客個別の言語フィールドは存在しない**。場面2（送信前翻訳）の「翻訳先言語をどう決めるか」は設計判断が必要（PO 確認事項）。現状は `generate_outbound_draft()` の引数 `target_language` を呼び出し側で指定する形（`message_translator.py:477`）。
...
| Webhook 送信先 env | `ADMIN_NOTIFICATION_DISCORD_WEBHOOK`（`translation_monitor.py:26`）|

監視項目（`translation_monitor.py:29-32`）:
- 翻訳失敗率 > 20%（`TRANSLATION_FAIL_RATE_THRESHOLD`）
- 低確信度比率 > 30%（`TRANSLATION_LOW_CONF_RATE_THRESHOLD`）
- 1 時間デバウンス（同一テナントへの連投防止）

**ブロッカー（現在機能しない理由）**:
1. `check_translation_health_task` が同じ EventLoop クラッシュで落ちている（`translation.py:137`: `asyncio.run(_run_health_check())`）
...
必要な作業:
1. `translation.py:137` に `engine.dispose()` を追加（A-1 と同時修正）
...
| 優先度 | 項目 | 状態 | 接続 |
|--------|------|------|------|
| **CRITICAL / Phase 1** | EventLoop クラッシュ（translate + health check の2タスク） | `translation.py:38,137` に `engine.dispose()` 追加 | Phase 1 設計へ |
...
| **在る・動く** | 場面3（手記録の受信翻訳）— conv_logs API | `conv_logs.py:314` で既に発火 | 動作確認のみ |
| **新規実装** | 場面2（送信前の自動翻訳フック） | 送信フロー（`leads.py:1084`）に hook なし | Phase 2 設計へ |
| **PO 確認事項** | 場面2の翻訳先言語をどう決めるか（顧客言語フィールドなし） | B-3 参照 | Phase 2 設計前に PO |
| **PO 確認事項** | B-2 outbound 記録も translate_inbound を呼ぶ（意図確認） | `conv_logs.py:137-151` | PO に確認 |
