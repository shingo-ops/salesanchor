...
今回は既存 i18n パターンの踏襲のみであり、外部サービス・外部ライブラリ・新規UIパターン導入はない。
既存実装である `customerType_trust` / `responseSpeed_24h` 等の option label i18n パターンを過去事例として採用し、今回も同じく「DB保存値は変更せず、表示ラベルだけ t() 化する」方針を適用する。
