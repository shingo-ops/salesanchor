...
|---|------|------------------------|
| 1 | カレンダーが「折りたたまれている」 | `.schedule-grid__viewport` の CSS 定義が存在しない → `overflow-y: auto` なし・高さ未拘束 |
| 2 | 左メニューとページ本体の間に余白がない | `.schedule-page { padding: 0 }` → ヘッダー・本体が左端に密着 |
| 3 | ヘッダーが他ページと不揃い | `.schedule-shell__header { padding-right: 0 }` → アバタークリアランスなし |
...
**`.schedule-page` (schedule.css:42-49 @ 7c5a5ea2)**
```css
...
**`.schedule-shell__header` (schedule.css:53-54 @ 7c5a5ea2)**
```css
padding-right: var(--page-header-avatar-clearance); /* #7: 固定アバターとのクリアランス 68px */
```
→ PR #2453 で削除。現在: なし。

**`.schedule-shell__content` (schedule.css:180-188 @ 7c5a5ea2)**
```css
...
**`.schedule-main` (schedule.css:351-353 @ 7c5a5ea2)**
```css
min-height: 0; /* allow flex children to shrink within the constrained grid cell */
```
→ PR #2453 で削除。

**`.schedule-main__surface` (schedule.css:359-364 @ 7c5a5ea2)**
```css
flex: 1; /* fill .schedule-main flex column */
```
→ PR #2453 で削除。

**`.schedule-grid__viewport` (schedule.css:431-434 @ 7c5a5ea2) — ブロックごと消えた**
```css
...
### `.schedule-page` (origin/main:schedule.css:22-27)
```css
.schedule-page {
  display: flex;
...
### `.schedule-shell__header` (origin/main:schedule.css:29-35)
```css
...
### `.schedule-shell__content` (origin/main:schedule.css:148-154)
```css
