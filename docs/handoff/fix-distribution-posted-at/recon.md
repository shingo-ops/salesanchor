...
- `tcg_distribution_svc.py:215` — 配信出力の SELECT（今回の変更対象）
- `tcg_analysis_dashboard_svc.py:378` — ダッシュボード用 MAX(sm.received_at)（変更対象外）
- `tcg_line_import_svc.py:306,315,425-447` — インポート時の received_at 書き込み（変更対象外・触らない）
- `tcg_import_progress.py:160` — インポート進捗確認クエリ（変更対象外）
- `tcg_supplier_quality_svc.py:84` — 仕入元品質クエリの ORDER BY（変更対象外）
...
`line_posted_at` は `pmg-import-delivery-ssot` で追加済みカラムであり、`tcg_line_import_svc.py:436` の INSERT文で既に書き込まれている。
