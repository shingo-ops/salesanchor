...
- 既存呼び出し元 `discord_oauth.py`・`meta_inbox.py` は `extra` 未指定のままで影響なし（後方互換）。
...
- KPI: `test_oauth_state_extra.py` 6件 + `test_oauth_state.py` 14件 + `test_discord_oauth.py` 9件 + `test_meta_oauth_endpoints.py` 21件 = 計 50 件 PASS
...
| 2 | `test_oauth_state_extra.py` 6 ケース新規作成 | Generator |
...
- 予約キーを追加・変更する際は `test_oauth_state_extra.py` の衝突テストを更新すること
