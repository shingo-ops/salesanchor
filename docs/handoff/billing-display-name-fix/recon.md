...
| `backend/app/tasks/reports.py:46` | `COALESCE(billing_display_name, ba.name, c.name)` — レポートは billing_display_name を優先使用 |
...
- `backend/app/tasks/reports.py:46` は `COALESCE(billing_display_name, ba.name, c.name)` — レポートは優先使用
...
- `frontend/src/pages/register/RegisterChangeBillingPage.tsx:44` で `address.name = ""` 初期値
- フォームに name 入力欄なし
...
- `frontend/src/pages/company-detail/CompanyDetailPage.tsx:130` は `isAlreadyRegistered` で change_billing ボタンを条件表示
