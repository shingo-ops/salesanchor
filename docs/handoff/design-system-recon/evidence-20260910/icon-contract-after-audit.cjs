...
const sha = 'current-worktree';
const out = __dirname;
const git = (...args) => cp.execFileSync('git', args, { cwd: root, encoding: 'utf8' });
const files = git('ls-files', '--cached', '--others', '--exclude-standard', 'frontend/src').trim().split('\n').filter(p => /\.(tsx?|json)$/.test(p));
const cache = new Map(files.map(p => [path.join(root, p), fs.readFileSync(path.join(root, p), 'utf8')]));
...
const summary = { sha, typescriptVersion: ts.version, method: 'JSX call signature props.weight declaration originates from constants/icons.tsx; tracked and new source loaded from current worktree; dependencies read from installed node_modules', exclusion: '(stories|test|spec)\\.|/design-preview/', total: rows.length, production: production.length, productionFiles: new Set(production.map(r => r.file)).size, counts: Object.fromEntries(keys.map(k => [k, production.filter(r => k in r.attrs).length])) };
