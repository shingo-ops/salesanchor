...
const sha = '76c6dff98e3fa68f47c381d044e86fd0564d9509';
const out = __dirname;
...
for (const sf of program.getSourceFiles()) {
  ...
  function visit(n) {
    if (ts.isJsxOpeningElement(n) || ts.isJsxSelfClosingElement(n)) {
      ...
      const match = sigs.some(s => s.parameters.some(p => checker.typeToString(checker.getTypeOfSymbolAtLocation(p, n)).includes('IconProps')));
      if (match) {
        const attrs = {};
...
const summary = { sha, typescriptVersion: ts.version, method: 'JSX tag call signature parameter type text contains IconProps; tracked source loaded from fixed SHA; dependencies read from installed node_modules', exclusion: '(stories|test|spec)\\.|/design-preview/', total: rows.length, production: production.length, productionFiles: new Set(production.map(r => r.file)).size, counts: Object.fromEntries(keys.map(k => [k, production.filter(r => k in r.attrs).length])) };
