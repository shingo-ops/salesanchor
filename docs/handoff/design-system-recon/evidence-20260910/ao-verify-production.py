...
out=Path('/tmp/ao-release'); out.mkdir(parents=True, exist_ok=True)
def get(url):
 import subprocess
...
status,html=get('https://app.salesanchor.jp/');asset=re.search(r'src="(/assets/[^\"]+\.js)"',html.decode())[1];assetstatus,b=get('https://app.salesanchor.jp'+asset);s=b.decode();anchors=list(re.finditer(re.escape('teams.newTeam'),s));assert anchors
...
full=[s[m.start():m.start()+430].split(']})')[0] for m in re.finditer('form-actions',s) if '`/teams`' in s[m.start():m.start()+430]]
assert len(full)==1
...
for i,e in enumerate(excerpts):
 ...
 assert e.count('disabled:')==0,e
assert 'common.create' in excerpts[0]
assert '`/teams`' in excerpts[2]
...
(out/'ao-production-verification.json').write_text(json.dumps(j,ensure_ascii=False,indent=2)+'\n');print(json.dumps(j,ensure_ascii=False,indent=2))
