...
`git grep -i "login\|password.reset\|forgot.password" docs/adr/` および `docs/adr/FEATURE-INDEX.md` を参照。
...
- `frontend/src/components/ProtectedRoute.tsx:17-19` — 未ログイン時: `<Navigate to="/login" replace />` — `state={{ from: location }}` 未設定
