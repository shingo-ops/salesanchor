...
デプロイ失敗 CI run 35645576032 にて `20260920_010000_phase3_fk_rewire_unit_condition.sql` が `tenant_004.units` を参照しようとしたが、SSOT移行（PR #3585 相当）によって `tenant_004.units` は `public.units` に移動済みであるためテーブルが存在しない状態でエラー。
...
- `docs/adr/ADR-155-*.md` — 対象ADR（SSOT移行設計）
