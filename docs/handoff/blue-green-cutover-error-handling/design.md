...
なし（Docker 標準の `docker inspect` / `docker network disconnect` / `docker rename` を使用）。
...
`set +e` + 個別エラーチェックが Steps 4-6 の各操作を保護。最後の `set -e` 復帰でそれ以降の安全を確保。
