...
## 既存ADR検索結果
- ADR-158 本体（Phase 1/2/3/4/5 計画）: docs/adr/ 配下に存在確認済み
- 関連: ADR-025 (本番DB直接INSERT禁止), ADR-072 (reset_tenant_context)
...
- .github/workflows/deploy.yml — マイグレーションはrun_all_migrations.sh経由（deploy.yml編集不要）
