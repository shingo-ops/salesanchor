...
## 外部事例
- Phase 1 (PR #3783): GEMINI_EXCLUDED 2件確認済み。同パターンの安全ガードをGate 1にも適用。
...
デプロイ後 24h で:
```sql
SELECT prompt_version, count(*) FROM public.extraction_jobs
WHERE created_at > now() - interval '24h' GROUP BY prompt_version;

SELECT pid_basis, count(*) FROM public.analysis_results
WHERE updated_at > now() - interval '24h'
AND pid_basis LIKE 'RAWCODE%' GROUP BY pid_basis;
```
