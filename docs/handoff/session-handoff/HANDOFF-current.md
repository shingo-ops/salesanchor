...
## 最新セッション（2026-07-27・設計パートナー）

### 設計パートナーの起動と守り（①②の定着・origin/main反映済み）
- ①「まず本書を読む」: design-partner-preamble.md 新設（PR #3099）・§8守り手3から参照。
  PO が claude.ai 常設指示欄に貼付済み（実物照合で preamble と一字一致を確認）。
- ②「同期の鮮度」: claude.ai の GitHub 同期は手動 Sync now。PO 実行 →
  本書最新版（§2=12項目・標準設定入り）がナレッジに届くことを実測確認。
  運用: 本書やルール更新時は Sync now を押す（機械強制不可）。
- 標準設定4項目＋停止の扱いを §0 に追記（PR #3092）。毎セッション冒頭で宣言。

### 関所の宣言欄・借用の根治（ADR-150・PR #3115）
- 設計パートナー正本便が lessons-guard を借りていた妥協（#3092・#3099）を根治。
- ADR-150「担当領域を窓の明示リストで管理」新設（Accepted）＋
  agent-complete-design/design.md に受け入れ基準集約表・外部事例欄・ADR-150参照を追加。
- 以後 design.md 自身が関所を通る。借用不要。索引 README は generate-adr-index.js の自動生成物（手書き禁止）。

### 教訓ポスト（本セッション 25 件・lessons.d）
判定マーカー／三点diff／コメント発火の再採点待ち／ls-remoteは0件でも成功終了／
カード禁止語と実行の字面衝突／PR番号は変数受け（発話しない）／ADR索引は自動生成。

### 証拠登録簿
EV-20260724-001 追記（PR #3079）。

### 未完・次セッションへ
- 本店ローカル（~/salesanchor）で他セッション稼働中（数秒でSHA変化・未コミット多数）。手動で触らない。
- 台帳 active-work.d の IN_PROGRESS 残骸は別セッションの掃除便が担当（#3100 で確認）。
- PR番号は設計パートナーが発話しない（本セッションで3回取り違え・変数受けで実害ゼロ）。
- 作業台は上限100・reaper が次の作業台作成時に自動回収。

### 冒頭で観測すること
- 本書が最新版（§2=12項目・標準設定入り）で届くか。届かなければ Sync now を PO に依頼。
