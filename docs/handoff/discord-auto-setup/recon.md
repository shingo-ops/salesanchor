...
| `docs/adr/ADR-072-tenant-schema-prefix-enforcement.md` (CLAUDE.md参照) | MEDIUM | write endpoint の **reset_tenant_context()** 必須ルール（新規APIエンドポイントに適用） |
...
| ロール作成 | POST /guilds/{guild_id}/roles | `backend/app/services/discord_role_sync.py:97` に実装済み |
