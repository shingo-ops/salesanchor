...
`line_import_admin.py` が依然 `from app.tcg_config import TCG_SCHEMA` を import しており、
`tcg_config.py` のデフォルト値が旧 `tenant_004` のままになっている。
...
これらは引き続き `tcg_config.py` を import するが、デフォルト値が `public` になることで安全になる。

### SQL 中の tenant_004 ハードコード調査結果

- `backend/app/routers/tcg_product_master.py:306-313` — ドキュメントコメント内のサンプルSQL（実行されない）
- `backend/app/services/tcg_distribution_svc.py:859` — `tenant_id=4  # tenant_004`（Discord通知のテナントID整数値・スキーマ文字列ではない）
