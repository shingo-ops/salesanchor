...
recon: `docs/handoff/line-import-missing-channel/recon.md`（事象の特定経緯・原因・影響範囲）
...
- 同一リポジトリの先行事例: 未登録送信者に対する自動登録（`tcg_line_import_svc.py` の
  「4b. 未解決仕入元の自動登録」, PR #3530 系）。仕入元とチャネルをその場で作って取り込みを
  続ける方式が既に本番で稼働している。今回はその扱いを既知の仕入元にも揃えるだけで、
  新しい方式を持ち込まない。
- 2026-09-21 の本便直前の障害（`docs/handoff/line-import-schema-rewire/recon.md`）:
  スキーマ移行の検証が「文字列の有無」を測っていたため取りこぼしを検出できなかった。
  今回も「データ移行が完了している前提」に依存せず、アプリ側が欠落を埋める設計にしている。
- PostgreSQL 一般の慣行: 参照先が無い場合に例外で止める設計は、データ移行の過渡期には
  可用性を落とす。身元が確定している行については冪等な作成（存在しなければ作る）に倒すのが
  運用上の定石で、今回の `supplier_channels` はこれに該当する。
...
- **契約の変更**: 従来は「仕入元は居るのにチャネルが無い」場合に `ValueError` で
  ロールバックしていた（`backend/tests/test_tcg_import_progress_pg.py` で保証）。本便で
  「その場で作って続行」に変更し、当該テストも新しい契約に合わせて更新した。
  ロールバックの保証は「仕入元自体が `public.suppliers` に無い場合」で維持している。
