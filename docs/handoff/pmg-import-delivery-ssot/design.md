...
## 2026-09-11 認知負荷を減らす進捗表示（画面改善便）

PO原文「心理学、脳科学、認知科学的に人間がパット見て何が書いてあるかを理解しやすい表示にしてくれ」。続行原文「進める、離席するので最後まで完走させて結果を報告してくれ」。直前の色分け/文字の緩急/概要優先の提案への具体的改善依頼として設計・実装を進める。理解速度の実測結果や番号付きGOをPOの発言として創作しない。親の履歴・本番切替REVISEは別件として維持。

### Whyと根拠（2026-09-11閲覧）

- W3C COGA「Use a Clear and Understandable Page Structure」（初出2021-04-29）: 意味のまとまり、余白、見出し、重要度の視覚差で探索を支える。https://www.w3.org/WAI/WCAG2/supplemental/patterns/o2p03-page-structure/
- Jakob Nielsen「Progressive Disclosure」（2006-12-03）: 頻用/重要情報を先に、詳細を利用者の操作で開く。https://www.nngroup.com/articles/progressive-disclosure/
- W3C WCAG 2.2 SC1.4.1: 色だけを意味の唯一の伝達方法にしない。https://www.w3.org/WAI/WCAG22/Understanding/use-of-color.html
- これらは設計指針であり、この画面の理解時間の実証ではない。脳部位の活性、普遍的な数秒理解、改善率など未測定の数値は使わない。外部導入事例の5W2H/実測値は本件では不要。自画面の契約保持試験と視覚検証を直接の合否根拠にする。
- 実物: ImportWorkflowPanel.tsx:35-55は主要値と8項目の抽出内訳を同じ段で表示。import-workflow.css:1-18は3列・ほぼ同じ文字階層。Badge.tsx:16-58には状態別5種、native detailsは既存で使用。ProgressBar.tsx:22-76は100%で成功表示と英語Doneを組み込むため、失敗を含む終了数には流用しない。

### What / Scope

対象は既存の総合取込ページの情報順序、進捗Panel、関連CSS、翻訳、回帰/視覚試験。API・集計値・抽出/配信操作・DB・共通部品/トークン・認証は変更しない。表示上の完了は抽出工程に限定し、解析の実行完了や配信可能と推測しない。参照: docs/specs/design-system/README.md、docs/specs/component-standard.md、ADR-027、ADR-067、ADR-113。

### How（実装契約）

1. 選択済み取込の概要を主役にする。取込未選択時は従来uploadを展開、選択済み時はuploadを「新しいファイルを取り込む」のnative detailsにまとめる。詳細の開閉で入力値/アップロード処理を消さない。進捗selectorとPanelを見つけやすくし、既存保留確認・履歴・全体配信は保持する。
2. Panel上部に結論1つ＋短い次の行動。状態の優先順は取得エラー/古い値→情報待ち/関連未記録→抽出状態不明→抽出中→抽出エラーあり→抽出終了（全て抽出限定）。エラー件数と要確認件数が正なら主結論に関係なく常時見える。データ未取得を0や完了にしない。
3. 3段階は取込/抽出/解析結果。各段の代表値を32px相当の既存font-3xl、太字、単位を添える。取込は投稿数、抽出は終了数/総数と成功・対象なし・エラーの短い内訳、解析は結果件数と要確認件数を区別。1019件の結果を「解析成功1019」と呼ばない。51投稿と1019明細を同じ割合で比較しない。
4. 色は既存Badgeのsuccess/info/warning/danger/neutralと既存の意味別CSS変数。完了は緑、処理中は青、要確認は黄系、エラーは赤、未記録は中立。必ず文字ラベルを併記。原色全面塗り、点滅、装飾だけのアイコンは使わない。状態単位のBadgeを利用し、共通部品を別実装しない。
5. 抽出の進行を棒で示す場合はnative progressを青の中立な進行表示として使う。総数>0かつ全必要値既知、unknown=0の場合だけ終了数/総数を使う。100%は成功と同義ではなく「抽出終了」。総数0/NULLで割合や成功チェックを作らない。既存ProgressBarの成功表現を上書きしない。
6. 要確認>0時に「要確認の明細を見る」を1操作で提供し既存needs_reviewフィルタ、offset=0へ接続。抽出エラー>0でも既存extraction_error絞込への導線を表示する。既存フィルタ/ページ25件/行detailsは保持。通知や5秒pollで利用者のフォーカスや展開状態を奪わない。
7. 残存明細/残存結果、投稿再利用/新規内訳、解析結果なし、実行履歴未記録、関連記録/更新日時などの補足を「処理の詳細」にまとめる。ただしstale、NULL、不明、エラー、要確認など誤判断防止情報は概要から隠さない。更新時刻JSTと再取得は常時確認可能。既存の全情報は開けば閲覧可能。
8. 見出し/数値/補足の3段階、余白で群分け。PCは3列、390pxは1列。明細表だけ横スクロール可。ページ全体の横はみ出し0。既存フォント/色/余白トークンを使用。i18n ja/en、明暗、キーボードdetails/絞込を維持。既存PageLayout規則を守る。

### 受入条件と確認方法

- C1: 画像の51終了（37成功/14対象なし/0失敗）、結果1019、要確認313 fixtureで、概要に「抽出終了」「要確認313」と確認導線が見える。全体成功/配信可能の表示0。
- C2: pending/runningありは抽出中。unknown/NULL/未取得/関連欠落、初回失敗、前回値付き失敗をそれぞれ成功表示0で単体検証。総数0は空状態で割合なし。
- C3: failed>0が完了数に含まれても緑の全成功0、エラー数が折りたたみ外に見える。要確認>0との同時表示も検証。
- C4: 要確認ボタン1回でneeds_review、offset=0の既存GETを使う。新規POST/自動再送0、非管理者の要求0、旧ID応答破棄の既存試験を維持。
- C5: detailsをキーボードで開閉でき、従来の残存件数や履歴未記録へ到達できる。uploadの開閉で入力保持、初回upload展開、選択済みprogress優先をE2E確認。
- C6: PC日本語ライトと390px英語ダークを実スクリーンショットで検証。概要は縦に長い8項目列挙を使わず、エラー/要確認は開く操作なしで分かる。色なしでも文字で判別。フォーカス見失い/ページ横はみ出し0。
- C7: build、check:all、unit、既存E2Eと新状態試験成功。UIの意味判定は自動試験、見やすさはroot視覚レビュー、最終的な本人の理解のしやすさはPOが帰席後確認。5秒で3問（終了した工程/要確認数/次操作）が答えられるかはPO確認用の目標案で、実施済み成果ではない。

### Architect自己審査

APPROVE（この表示改善のみ、同一AIによる自己審査）。実APIが持つ既知値だけで状態を導出し、処理完了と内容確認を分離できる。旧ProgressBarを使うと失敗を成功色へ変える矛盾を発見し、共通部品変更を避けnativeの進行表示へ限定した。詳細化で重要警告を隠すリスクはC2/C3/C5で検査。未解決の事業判断なし。既存履歴の設計REVISEは解除しない。
維持担当: rootが設計/証跡/レビュー、PO指定Terraが本カード範囲だけ実装、既存CIと状態fixtureが回帰を検査。ページ専用CSSに限定し他画面の色/部品を変更しない。正式カード検査後に実装へ渡す。
