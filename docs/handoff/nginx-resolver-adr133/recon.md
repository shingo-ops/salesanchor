...
|---|-----------|------------|------------|-----------------|---------------------|----------------------|---------|
| 1 | `nginx.conf:95` | app.salesanchor.jp | prefix | `/api/v1/auth/` | `http://backend:8000/api/v1/auth/` | ✓ 一致 | **変換そのまま可** |
| 2 | `nginx.conf:108` | app.salesanchor.jp | exact (`=`) | `/api/v1/conversations/stream` | `http://backend:8000/api/v1/conversations/stream` | ✓ 一致 | **変換そのまま可** [SSE] |
| 3 | `nginx.conf:128` | app.salesanchor.jp | exact (`=`) | `/api/v1/leads/stream` | `http://backend:8000/api/v1/leads/stream` | ✓ 一致 | **変換そのまま可** [SSE] |
| 4 | `nginx.conf:150` | app.salesanchor.jp | prefix | `/api/` | `http://backend:8000/api/` | ✓ 一致 | **変換そのまま可** |
| 5 | `nginx.conf:192` | app.salesanchor.jp | exact (`=`) | `/metrics` | `http://backend:8000/metrics` | ✓ 一致 | **変換そのまま可** |
| 6 | `nginx.conf:255` | api.salesanchor.jp | prefix | `/api/v1/auth/` | `http://backend:8000/api/v1/auth/` | ✓ 一致 | **変換そのまま可** |
| 7 | `nginx.conf:268` | api.salesanchor.jp | exact (`=`) | `/api/v1/conversations/stream` | `http://backend:8000/api/v1/conversations/stream` | ✓ 一致 | **変換そのまま可** [SSE] |
| 8 | `nginx.conf:288` | api.salesanchor.jp | exact (`=`) | `/api/v1/leads/stream` | `http://backend:8000/api/v1/leads/stream` | ✓ 一致 | **変換そのまま可** [SSE] |
| 9 | `nginx.conf:310` | api.salesanchor.jp | prefix | `/api/` | `http://backend:8000/api/` | ✓ 一致 | **変換そのまま可** |
...
**9 箇所すべてで location prefix == proxy_pass URI suffix** のため、URI の末尾を除いた変数形式 `http://$backend:8000` にしても転送先パスは変わらない。`rewrite` / `break` の追加は不要。

### 1-B. backend 以外への proxy_pass（変数化スコープ外）

| file:line | server_name | location | proxy_pass 現行値 | 対応 |
|-----------|------------|---------|-----------------|-----|
| `nginx.conf:162` | app.salesanchor.jp | `/grafana/` | `http://49.212.160.98:3000/grafana/` | 外部 IP 直指定・DNS 不要 → **対象外** |
| `nginx.conf:179` | app.salesanchor.jp | `/status/` | `http://49.212.160.98:3001/` | 外部 IP 直指定 → **対象外**（注: location `/status/` を `/` URI に strip する挙動あり） |
| `nginx.conf:201` | app.salesanchor.jp | `/` | `http://frontend:8080/` | frontend コンテナ（Docker DNS 問題は原理上同一）→ **本件スコープ外・別途検討** |
| `nginx.conf:368` | monitor.salesanchor.jp | `/` | `http://49.212.160.98:3001/` | 外部 IP 直指定 → **対象外** |
...
SSE 専用設定を持つ location: `nginx.conf:107-124`、`nginx.conf:127-144`、`nginx.conf:266-284`、`nginx.conf:286-304`

各 SSE location が持つ設定（4 箇所すべて同一）:

| 設定 | 値 | 目的 |
|-----|---|-----|
| `proxy_buffering off` | `nginx.conf:116,136,275,296` | バッファリング無効（SSE イベント即時転送） |
| `proxy_cache off` | `nginx.conf:117,137,276,297` | キャッシュ無効 |
| `proxy_read_timeout 3600s` | `nginx.conf:118,138,277,298` | 長時間接続維持（1時間） |
| `proxy_send_timeout 3600s` | `nginx.conf:119,139,278,299` | 同上 |
| `proxy_connect_timeout 10s` | `nginx.conf:120,140,279,300` | 接続タイムアウト（通常と同値） |
| `proxy_http_version 1.1` | `nginx.conf:121,141,280,301` | HTTP/1.1 keep-alive（SSE 必須） |
| `chunked_transfer_encoding on` | `nginx.conf:122,142,281,302` | チャンク転送エンコーディング |
...
`Connection ''` ヘッダーの上書きは現行設定に存在しない（`nginx.conf:107-124` 全行確認済み）。`proxy_http_version 1.1` が keep-alive を確保しているため SSE 動作に問題はない。
...
根拠:
1. **宣言 1 回で全 location に適用**: `app.salesanchor.jp` サーバーブロック（`nginx.conf:67`）と `api.salesanchor.jp` サーバーブロック（`nginx.conf:228`）それぞれで 1 回宣言すれば、配下の全 location が `$backend` を使用できる
...
変更が必要なサーバーブロック:
- `nginx.conf:67-207`（app.salesanchor.jp HTTPS）
- `nginx.conf:228-325`（api.salesanchor.jp HTTPS）

変更不要:
- `nginx.conf:11-24`（jarvis-claude.uk HTTP → return 301 のみ）
- `nginx.conf:29-47`（jarvis-claude.uk HTTPS → return 301 のみ）
- `nginx.conf:53-65`（app.salesanchor.jp HTTP → return 301 のみ）
- `nginx.conf:333-378`（monitor.salesanchor.jp → 外部 IP 直指定）
- `nginx.conf:386-446`（salesanchor.jp → 静的ファイル、proxy_pass なし）
...
nginx コンテナのネットワーク: `frontnet` のみ（`docker-compose.yml:34-35`）  
backend コンテナのネットワーク: `frontnet` + `backnet`（`docker-compose.yml:130-132`）
...
- zero-downtime-deploy/recon.md セクション 8「nginx upstream がコンテナ名固定」の改善策と整合（`docs/handoff/zero-downtime-deploy/recon.md:245`）
- backend 起動完了まで約 22 秒（`docs/handoff/zero-downtime-deploy/recon.md:209-211`）のうち、DNS 解決は TTL（5s）後に自動再試行 → restart 後の自動復帰: **≤ 5s + nginx 再接続オーバーヘッド ≒ ≤15s**（KGI #2 の目安と整合）

---

## 5. 隣接案件（zero-downtime-deploy）との重複確認

`docs/handoff/zero-downtime-deploy/recon.md:245` に以下の記述あり:
