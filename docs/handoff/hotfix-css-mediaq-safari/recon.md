...
| `frontend/dist/assets/index-Bf-KW5H6.css:107046` | `.sidebar-panel` base rule（position:fixed, no transform） |
| `frontend/dist/assets/index-Bf-KW5H6.css:155252` | `@media (width<=767px)` block（Level 4 syntax、Safari < 16.4 で無視） |
...
本番 CSS 全体で 27 箇所の `(width<=...)` / `(width>=...)` が非対応 Safari で無視される。
