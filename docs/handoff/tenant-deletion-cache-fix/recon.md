...
get_current_tenant はキャッシュヒット時に DB を確認しないため（`backend/app/auth/dependencies.py:207`）、
論理削除直後から 403 が返ることを保証するにはキャッシュ削除が必要。
...
| # | 不明点 | 解消方法 | 状態 |
|---|-------|---------|------|
| 1 | Redis 未接続時の挙動 | `backend/app/cache.py:94` の if not r: return で no-op が保証される | ✅ 解消済み |
| 2 | 物理削除後もキャッシュ削除が必要か | DROP SCHEMA 後に public.tenants 行も削除されるため、キャッシュが残ると stale key になる。必要 | ✅ 解消済み |
