...
| # | 確認事項 | マーク | file:line | 詳細 |
|---|---------|--------|-----------|------|
| B1 | 部品7点の実在 | ✅ | `frontend/src/components/Button.tsx` 他6部品 | 全7部品存在確認 |
...
| B6 | main に Card.tsx 存在 | ✅ | `frontend/src/components/Card.tsx` | main にも同一内容で存在 |
...
| C2 | Badge の展開 | ⚠️ | **33 / 97 / 130 件**（展開率 25%） | CSS .badge-/.status-badge クラス: 130件 / React Badge コンポーネント: 33件 |
| C3 | DataTable の適用 | ⚠️ | 主要3ページ✅ / raw table タグ残 34件 | LeadsPage/CompaniesPage/OrdersPage は DataTable 採用済み |
...
| D4 | Modal の非公式 bp 640px | ⚠️ | `frontend/src/components/Modal.css:96` | @media (max-width: 640px) でボトムシート。公式 767px とズレ（PR-A で修正対象） |
...
| # | 確認事項 | マーク | 件数 / 値 | 詳細 |
|---|---------|--------|----------|------|
| E1 | 非公式 @media breakpoint（確定） | ⚠️ | **6値・8か所** | 480px(1): `frontend/src/company-forms.css:206` / 560px(1): `frontend/src/pages/inbox/InboxPage.css:1453` / 640px(2): `frontend/src/components/Drawer.css:104`, `frontend/src/components/Modal.css:96` / 720px(1): `frontend/src/pages/goal-setting/GoalSettingPage.css:32` / 960px(1): `frontend/src/pages/dashboard/DashboardPage.css:66` / 1023px(1): `frontend/src/pages/super-admin/ParseReviewPage.css:92` |
...
| 1-4 | サイドバー幅の決定 | ✅ | `frontend/src/sidebar.css:19` | DesktopShell で --sidebar-width-collapsed: 54px |
...
| # | 確認項目 | マーク | file:line | 事実 |
|---|---------|--------|-----------|------|
| 2-1 | 640px 使用箇所全列挙（@media） | ✅ | `frontend/src/components/Drawer.css:104` / `frontend/src/components/Modal.css:96` | @media として 2か所のみ |
...
| 3-4 | Card.tsx 「Preview専用」コメント | ✅ | `frontend/src/components/Card.tsx:1-11` | コメントのみ。技術的依存なし |
...
| 4-5 | /design-preview 本番到達可否 | ✅ | `frontend/src/App.tsx:289` | DEV限定でない。本番で到達可能 |
...
| E5 | サイドバー非表示時の検索・ユーザーメニュー到達手段 | avatar-btn は position:fixed（`frontend/src/sidebar.css:109`）で常時表示。グローバル検索バーはLayout層に存在しない。影響なし |
