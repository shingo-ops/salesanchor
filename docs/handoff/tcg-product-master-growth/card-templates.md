...
## 8. 設計パートナーが書いてよい値・書いてはいけない値

2026-09-05 に、1日で9件の推測混入が起きた。すべて手元の情報か1回の確認で判明するものだった。本節はその再発防止である。

### 8-1. 書いてよい値

本便または過去便の生出力に、そのまま現れている値。
公式サイトなどの取得結果に、そのまま現れている値。

### 8-2. 書いてはいけない値

PR番号・commit hash・件数・日付・ファイルの行数を、確認せずに書くこと。
「たぶん」「はず」で埋めた値を、断定形で書くこと。
「確実に通ります」「必ず成功します」など、検証していない結果の保証。

### 8-3. 停止条件の期待値も同じである

停止条件に書く数値は、設計パートナーが暗算した値であってはならない。

2026-09-05 の実例。git diff origin/main --name-only の出力を「1行のみ」と書いたが、ブランチの派生元に別の変更が含まれており2行だった。git log を見れば分かったことである。

停止条件に数値を書く前に、その数値の根拠が生出力にあるかを確認する。無ければ、第1節4項のとおり「あってはならないものが0件」の形にする。

### 8-4. 迷ったときの書き方

「未確認」と書く。空欄のままカードを出してよい。
実装役に読み取りで確認させる手順を1つ足す。

### 8-5. 2026-09-05 に起きた推測混入の一覧

| # | 推測した内容 | 実際 | 確認手段があったか |
|---|---|---|---|
| 1 | migration PR は #3284 | #3283 | 直前の報告に書いてあった |
| 2 | MEGA スタートデッキ100 のマークは MC | 既存は MBB | 実測すれば分かった |
| 3 | ポケモンの mark は空の可能性が高い | 89%埋まっていた | 実測すれば分かった |
| 4 | NONE 184種のうちポケモンは約30種 | 43種 | 数えれば分かった |
| 5 | permit は30分間なら何度でも使える | 1回で消費 | スクリプトを読めば分かった |
| 6 | UPDATE はコメント込みで3本 | 2本 | 自分のSQLを読めば分かった |
| 7 | スペシャルBOX 3種の発売日 | 公式は抽選で確定日なし | 公式を読めば分かった |
| 8 | 対象ADRと設計docを直せば確実に通る | 設計docの5要件で再度落ちた | 検査コードを読めば分かった |
| 9 | 差分は1ファイルのみ | 2ファイル | git log を見れば分かった |

## 9. カードを破棄するときの作法

方針変更で前便のカードが無効になったとき、次のカードの冒頭で「前便の CM-NN は破棄」と名指しする。

2026-09-05 に、破棄したはずのカードが実行され、混乱が生じた。設計パートナーが「もう出さない」と考えているだけでは、実装役には伝わらない。

同様に、過去便の禁止事項を解除するときも「過去便の CM-NN の禁止は無効とする」と名指しで書く。update-branch の許可と、PR番号の誤りによる禁止の撤回で、いずれもこの形を使った。
