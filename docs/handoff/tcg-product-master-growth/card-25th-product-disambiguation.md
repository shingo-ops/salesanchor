...
状態: 実装指示書準備済み・未発行。設計§17改訂4は同一AI自己審査APPROVE。最新本番マスタ照合と局所27対照完了。本25th便の実装担当への明示委任は未受領のため、現時点では実行しない。
...
発行前の人手照合: 空箱保護は本番反映済み。最新マスタ照合と自己審査は完了。明示委任前なので未発行。書式検査の成功を発行承認に代用しない。
...
開始条件
設計§17改訂4の最終自己審査と文書保存の証拠、および本便の実装担当への明示委任を確認する。本カードが未発行の場合は手順0より前に停止する。
...
新セットの属性・固定27例は設計§17改訂4に従う。本書の発行前に作業しない。空箱の除外語を商品辞書へ加えない。
