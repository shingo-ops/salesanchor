...
状態: 再審査草案・未発行。空箱はPR #3470で本番反映済み。設計§17改訂3の否定表現4語の提案と最新マスタ照合が未完了で、自己審査REVISE。実行しない。
この文書は、25th通常商品とスペシャルセットを辞書で分けるための実装指示書です。
親: `docs/specs/product-master/README.md`
設計: `docs/handoff/tcg-product-master-growth/design-keyword.md` §17改訂3（属性と登録安全契約は改訂2を継承）。
...
発行前の人手照合: 空箱保護は本番反映済み。否定4語の追加案の方針確認と最新マスタ照合が未完了。書式検査の成功を発行承認に代用しない。
...
開始条件
設計§17改訂3の残件解消・最終自己審査と文書保存の証拠、および本便の実装担当への明示委任を確認する。本カードの草案状態が解消されていない場合は手順0より前に停止する。
...
ただし本書は未発行案であり、方針確認前に否定4語を登録しない。空箱の除外語を商品辞書へ加えない。
...
匿名入力で空箱を除いた設計の15商品判定対照を再現し、サプライ6例と否定4語×3欄の12例は実解析から配信行取得まで通して出力0を確認する。
