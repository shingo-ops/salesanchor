...
## Gemini発送情報保持の実API比較（2026-09-10）

原文の発送情報が保存済み抽出では空となる欠落を読取で確認。応答原体は保存されておらず、当時の応答を断定しない。PROMPT_TEXTは補足の一般指示のみ、parse_extraction_responseは空の状態/備考を許可、extract_messageはitemsがあればdoneとする。コードgemini_extraction_svc.py:38,246-286,315-324、tcg_extraction.py:149-153,191-224。

POの抽出精度改善依頼を受け、同じ既存呼出し関数/モデル/設定で匿名短文6種と17商品長文を各案2回比較。計28 API呼出し。現行12/14試験合格、改善14/14。共通発送見出しの2商品×2回の脱落を改善。長文は両案成功で、過去の脱落原因をプロンプトだけと断定する根拠ではない。純パーサの試験と実API比較を区別。設計§15と匿名評価JSONへ全応答/採点/入力を保存。顧客原文・供給者名・取引金額・内部ID・APIキーは添付に含めない。製品/DB/シート変更0。
