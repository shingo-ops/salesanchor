...
| PR 番号を直接書く | 別セッターの PR 本文を gh pr edit で上書きする事故が起きた。番号は $(cat .pr-number) で読むか gh pr list --head ブランチ名 --json number で引く。報告に出た番号を実在の確認なしに使わない | 2026-09-08 実測・事故 |
| commit と push と PR 作成を1便に詰める | どれが失敗したかを確かめずに次へ進むと、失敗に気づかない。commit まで、push まで、と分けて、各段で git log や .pr-number の実在を確かめる | 2026-09-08 実測 |
| 行の移動をした PR で「削除するファイル: なし」と書く | 行の並べ替えは insertions と deletions を両方生む。PR 本文を書く前に git diff --cached --stat で deletions を確かめ、0 でなければ実ファイル名を列挙する | 2026-09-08 実測・既存の規則を踏んだ |
| 報告に出た PR 番号を読み違える | 番号は .pr-number と gh pr list --head の両方で取り、一致を確かめてから使う。設計パートナーが読み違えた事例が同日に3回ある | 2026-09-08 実測 |
