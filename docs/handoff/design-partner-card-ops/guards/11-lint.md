## 11. card-lint 検査式（scripts/card-lint.sh に写す）

カードの検査項目一覧。採番だけでは機械実装済みを意味しない。機械検査の実体は `scripts/card-lint.sh` と照合する。L24は警告のみで停止しない。L32は未実装のため人手で照合する。

| # | 検査 | 対応する節 |
|---|---|---|
| L01 | 手順のコマンド行が `cd ` で始まらない | §1 |
| L02 | プレースホルダ（`<` と `>` に挟まれた日本語） | §3 |
| L03 | `END OF CARD` が無い | card-ops §5 |
| L04 | 受領確認の文言が無い | §5.5-10 |
| L05 | 出力先と列挙対象が同じディレクトリ | §1 |
| L06 | `psql` 行に `<<`・`< file`・`-f `・`\| psql`・書き込みSQL | §2 |
| L07 | `psql` があるのに `default_transaction_read_only=on` が無い | §2 |
| L08 | `gh pr create` を直接呼ぶ | §5 |
| L09 | `gh-pr-merge-safe.sh` に番号を渡す | §6 |
| L10 | `--body-file` の値が `<` で始まる | §3 |
| L11 | `rm ` を含む | §10 |
| L12 | `git worktree add` を直接呼ぶ | §4 |
| L13 | `-b` のブランチ名が `release/` `hotfix/` 以外 | §4・§5 |
| L14 | `--draft` | §5.5-2 |
| L15 | 停止時の報告経路が本文に無い | card-ops §3 |
| L16 | `timeout ` を使う | §1 |
| L17 | `git show $SHA:` を引用符なしで使う | §1 |
| L18 | `--admin` `--auto` | §6 |
| L19 | 出力上限の無い一覧系（`gh run list`・`git branch -r` 等） | §1 |
| L20 | 外側フェンスがバッククォート3つで、内側にも同じものがある | card-ops §5 |
| L21 | **`gh run view` / `gh run watch` を含む** | §9 |
| L22 | `~/.claude/permits/` への直接アクセス | §2 |
| L23 | 危険語（`DELETE FROM` 等）を本文に含む | §2 |
| L24 | 1行の長さが200を超える（awk length、暫定・警告のみ・停止しない） | 00-common |
| L25 | コマンド行のダブルクォート内にバッククォートがある | 00-common |
| L26 | マージ後の手順に worktree パスの cd がある | 06-merge |
| L27 | gh pr edit または gh pr view に数字を直書きしている | 05-pr |
| L28 | 手順の外に実行を依頼する散文がある | 00-common |
| L29 | カード冒頭に「読んだ節」の書き出しが無い | 00-common |
| L30 | git commit と push か PR 作成があるのに git log が無い | 00-common |
| L31 | gh-pr-merge-safe.sh の出力を報告に残していない | 06-merge |
| L32 | **未実装・人手照合**：未確定の目印が残っていないか文脈を読んで確認する。JSX・TypeScriptの型引数・HTML等の正当な山かっこを一律違反にしない | 00-common |
| L33 | 同じ検査番号が2回現れる／番号が飛んでいる | 11-lint |

**検査式にできていない**（人手の照合 §5.5 に残す）: 散文指示の検出、行番号の決め打ち、期待値の見積り、worktree パスの実在。

**L02 の補足（2026-09-08 実測）**: L02 の式は「`<` と `>` に挟まれた**日本語**」だけを見るため、`<WT>` のような英字の目印は素通りする。CARD-PMG-KW-V4 でこれを踏み、実行役が手順4で停止した。L32 は未実装のため、英字の目印はカード発行前に人手で照合する。山かっこの一律検査は JSX・TypeScript の型引数・HTML 等を誤検知するため採用しない。

**採番整理**: L24・L25は scripts/card-lint.sh の既存実装に対応させ、L26の同内容重複を除去する。他番号は変更しない。次に足すときは L34 から。文書上の採番は機械実装済みを意味しない。
