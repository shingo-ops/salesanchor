...
| L32 | 山かっこで囲まれた語（日本語・英字を問わず。`<WT>` `<path>` `<run_id>` 等） | 00-common |
| L33 | 同じ検査番号が2回現れる／番号が飛んでいる | 11-lint |

**L02 の補足（2026-09-08 実測）**: L02 の式は「`<` と `>` に挟まれた**日本語**」だけを見るため、`<WT>` のような英字の目印は素通りする。CARD-PMG-KW-V4 でこれを踏み、実行役が手順4で停止した。L32 で補う。

**採番の不良（2026-09-08 実測）**: `L26` が2行重複し、`L24` と `L25` が欠番になっている。次に足すときは L34 から。

L04 の判定は文字列 受領確認 の有無で行う。受領した・確認した などの言い換えは検出されない（2026-09-09 実測。CARD-PMG-SHA-01 が差し戻された）。
L24 は1行200字超を数える警告で、実行は止めない（2026-09-09 実測。本表に未記載だった）。
L29 は見出しの文字列 読んだ節 を探す。guards 照合結果 などの言い換えでは通らない（2026-09-09 実測。CARD-PMG-GUARD-PR-01 が差し戻された）。
L04 と L29 は同じ根で、検査が文字列を見ているのに設計側が言い換えると落ちる。見出しは逐語で写す。
