...
- `scripts/run_all_migrations.sh` 旧529行目 — FK張替えより**前**に DROP を実行しようとする（早期実行・削除対象）
- `scripts/run_all_migrations.sh` 旧524行目 — fix_phase2c_fk_drop_only.sql でFKのみ削除
- `scripts/run_all_migrations.sh` 旧660行目 — unify_tcg_products_to_public.sql（FK張替え）
- `scripts/run_all_migrations.sh` 旧663行目 — drop_tcg_products_phase2c.sql（正規位置・残す）
...
- `scripts/run_all_migrations.sh` — 旧526〜529行目（4行）削除
