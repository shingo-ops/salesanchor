...
- 守り手: 人手で守る（理由: 未定義CSS変数の使用と、例外文言のログ出力を検査する関所が現時点で存在しない。CV-15 で既存の関所4本を実測し、いずれも定義側のみを見ていることを確認した。逆向きの照合を行う番人の新設は別テーマとする）
- 対象: 未定義トークンによるボタン非表示の再発、および Gemini 例外に含まれる APIキーがログ・DBへ出力されること（APIキーの管理方針は ADR-075 に従う）
