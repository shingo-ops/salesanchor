...
recon: docs/handoff/deal-removal-serviceC/recon.md
対象ADR: ADR-121
...
| 変更 | 詳細 |
|------|------|
| DesktopShell.tsx 修正 | showManagementCenter の hasAny() から "deals.view" を除去 |
| RolesPage.tsx 修正 | MENU_VIEW_KEY から "案件": ["deals.view"] エントリを除去 |
...
- 守り手: backend/tests/test_security.py（deals エンドポイント削除を CI 常時検証）＋ .github/workflows/workflow-lint.yml（CI 体制保証）
- 対象: deals API の再追加を防ぐ。routers/deals.py および schemas/deal.py が復活した場合、ImportError により main.py 起動失敗＋CI failing でマージ不可。
