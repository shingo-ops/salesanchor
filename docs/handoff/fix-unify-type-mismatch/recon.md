...
| `migrations/20260919_010000_master_ssot_work_id_recast.sql:74` | work_id UUID→INTEGER スワップ（pg_attribute ガードパターンの参考） |
| `migrations/20260916_120000_phase_c_drop_tcg_uuid.sql:7` | tcg_uuid DROP（このmigration適用済みの場合、tcg_uuid列が消える） |
| `scripts/run_all_migrations.sh:1` | 全 migration を毎デプロイ再実行する設計 |
