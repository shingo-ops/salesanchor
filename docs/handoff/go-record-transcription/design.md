...
## セッション委任の成立（2026-09-11 PO承認・現行）

PO原文: 「セッション上で委任した時点でGOを出せる権限を移譲されたと認識して良い」。設計担当が「このセッションで委任を受けた時点で、合意した対象についてAIがGOを発行する権限を得た」「GitHubでの追加有効化は求めません」と説明した後、PO原文「GO」を受領した。

この決定は、セッションでの委任成立をGitHubの専用操作まで保留する従来案を置き換える。委任はPOの発話で成立し、台帳登録・CI対応・App準備はその成立を遅らせる追加承認条件ではない。後述する本人workflow操作案、代理GO未有効という過去の権限判断は検討履歴であり、現在の承認条件に使わない。現行検査が代理GOに未対応という実装事実は変わらない。

### 合意と実装の区別

- PO承認済み: 本セッションでの委任を起点に、合意済み対象（GO制度変更を含む）でAIがGOを発行できる。POの追加GitHub操作は不要。
- 維持する条件: 24時間、範囲外/不明時停止、取消、対象PRとHEADの照合、差分レビューとCI、追加commit時の再GO。委任は自動的な設計合格・実装役への切替・外部設定変更の承認を兼ねない。
- 今回の番号なし「GO」は上記方式の承認として保存する。PO発話に番号を付加してGO #3418等へ転記しない。AIが個別GOを発行する場合は、別のAI発行記録を作り、PO本人の原文とは区別する。
- 観測時刻2026-09-10T22:35:50Z（JST翌07:35:50）はclock取得時刻であり、元の委任発話の時刻ではない。既存委任の発話時刻・失効時刻は本便では確定できていない。今回の確認や保存時刻で開始を上書きし、24時間を延長しない。元の時刻を一次記録から確認する作業は追加承認を求めることとは異なる。

### 記録と照合の設計案

委任記録は同一の委任に固定IDを付け、元発話の参照と原文、セッション識別、対象/許可操作、受任AI、開始/失効UTC、取消とその証拠を保持する。GO発行ごとに委任ID、対象repo/PR番号/HEAD、発行日時、レビューとCIの根拠を結ぶ。重複登録・再実行・同じ委任の再確認では開始/失効を変更しない。正式な元発話時刻が取得できない時に推測値を記入しない。

PR本文の既存4欄は維持する。発行者欄は「POの委任に基づくAI発行」と実行AIを明記し、日時はAIが実際にGOを発行した日時、GO原文はそのAI発行原文、バックアップ確認は実際の結果を記録する。委任IDと根拠の参照は4欄の外に付記する。PO名の部分一致で代理記録を偶然通す方法は採らない。

検査は「本人発行」と「委任に基づくAI発行」を明示的に分ける。代理では文字列allowlist追加だけで済ませず、元委任の参照・対象範囲・期限・取消・PR/HEAD・品質根拠の整合を照合する。PR本文だけではセッション原文の本人性を機械的に証明できないため、元発話を確認した担当による転記証明と機械検査の範囲を分けて記録する。外部CIが元セッションへアクセスできると推測しない。信頼できる転記記録の保存/更新権限と検査経路は引き続き設計残件。

### 受入条件案と自己審査

| 条件 | 確認方法・合格基準 |
|---|---|
| セッション委任成立に追加PO操作を要求しない | 正常ケースの工程にGitHub有効化待ち0件 |
| PO本人の原文とAI発行を区別 | 代理GOにPO名義への置換0件、委任参照欠落は拒否 |
| 期限と範囲を維持 | 期限境界/取消/範囲外/時刻不明/HEAD不一致の入力は個別GO拒否 |
| 同じ委任で期限を再開しない | 二重保存・再実行・再確認前後で開始/失効の変化0件 |
| PR作成とマージの検査を分離 | GO未発行のPR作成は許可、マージ前の欠落/不正は拒否 |

同一AIによる自己審査: この承認起点の変更はPO原文と整合。全体判定REVISE。元発話の転記証明の保存/検査、認可コードの分離、障害復旧枠、実App統合試験が残る。委任意思の再承認や追加有効化操作待ちを停止理由にしない。これらの検査実装は本便では変更していない。
...
セッション委任はPO発話で成立する方針へ更新済み。直下の「セッション委任の成立」を現在の承認条件とし、旧有効化案は検討履歴とする。代理GOの検査経路は未実装。
...
同一AI自己審査REVISE。権限委譲の対象に関するPO確認は解消した。上の未実装/未実測事項は確認済みの方式・仕様と区別する。設計草案の提出は可能だが、製品実装可能・代理GO検査対応済み・本番導入済みとは扱わない。
