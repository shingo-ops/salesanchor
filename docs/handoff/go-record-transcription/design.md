...
|---|---|---|---|
| A0 入力範囲の確定 | POが必要とする取消の入口 | 現画面のみ/同会話の別端末も含む、を区別した入口一覧。未指定入口を除外しない | POへ1問提示、回答待ち |
...
Architect同一AI自己審査REVISE。試験順序の循環と件数の数え違いを除き、AとBの依存を明示した。POの取消入口の回答、A1の実物、B0の設定差分が未確定。正式カード/追加診断/追加担当/新repo/鍵/GOは0。未解決のまま試験を開始せず、PO回答後は対象入口の接続方法の読取照合から進める。
