...
履歴要約（現在地は直下の2026-09-13 P2節を参照）: 専用App方式・取消期限・追加commit時の再GOは合意済み。通常の次予約解放は本番デプロイ成功後。マージ前の確定失敗は順番を返し、修正後に最後尾へ再受付。PO承認の緊急PRは待機列の最前列へ移す（実行中処理を中断しない）。以下の「現行の予約契約（失敗・緊急優先）」を最新の設計方針とし、それ以前の追補は検討履歴として読む。全体自己審査REVISE、ガード/CI実装未着手。公開検証repoのみ作成確認済み、App/鍵/保護設定/実機試験は未実施。

## 2026-09-13 P2期限・取消の詳細契約（草案）

POの「進めてくれ」をP2設計の継続として受領。P1は実機6/6合格、P2は実装前。以下はPlannerが作成し、末尾で同じAIがArchitect審査する。新たなPO決定・正式委任・実装カードとして扱わない。根拠はrecon.md「2026-09-13 P2の実物照合」。

### 目的・対象・変更前後

帰席後に「誰に、何を、いつまで任せ、いつ止まったか」を追える記録を作る。同じ元発話の再送による期限延長0秒、取消後の復活0件、確認できない状態での新規送信0件を成功条件にする。P1の合成受付は理由だけを出力する。P2では元発話・受付・確定記録・取消を結び付け、実行前判定へ渡せる状態を保存する。

本設計の対象は記録型・期限計算・状態遷移・競合/応答不明・検証条件。製品マージ/配備/再解析/配信を送信するコード、App作成/鍵/保護設定の適用、実grant発行は対象外。P2-10/12/14の判断はP3で検証するscope/品質入力を受け取る契約までとし、本文の自己申告だけでその入力をtrueにしない。

### 入出力と固定値

| 記録 | 必須情報と扱い |
|---|---|
| policy | schema_version、制御repo ID、固定code SHA、状態初期commit、PO ID、受付担当ID、受任主体、許可操作集合、scope版/hash。未確定値で稼働しない |
| source | target_repo_id、source_session_id、source_event_id、source_received_at、原文参照/hash。原文は非公開の正規保存先に置き、公開sandboxへ送らない |
| receipt | source参照、受付repo/Issue ID、sender/authorの数値ID、run ID/attempt、固定workflow/code SHA、本文snapshot hash、取得時刻。Issue編集は別の承認として採用しない |
| grant | origin_key、grant_id、source/receipt参照、受任主体、scope版/hash、操作・tenant・接続IDの集合、start/expires、policy版。作成後に上書きしない |
| revoke | revoke_event_id、origin_key、存在する場合grant_id、取消元発話参照/hash、受付証拠、記録時刻。対象の意味が未確認なら全体取消と推測しない |
| operation check | grantとpolicy版、元発話配送の確認位置、scope/品質証拠、now、操作ID、repo/PR/head/base、接続ID集合。判定結果と理由を返す。P2単体ではGO/外部送信しない |
| state | schema_version、control_repo_id、sequence、origin→grant索引、取消索引、操作予約と追記イベント。既存候補release/go-merge-stateの状態ファイル内に集約する |

origin_keyは従来案どおり(target_repo_id,source_session_id,source_event_id)。grant_idはこの3項目の順序固定配列をUTF-8 JSON化したSHA-256に接頭辞を付ける草案とする。scope/受付時刻/run/attemptをID材料へ入れない。各IDは空でない文字列か型を固定した整数とし、boolを整数扱いしない。文字列の暗黙trim/Unicode変換はしない。不正形式は拒否する。

同一内容の判定用hashは、承認に関する固定項目だけを対象にする。輸送上のreceipt時刻/run IDは除外して別履歴へ残す。操作・接続ID集合は重複を拒否してソート、JSON key順と空白を固定、浮動小数点/NaN/重複key/未知schemaを拒否する。原文の完全性hashは正規化前のバイト列から別計算する。hash一致は内容一致の補助であり本人性の証明ではない。

### 時間の契約

startは正規に確認した元発話の受信時刻、expires=start+86400秒。比較値はUTC Unix epochからの整数ミリ秒に固定し、期限差分は86400000ミリ秒とする。小数/秒精度への丸めで期限を動かさず、表示だけJSTへ変換する。後続セッション起動、登録再送、処理遅延でstart/expiresを書き換えない。新しい元発話の委任は別IDで審査し、古いIDを更新しない。

実行可能条件はstart <= now < expires。未来start、時刻不明、時刻取得経路不明は拒否。expires一致から拒否する。期限切れ後も取消・監査追記・送信済み処理の読取監視は許容するが、新規GO/変更は許容しない。履歴を消して「未登録」に戻さない。

登録時点で失効済みなら、失効した委任の証拠として記録可能だが実行可能にはしない。状態はACTIVE/REVOKEDという保存値だけで決めず、現在時刻からEXPIREDを算出する。旧LINEセッションの開始時刻を今回の設計保存日時へ置き換えない。

### 状態遷移と順序逆転

| 現在の記録 | 要求 | 結果 |
|---|---|---|
| origin未登録 | 有効な登録 | 固定grantを1件保存。時刻と取消索引から可否を算出 |
| grantあり | 同origin・同じ承認内容の再送 | 同じID/開始/失効/範囲を返す。必要なら受付履歴だけ追記 |
| grantあり | 同origin・承認内容が違う | ORIGIN_CONFLICT、元記録を保持 |
| origin未登録 | 検証済み取消 | originを指す取消索引を先に保存。後着の登録は実行可能にしない |
| grantあり | 検証済み取消 | 取消を追記。後着の登録・再送で取消を除去しない |
| 取消済み | 同じ取消の再送 | 元の取消結果を返す。別内容で同じ取消IDなら競合拒否 |
| 取消/期限切れ/配送確認不能 | 新規操作判定 | 拒否または未確認停止。外部送信0回 |
| 操作予約確定済み | 後着取消 | 後続操作を停止し、既に確定した操作は別追跡。「取消成功で送信済み処理を巻き戻した」と表示しない |

取消元が存在する委任を正確に指せる証拠が必要。AIが曖昧な発話からoriginを選んで取消索引を作らない。未登録originの取消も、固定した正規受付主体だけが記録できる。

### 保存・競合・取消到達性

状態更新は検証済み先端Pの唯一の子commitを作り、force=falseでrefを前進させる。登録/取消/操作予約は同じ先端で競合させる。敗者は旧判断を再送せず、最新状態・期限・配送確認を再評価する。取消が先に確定すれば予約は拒否する。予約取得は期限の延長ではなく、実送信直前にも同じexpiresと実時刻を照合し、期限切れなら送信しない。予約が先なら従来の送信確定境界として、その操作と後続取消を別表示する。

ref更新APIは期待する旧SHAを引数で比較する機能ではない。単一親・分岐した子commit・強制更新なしという条件の下で競合を拒否する。状態writerが複数親を作る、別writerが巻き戻す、偽の初期状態を作る場合まで保証しない。権限制約と初期commit照合は別の実機条件。

応答不明は保存失敗と同義ではない。候補commitが現在の正当な履歴内にあり、当該operation ID/内容hashが一致することを再照会する。先端一致だけでは、後続commitがある場合を判別できない。保存済みの確認ができても、外部送信を再実行する許可へ変換しない。破損/404/403/初期先端不一致では空状態を初期化せず停止する。

重要な未解決点: チャットで取消済みでも、受付側へ配送されていない取消はGit履歴だけでは発見できない。操作予約前に、元発話の受信経路と結び付いた連続した確認位置まで処理済みであることを検証する必要がある。その確認を表すフィールドをAIが自己申告するだけでは不足。正規の取り込み主体、欠落検知、取得不能時の停止、予約との整合を実測するまで、即時の取消保証は合格にしない。追加のPO有効化クリックを導入する解決にはしない。

GitHub Actionsのconcurrencyだけを取消順序・配送の保証にしない。未処理要求は永続記録から再照会可能にする。runner取消・待機・再起動を取消記録の代わりにしない。

### 既存14条件を具体化する入力と期待結果

P2-01: 固定した合成source時刻t、now=t+1、未登録originで1grantを保存する。
P2-02: receipt/runだけ異なる同内容再送でID/start/expires/scope不変。
P2-03: 同originのscope/受任主体/操作集合/元時刻を各1項目変更し競合拒否。
P2-04: event_id欠落、時刻欠落、未来start、未知policyを別入力で拒否。
P2-05: now=t+82800で登録しexpires=t+86400、残3600秒。
P2-06: now=expires-1は期限条件のみ通過、一致/+1は拒否。その他の条件は別途必須。
P2-07: 登録→取消→登録再送、取消→登録の両順序で復活0。
P2-08: 取消の読取拒否・記録破損・配送確認不能・保存応答消失で新規送信0。
P2-09: sender/authorを個別に未知IDへ変更して受付拒否。
P2-10: 自己認可変更はscope不適合、制度改善は別の明示scopeが検証済みの場合だけ候補。意味判定証拠欠落は拒否。
P2-11: PR本文だけにgrantを追加しても正当な履歴索引にないため拒否。
P2-12: head/base/Reviewer/CIを個別に不一致または未確認へ変え、候補発行0。
P2-13: 同じPから取消と予約の子commitを作る2順序、取消配送遅延の別条件を確認。遅延不明は合格にしない。
P2-14: 接続ID集合[A,B,C]に対し[A,B,D]と[A,B,B]を拒否。同じ件数だけでは合格にしない。

14は試験群数であり、上記の複数入力を14回実行済みと数えない。今回はいずれも未実装・未実行。P1実機6件は別の証拠。

### 実装範囲・維持・審査

将来の実装範囲は制御側の型/正規化/時刻判定/状態遷移/保存アダプタ/契約試験。製品側GO検査とmerge/deployの接続はP3/P4。新規実装ファイルの実在する親ディレクトリ・命名・専用App/権限の具体差分が未確定なので、正式カードの触るファイル欄を仮置きしない。

設計担当はcontractと14試験群、実装担当は固定policyを読み取るコード、PO管理の担当は認可根・鍵停止/復旧、Reviewerは実権限と否定試験を維持する。状態初期化・鍵/権限変更は通常grantのscope外。人の責任と機械の停止条件を両方確定する。

Architect判定: REVISE。理由は取消の正規配送/欠落検知、状態writer/App/固定codeと初期commit、信頼できる時刻取得、実権限の対照試験が未確立であるため。14試験群の契約は具体化済みだが、実装可能とは認定しない。P1合格は維持。PO判断の新規受領0、実装0、権限変更0、GO発行0。本節はADRのWhyへ転記できる設計理由を整理した草案である。

### 取消の正規取り込みと状態writer（2026-09-13追補・草案）

今回の追加調査で、既存CLIラッパーは入力をcodex execへ渡すだけで、受付証明と保存処理を持たないことを確認した。Codexのthread/readと通知は調査候補になるが、それだけを取消の到達証明にはしない。詳細証拠はrecon.md「2026-09-13 入力経路の仕様照合」。

**結論**: 通常チャットを利用する方針は維持する。代理GO対象の入力を受けた時点で、AI推論より前に記録する独立した受付処理が必要。既存画面にその処理を接続できるかは未確認であり、新しい画面への移行をPO合意済みとしない。接続未確認のセッションで代理GOを有効化しない。

| 候補 | 評価・理由 |
|---|---|
| 会話履歴を後から読む | 元発話調査の補助。通知断線中の取消と現在の入力の完備性を証明できない |
| AIがIssueへ転記する | P1で送信と判定は検証したが、転記されなかった取消を検知できない。本番の単独の認可根には不採用 |
| 入力受信処理が先に永続記録する | 推奨草案。元入力と保存位置を結び付け、モデル停止中も取消を記録できる。ただし現UIへの接続と権限分離は未実測 |

#### 受付と送信確定を同じ順序にする契約

1. 正規入力を受けるホストが、固定session ID・単調なsource_seq・ランダムな受信ID・受信時刻・入力hashを割り当てる。元発話IDをモデルに自由生成させない。元入力が再配送された場合は同じ受信IDを用い、配送IDを失った再送は同一と推定しない。
2. 受信ホストはAIへ渡す前に、状態writerへ受信イベントの保存を要求する。保存応答未確認の入力がある間、そのセッションの新しい操作確定を止める。入力は消さず「記録確認中」として扱う。保存遅延でstartを延長しない。
3. 取消の意味判定が終わっていない新入力も、先に未処理として記録する。未処理入力を無視して先の委任で送信しない。取消対象が曖昧なら確認待ちとし、無関係な入力と推測して解除しない。
4. 状態writerだけがsource_seqの連続性・入力hash・分類済み位置・取消と操作予約を更新する。操作確定要求はsession/generation/最後に分類済みのseq/状態版へ結び付け、未分類入力または欠番があれば拒否する。
5. 受信ホストからの入力追加と操作確定を同じ受付順序で処理する。単に一度「最新seq」を読み、その後別経路で予約を書く二段階方式は採らない。取消記録と送信確定の先後を一つの正当な履歴に残す。
6. 受信ホストとの接続断・応答不明・世代違いでは確定しない。ハートビートの直近成功だけを新規確定の根拠にしない。古い接続が戻っても新しい世代へ無条件に復帰させない。
7. 再起動時は保存済みイベントと未処理範囲を照合し、同じ受信IDで復旧する。原文の履歴readは補助照合に使えるが、抜けたID/受信時刻を閲覧時刻で補完しない。
8. 予約後にも実送信直前の期限・scope・品質照合を維持する。送信結果不明は従来のIN_FLIGHTで保持する。取消の確定後に新たな操作を作らない。既に確定した操作の巻戻し保証を表示しない。

source_seqとgenerationは新設する受付契約の値で、Codex APIが現在発行すると確認した値ではない。すべてのPO入力経路がこの受付処理を通る保証もまだない。別端末・別入口からの取消を扱う経路が対象外なら、その制限をPOへ明示して方式判断を受ける。未承認の制限を実装担当が選ばない。

#### 状態writerの配置と責任

状態正本は既存案の専用Git branchを維持する。P2検証では実原文を使わず合成入力のみ。本番の原文・受信証跡はアクセス制限された保存先へ置き、公開sandboxに転記しない。受信ホストは直接stateを書かず、writerは制御コードを書き換えず、製品送信主体はstateの取消を消せない権限構成を実機で確認する。

writerの登録APIは任意JSON/任意ref書込口にしない。許可する要求は入力記録、分類結果の検証、取消記録、操作確定、送信結果追記。policy/受付主体/初期commitの変更は別のPO管理経路。認証情報をAIの通常シェルへ渡す方式は採らない。ホストとAIが同じOS権限で秘密や記録を書けるだけの構成では分離を合格にしない。

新設の稼働場所、常時稼働の費用、端末停止時の挙動、管理担当、App権限と秘密の保管先は具体差分にしてからPOへ示す。今回は候補名のrepo作成・サービス起動・鍵発行をしない。

#### 先行する取り込み検証8条件（未実施）

| ID | 入力・障害 | 合格条件 |
|---|---|---|
| I01 | 同じ元入力を2回配送 | 保存IDと受信時刻不変、1つのsource_seq |
| I02 | 入力保存応答を消失 | 新規確定0、照合で同じ記録を回復、期限延長0 |
| I03 | source_seqの欠番/逆順 | 欠番解消と分類確認まで確定0 |
| I04 | 取消の受信後にモデルを停止 | 取消保存は進み、新規確定0 |
| I05 | 新入力到着と確定要求の競合 | 2順序で履歴が一致。入力先着なら未分類のまま確定0 |
| I06 | 接続断と旧世代の再接続 | 旧世代の確定0、再同期なしの復帰0 |
| I07 | 保存後・AI配送前で再起動 | 元時刻/ID不変、未処理入力の消失0 |
| I08 | 別端末/通常画面/履歴通知経由の取消 | 対象入口の取りこぼし0。経路不明なら未確認停止 |

これらはP2-04/08/13の前提を補う8試験群で、P2の14群やP1実機6件に合算しない。実行には対応する限定設計・正式カード・担当の明示委任が必要。

追加Architect自己審査: REVISE。入口のAPI形状を確認したが、現UI入力の完全な観測とwriterへの原子的な確定経路を証明できていない。今回は方式候補と8検証条件を具体化した段階。次はこの連携を既存画面に接続できるかの限定検証設計。P1合格維持、P2実装カード未発行、代理GO有効化なし。

#### 現画面接続の候補と限定検証（2026-09-13）

前節で未確認だった入力時の接続候補として、CodexのUserPromptSubmit hookを確認した。現環境のユーザーhooks.jsonにはカード検査用の同イベントが1件あり、呼出先は既存card-lint-hook.sh。これは設定とコードの実物確認で、現在の各入力で起動したという実行証拠ではない。既存hookは変更しない。

公式仕様ではUserPromptSubmitへprompt/turn_id/session_id等が渡り、同期command hookは処理終了まで待機し、明示的なblockまたはexit2で入力を止める。turn_idは元発話IDの代わりにならない。同一turnの追加入力、キュー入力、再開後の入力で発火するかは実機で確認する。別クライアントへの適用を推測しない。

現在のcard-lint-hook.shは、カード以外や入力欠落・検査器不在等でexit0になる。保存先も共有の一時ファイルへの上書きである。この処理を認可正本へ転用せず、カード検査を維持したまま独立の受信記録部品を設計する。デバッグ用の最後の入力ファイルは他セッションと競合し得るため、今回のPO発話の証拠として読まない。

フックが入力を止められることと、実行中の別担当/外部送信を止められることを区別する。UserPromptSubmitだけで既存の送信所有者を失効させたとは扱わない。最終送信入口で正規writerの状態を検証する条件を維持する。MCP型hookは接続先エラーや不在で操作が継続する公式条件があるため、単独の強制停止装置にしない。command型の例外/timeout/killでも安全に止まるかは未確認。

**検証を3段階に固定する。**
- H0（今回完了）: 公式入力/停止仕様、現地設定のイベント名/呼出先、既存処理の失敗分岐を読み取り照合。設定変更・hook起動は0回。
- H1（準備計画）: 他セッションを含まない隔離した診断用の作業場所とテストセッションで、固定合成マーカーだけを扱う記録hookを検証。通常設定を上書きしない。承認前に診断部品・有効化する設定差分・戻し方を文書として提示する。追加セッション/担当と設定適用は明示承認後。
- H2（H1後）: 現画面・実行中追加入力・再開・複数入口の捕捉と、外部の状態writerへの連携/最終送信拒否を対照試験。H1成功をH2合格にしない。

| H1で固定するケース | 合成入力/障害 | 観測と停止条件 |
|---|---|---|
| H1-01 | 診断マーカー1件 | 実hookイベント1件、session/turnと記録対応、原文を公表しない |
| H1-02 | 同じ本文を別入力として2回 | 別受信として識別。turn/hashだけで同じ発話へ潰さない |
| H1-03 | 明示blockを返すマーカー | モデル処理前に停止。既存担当の実行停止と混同しない |
| H1-04 | 診断記録先を意図的に書込不能 | 偽ACK0。会話が継続した場合は停止強制の失敗として記録 |
| H1-05 | hookのtimeout/異常終了 | 実挙動を観測。素通りならこのhook単独の強制停止案を採らない |
| H1-06 | 再開/実行中追加入力 | 入力と発火を照合。元時刻/入力ID不明を後付けで補わない |

H1の6ケースはI01〜I08の実装前提を調べる診断であり、P1の6件と別。H1は未実施。診断部品草案は[hook-h1-diagnostic.py.txt](hook-h1-diagnostic.py.txt)に保存し、構文のみ確認済み。設定差分と正式実行カードは未作成。短いプロンプトでも新規モデル実行を伴うため、通常の読取調査として起動しない。hookの信頼確認を省略するオプション、既存ガードの無効化、ユーザー全体の設定の一時上書きは許可しない。

自己審査: 接続候補の選定とH0は根拠あり。P2全体はREVISE。次の成果物はH1診断部品と隔離手順のレビュー可能な草案で、P2製品実装カードではない。既存画面接続の可否はH2まで未確定。

### H1隔離設定・正式カード完成（2026-09-13）

[TH-GO-HOOK-H1-01.txt](TH-GO-HOOK-H1-01.txt)を本段階の正式カードとする。入力は診断部品・[hook設定草案](hook-h1-hooks.json.txt)・[診断セッション指示](hook-h1-agents.md.txt)の3件でhashを固定。専用場所/private/tmp/salesanchor-go-hook-h1-20260913へ原本のまま配置し、ユーザー設定を上書きしない。製品Gitとは独立した診断用Git境界を作り、commit/pushなしとする。

実行承認で求める範囲は、実行担当1名と試験対象の診断セッション1つ、専用プロジェクトと診断hook1件の通常信頼登録。2つの役割を「担当1名だけで追加AIなし」と誤記しない。既存hookの無効化や信頼確認の迂回は含まない。初回プロンプトを送る前に通常の/hooksで定義を照合する。

マーカーはnormalを2回、block、書込失敗、timeout、exit1、同じsessionの再開後normalの最大7入力。実行中追加入力の経路はH2へ明示して残す。2秒制限に対する10秒待機とexit1はfail-openの有無を観測する診断であり、不合格/不明が出たら残りを停止する。P2安全性や現画面の合格と混同しない。

書込失敗は/dev/null配下へのディレクトリ作成で例外を起こす。既存ファイルの権限を変えない。記録内容はsession/turn/イベント/モード/診断時刻のみ、実プロンプト本文・認証情報は保存しない。レコードが書けてもtimeoutなら保存ACKとはみなさない。保存先は診断用で、永続認可の正本ではない。

検証: Python構文、JSON設定、カード内3hash一致、CLI0.154.0の起動/明示ID再開のhelp、公式のproject-local hook・通常信頼登録を照合。card-lint exit0（長行警告5件）。実機0、モデル追加起動0、信頼/設定変更0。

Architect同一AI自己審査: H1の限定診断実行設計APPROVE。根拠は実在する原本、固定出力、上限、通常信頼経路、拒否/未知時の停止と証拠保存。これは診断を実施可能にする判定で、試験結果合格ではない。P2全体REVISEを維持。次の一手は本カード範囲へのPO明示委任。以前のP1委任は流用しない。

### H1実測後の設計判断（2026-09-13）

POの限定委任に基づき正式カードを実行。合成5入力のうちnormal2回・明示block・書込失敗の4件は期待一致、timeout1件は2秒超過のエラー後にモデルがH1-ACKを返した。親も専用ログ/JSON/hashを直接照合した。exit-error・再開は停止条件で未実施、実行中追加入力・現画面経路は未確認。証拠とPO承認範囲はrecon.md「2026-09-13 H1実機診断の停止結果」。

Architect同一AI自己審査: hook単独の強制停止案はREJECT。時間切れ時の診断記録が存在しても、処理を止めた証明にはならない。P2全体REVISE、製品実装カード未発行、代理GO0。上のH1未実施/承認待ちは準備当時の履歴。限定診断設計APPROVEを本番認可の合格には使わない。

Whyへ残す根拠: 実機5件中1件で障害後に応答が継続したため、モデルの停止に認可を依存させられない。記録hookは入力観測の候補として残せるが、未保存・期限切れ・取消・配送不明時の外部送信拒否は、既存P2の別主体による境界で保証する必要がある。これは次の設計条件であり、その主体/実権限/全入力捕捉は未確立。今後の検証を経ずに安全としない。タイムアウト延長や再試行で今回の不合格を取り消さない。

### 送信側で拒否する契約（2026-09-13・草案）

目的は、H1のようにモデルが応答を続けても、期限切れ・取消・未保存・根拠不明の新規外部変更を0件にすること。対象は既存P2/P3の送信判定とP4へ渡す配備開始条件。実装/設定変更、診断再開、新担当起動は含めない。既存の24時間・チャット委任・追加の有効化クリック不要というPO合意を変更しない。

#### 既存経路との具体的な差

2026-09-13読取基準のmain 66b417665c013fdb354d5bda63226d07a1b2182cで、deploy.yml:3-6はmain push起動、:64-78はSSH資格利用とLPへのrsync、:183-184は最新origin/main取得を確認。既存の直列化(:14-16、ADR-092)は維持する条件で、期限/取消を判定する機構ではない。後段のDocker開始直前だけに検査を追加する案では先行LP書込を防げない。PR起点SHAと配備時mainが同一という保証もこの記述からは得られない。

同mainのgh-pr-merge-safe.sh:19-22はActions時exit0、:93-111はmain追従・push・再merge。承認HEADを固定する新経路へそのまま転用しない。mainの有効rules GETは4種（deletion/non_fast_forward/pull_request/required_status_checks）、必須status13件、レビュー人数0、merge方式のみ。専用送信Appだけに更新を限定する規則はこの応答で確認できない。ruleset詳細のbypass_actors:nullは迂回者不存在の証明にしない。通常資格/他App/SSHによる対照試験は0件。

#### 責任と拒否位置

| 主体 | 許す要求と固定する対象 | 必ず拒否するもの |
|---|---|---|
| セッション担当 | 登録対象への型付き操作要求。grant_id・operation_id・repo/PR/head・操作種別を提出 | 任意shell/URL/ref、自己判定のapproved=true、資格取得 |
| 元入力受付/状態writer | 正規元入力の順序・未分類・取消・確定操作を同一履歴で扱う | hook記録がないことを取消なしへ読み替える、欠落したseqの補完 |
| 送信主体 | 信頼するwriter状態、固定policy、外部から再取得した品質証拠に基づく1操作 | 自己申告/古い許可票、旧HEAD、期限不明、状態読取不能、未確定送信の再送 |
| 配備主体 | 確定merge SHAに結び付く別の配備開始判定を、最初の本番書込より前に行う | mainの可変先端の採用、マージ許可を配備許可へ流用、失効後の新規開始 |
| PO管理の保守経路 | 固定policy/鍵/許可主体の別承認による保守 | 同じ委任による認可根の自己更新 |

App鍵や本番資格を通常AIシェル/対象PRコードに渡さず、制御コードは製品PRをcheckoutして実行しない。送信Appは状態writerの取消を書換不可とする。旧節の「製品Appに状態保存先writeも許す」候補は本草案では採用しない。専用Appは名前だけで分離成立とせず、実installation/repo/権限と拒否試験で確かめる。PO管理の復旧経路は別記録し、委任AIが通常資格へ切り替えて同じ操作を行う代替経路にしない。

#### 操作確定と障害の扱い

1. 送信主体は要求を構造検査し、repo/PR/head/対象集合を登録済みscopeへ照合する。レビュー担当・CI名だけでなく対象SHA・結論・必要な発行主体を取得する。required review count=0を設計レビュー合格の証拠にしない。
2. writerで元入力追加・未分類・取消・操作確定を一つの順序で処理する。入力配送が完全か確認できない間は拒否。hookが最後に成功した時刻や短い許可票だけでは、まだ配送されていない取消を検知できない。
3. 確定記録はgrant/policy/操作ID/repo/PR/head/base/証拠参照/入力確認位置/判定時刻/元expiresへ結び付ける。送信前に確定保存を確認できなければ0回。別プロセスへ無期限の許可票を渡さない。
4. writerで操作確定後、送信主体は同じexpiresと実時刻を直前照合する。予約で期限を延ばさない。確定後に後着した取消は次の操作を止めるが、確定済み操作の巻戻し保証はしない。この先後は元入力受付と同じ順序で証明する必要がある。証明できない現状では実装合格にしない。
5. 同期merge APIを候補としsha=head、merge_method=mergeを必須にする。公式APIのshaはHEAD一致条件で、baseの期待SHA比較ではない。base変化をAPI一引数で封じたとはしない。送信直前再取得とサーバー側保護/専用主体への限定を組み合わせ、その競合試験に合格するまで未確立とする。
6. 送信後のtimeout/応答欠落はIN_FLIGHT保持。PRとmerge SHA/runの読取照合のみで、単に未マージ表示や一定時間経過を見て再送しない。HTTP失敗から未送信を断定せず、拒否確定と応答不明を分ける。再試行の可否を別に確定する。
7. merge確定と配備開始は別操作。待機中の取消/失効なら配備0回とし、MERGED・配備未開始を表示する。自動巻戻しや新しい期限を発行しない。進行中migrationを取消で強制終了する方式はADR-092に反するため採らない。既に開始済み処理の観測と、新規配備/再解析/配信の停止を分ける。
8. 再解析・3接続配信の送信側にも別の開始判定が必要。配備成功だけでは許可しない。解析ジョブの完了集合・対象テナント・固定3接続IDの照合を残すが、実エンドポイント/資格/送信境界は今回未調査で、マージ用Appだけで達成したとしない。

#### 受入試験の具体化（S01〜S10、未実施）

| ID | 入力/障害 | 合格条件 |
|---|---|---|
| S01 | H1同様のhook timeout後に操作要求 | 正規入力保存を確認不能なら外部送信0 |
| S02 | hook無発火・入力欠番・未分類取消 | いずれも送信0。最後の成功記録で許可しない |
| S03 | 保存応答欠落、writer403/破損 | 送信0、空状態初期化0 |
| S04 | 有効条件を満たした合成操作を重複要求 | 正常1操作だけ送信、重複送信0、結果同定可能 |
| S05 | HEAD変更とbase変更を検査後に挿入 | HEAD不一致は拒否。base競合も未検証組合せのmerge0。検証前後SHAを記録 |
| S06 | 取消と確定を両順序で競合 | 取消先着なら送信0、確定先着なら当該操作だけ別追跡、後続0 |
| S07 | expiresちょうど/直前検査後に失効 | 直前判定で失効なら新規送信0。判定/送信時刻と境界の意味を記録 |
| S08 | merge送信後に応答消失/送信主体再起動 | 再送0、IN_FLIGHT保持、正当な結果のみ照合 |
| S09 | merge後待機中に取消/失効、別mainが進む | LPを含む本番書込0、可変mainへの置換0、配備未開始を表示 |
| S10 | 通常資格で直merge/状態改変/配備、製品Appでstate改変、対象PRで鍵参照 | 全て拒否。正規送信だけ成功する対照を同じ設定版で確認 |

既存P2/I/H試験群へ追加する送信境界の計画で、10件を実機成功数へ加算しない。保守担当はpolicyと許可主体一覧を維持し、Reviewerは設定版とS10の拒否証拠、運用担当はIN_FLIGHT照合と取消演習を更新する。具体的な担当名/配置/費用は未決のまま、受任AIが補完しない。

Architect同一AI自己審査REVISE。H1による単独hook不採用は維持。APIのHEAD固定と既存配備との差は根拠を確立したが、元入力の完全捕捉・権限分離・base競合・配備開始を止める配置が未確立。正式実装カードは発行しない。次は、この表を満たす制御主体/配備主体の配置と権限差分を具体化し、実物照合できる項目とPOの運用判断が必要な項目を分ける。新規PO判断を受領したとはしない。

### 制御・配備の配置と権限案（2026-09-13・未承認草案）

推奨候補は、製品repoと分けた非公開の制御repo、非公開の記録repo、制御repo側のGitHub-hostedジョブである。既存公開sandboxは合成試験専用として維持し、本番原文/鍵を置かない。repo名・ID・App ID・Environment名は未作成/未確定であり、ここで実装カードの実在パスとして発行しない。

#### 配置案と採用理由

| 案 | 利点 | 制約・判定 |
|---|---|---|
| 製品repo内のworkflowと状態branchへ集約 | 新しい置き場が少ない | 製品コード保守と認可コード/鍵への到達を分けにくい。今回の推奨から除外 |
| 非公開の制御repo＋記録repo、GitHub-hostedジョブ | コード更新資格と状態更新資格をrepo単位で分けられる。ADR-075のGitHub Secrets管理を維持 | 推奨草案。非公開repoの保護機能/契約・利用枠と起動遅延は未確認。原入力捕捉は別途必要 |
| 専用の常時稼働サーバー | 元入力受付と送信順序の処理を同一サービスへ置ける候補 | 稼働費/監視/更新/秘密展開が追加。現画面との接続が証明されていない以上、サーバー新設だけで解決しない |

GitHub-hostedジョブは操作の検証/送信場所の候補であり、現画面の入力を常時受信するホストの代替ではない。受付→writerの順序と欠落検知が未確立の間は全ての認可を停止する。離席と端末オフラインを同義にしない。端末や受信経路が切れた場合に新規操作を止める契約を維持する。

#### 資格の割当候補（全て設定未実施）

| 主体 | 保存する資格/実行場所 | 対象権限の上限 | 与えない権限 |
|---|---|---|---|
| PO管理の制御保守担当 | 個別管理。実担当名は導入時に指定 | 制御repoのレビュー済みコード/policy保守 | 今回の委任による自己認可変更 |
| 状態writer App | 制御repo側GitHub Secretsから固定writerジョブだけに渡す | 記録repo Contents write、対象repo ID限定 | 製品repoと制御コードrepoへのwrite、main merge、本番SSH |
| 製品merge App | 制御repo側GitHub Secretsから固定mergeジョブだけに渡す | 製品repo Contents write、PR/Checks/Commit statuses/Actionsの必要read候補 | 記録repo write、制御repo write、Administration、秘密設定変更 |
| 配備主体 | 制御repo側の別ジョブ/資格。既存鍵の単純コピーはしない | 固定SHAの配備/health/既定復帰に必要な接続だけ | merge App鍵、writer App鍵、任意コマンド要求 |
| 受任AI/入力転記担当 | 現行シェルへ新資格を渡さない | 登録された要求の提出、許可された結果の読取 | 上記秘密、状態直接書込、専用送信経路外のmerge/配備 |

必要read権限は候補で、各利用APIとの対応と実installationを未検証のまま「最小権限確定」としない。Appは別々にし、一つのApp秘密鍵から両repoへ書ける構成を避ける。短命tokenを生成するときもrepository_ids/permissionsを明示し、応答の対象/権限/期限を照合する。GitHubのtoken有効期限は委任の24時間や取消を代行しない。

Environmentを別名にするだけでコード保守者から秘密が隔離できたとはしない。制御repoにwriteできる主体は認可根の信頼対象になるため、受任AIには付与しない。既存PO合意に反する毎回のGitHub手動承認を追加する解決にはしない。保護機能が契約上使えるか、固定code以外のジョブが秘密へ届かないかを検証する。Actions policies GETは403のため現行設定未確認、他資格への切替で取得しない。

#### 配備側に必要な差分と適用順序

1. 先に非本番でS01〜S10の正常/拒否対照を行い、制御/記録repo・App・固定code・初期state・権限・鍵の所在を既存の権限秘密台帳の方針へ登録する。新repo作成/鍵発行は具体カードと別承認後。
2. 現行deploy.ymlの権限付き処理を、制御側の固定配備処理へ移す差分が必要。製品コードのビルドは本番資格を渡さないジョブで行い、出力をrepo/commit/run/内容digestへ結び付ける。成果物の存在だけで信頼しない。配備処理は任意URLやPR内scriptを指示として実行しない。
3. 既存のmain push→配備経路と新経路が同時に書ける状態を本番で作らない。旧workflowの切替、旧資格での直配備拒否、main更新者制限、固定SHA取得を一つの導入計画にまとめる。旧資格の削除/失効は今回実施せず、復旧経路・影響一覧・検証を揃えてPOへ渡す。
4. 最初のLP書込からDocker/migrationまでを配備操作の開始対象に含め、開始時の期限/取消を検査する。merge後に失効した場合は配備未開始で停止。開始済み処理はADR-092の非中断を維持する。
5. ADR-115のhealth失敗時に直前PREV_SHAへ戻す既定の復帰処理は維持する。前節の「自動巻戻しなし」は、取消や期限切れだけを理由とする新規巻戻しを指す。開始済み配備の既定復帰を禁止する意味ではない。復帰先固定・結果記録・deploy失敗扱いを保ち、任意の復旧PRを旧委任で開始しない。
6. 製品実行コードが必要とするDB/外部サービス資格までゼロにできるとは主張しない。認可根のApp鍵と製品の実行資格は別の保護対象。再解析/3シート配信は実エンドポイント/実行資格の特定が未完了で、この配置だけでは全対象の運用開始に進めない。

Secretsの管理元はADR-075/B-11に従いGitHub Secretsとする。新App鍵をローカル.envや恒久VPSファイルへ手渡しする案は採らない。既存SSH運用の制約は実物を確認してから差分にする。鍵管理台帳の既存テーマはdocs/specs/secrets-permission-ssotであり、実在しない鍵を登録済みとしない。

#### 運用・費用・審査

PO管理の保守担当は鍵失効/権限復旧/固定policy更新を担当し、運用担当は配備結果/IN_FLIGHT照合、Reviewerは否定試験と秘密の到達性を維持する。担当の実名・緊急連絡・復旧所要時間は未決。GitHub-hostedの採用候補は常時サーバーを新設しないためで、費用0の証明ではない。非公開repoは契約別の無料枠と超過課金がある。計算に必要な実行回数×所要時間、保存量、契約、残枠は未取得で、月額予測を創作しない。

Architect同一AI自己審査REVISE。配置と資格の分離案、旧配備切替と既定復帰の維持条件は具体化した。原入力受付の配置/全入力捕捉、非公開repo保護機能、実権限、費用、実担当が未確立。推奨案はPO採用決定ではなく、repo/鍵/本番設定変更の承認でもない。次は「未解決の入力受付」と「非本番で先に確認できる権限分離」の依存関係を切り分け、追加診断が必要な最小範囲を設計する。

### 最小検証の依存関係（2026-09-13・草案）

目的は試験を増やすことではなく、入力経路と権限境界の可否を少ない試験で判定すること。既存H1の5入力を再送しない。timeout後の継続は既に観測済みで、exit1/再開が成功しても単独hook REJECTを覆せない。公開P1の6件も本人確認/入力完全性/権限分離の代用にしない。

| 段階 | 先に必要なもの | 最小の成果物と終了条件 | 現在地 |
|---|---|---|---|
| A0 入力範囲の確定 | POが必要とする取消の入口 | 現画面のみ/同会話の別端末も含む、を区別した入口一覧。未指定入口を除外しない | PO原文「スマホ別端末も含める」受領。範囲合意済み、実端末/接続は未確認 |
| A1 入力経路の観測設計 | A0、対象ホスト/設定/通常信頼手順/記録先の実物 | 下記3観測のカードと原本。入口/適用手順が不明なら発行しない | 未確立、カード未発行 |
| B0 権限試験の準備設計 | 配置案の採否、実在する非本番対象、設定差分、実行担当と承認 | 専用資格と保護を適用する具体カード。鍵の値は文書に書かない | 設計草案のみ、作成/変更0 |
| B1 権限分離の対照 | B0適用後の実ID/設定版/資格 | 下記6操作の成功/拒否と対象差分を照合 | 未実施 |
| C 入力と送信の統合 | Aの対象全入口、B1合格、固定writer/送信実装 | I01〜I08/P2/S01〜S10の条件へ接続し、欠落・取消・応答不明で送信0 | 未実施 |
| D 配備移行の検証 | C合格、固定配備処理と旧経路停止差分 | 非本番で配備開始/失効/既定復帰を検証してからPOへ導入判断 | 未実施 |

前節「先に非本番でS01〜S10」は準備が必要なB0/B1を省略する意味ではない。S01〜S10を実行するために未承認のApp/鍵を先に作る循環を避け、この依存順序を現行とする。Bの設計はAの回答待ちでも進められるが、Bの成功だけでCへ進めない。

#### A1の最小観測3種（実行指示ではない）

| 観測 | 合成入力 | 何を比較するか |
|---|---|---|
| 通常入力 | 許可した各入口から1件ずつ固有マーカー | 送信した入口/件数と、受信イベント/session/turnの対応。別CLIの成功で現画面を代用しない |
| 実行中の追加 | 同じ会話の処理中に固有の取消マーカー1件 | モデル終了を待たず受信記録が発生するか。受信時刻/保存時刻/モデル到達を区別 |
| 受信不能 | 診断用の経路だけで保存失敗を起こす | 未記録を検出して外部確定を拒否できるか。hookの発火回数だけで欠落0としない |

合成マーカーだけを記録し、他セッション/実取消原文は読まない。現画面の設定変更、入力操作、障害注入は本便で行わない。今回の利用可能ツールから現画面の入力受信を直接監査する専用APIは確認できない。公式Hooksのsession_id/turn_id/promptと、transcript形式が安定インターフェースではない説明だけでは、全入口の一意受信/欠番検出を保証できない。A1で発火を確認しても、元入力の完全性と順序はCまで別条件として残す。

#### B1の最小対照6操作（非本番・合成対象限定）

1. writer資格で固定記録repoの合成stateを更新: 成功1件、単一親の履歴と記録内容を照合。
2. 同writer資格で制御コードrepoを更新: 拒否、差分0。
3. merge資格で条件合格済みの合成PRをmerge: 成功1件、許可HEADとmerge方式を照合。
4. 同merge資格で合成stateを更新: 拒否、差分0。
5. 通常の申請資格で同じ状態更新とmain更新を要求: 両方拒否、差分0（操作種別2件）。
6. merge資格で必須チェック不合格の別合成PRをmerge: 拒否、差分0。

6は「専用Appだけ通れる」と「既存品質条件を無視しない」を区別する対照。5は2操作なので、6行をAPI6回と誤記しない。最小7変更要求、各要求の前後GETを別に数える。拒否される予定の操作も成功すれば変更が発生するため、本番対象では試さない。予期しない成功が1件でもあれば停止し、自動巻戻し/別資格/条件緩和で続けない。実在対象・hash・事前状態・ログ先・承認が未確定なので正式カードを発行しない。

この7要求では鍵漏洩、配備資格、取消配送、全S10を網羅しない。PRコードのsecret到達性や配備資格の拒否試験は、配備実物が揃ったDで追加する。B1だけを権限全体合格にしない。

Architect同一AI自己審査REVISE。試験順序の循環と件数の数え違いを除き、AとBの依存を明示した。取消入口は後続PO回答で確定。A1の実物、B0の設定差分は未確定。正式カード/追加診断/追加担当/新repo/鍵/GOは0。未解決のまま試験を開始せず、対象入口の接続方法の読取照合から進める。

### スマホ・別端末を含む取消受付（2026-09-13）

PO原文「スマホ別端末も含める」を受領。確認文脈は「この画面に加え、スマホ・別端末から同じ会話へ送る取消」である。A0の入口範囲はPO合意済みとし、再承認は求めない。別の会話への発話を任意の委任へ関連付ける仕様までは合意していない。新しい端末登録/リモート有効化/診断起動/GO/本番変更の承認とは区別する。

#### 公式仕様と実機未確認を分けた入口一覧

| 入口 | 調査で確認した接続候補 | 検証で固定する実物 |
|---|---|---|
| 現在の画面 | ローカルの会話入力 | アプリ版、会話ID、実行host、受信記録の取得口 |
| スマホ | 公式Remote接続で同じhostの会話を操作 | 端末OS/アプリ版、接続hostと同じaccount/workspaceである確認（秘密は保存しない）、同じ会話の対応 |
| 別端末 | 対応するdesktopからremote hostの同じ会話を継続 | 実端末/アプリ/host/会話の対応。別のローカルコピーで代替しない |

公式Remote connectionsは接続hostが起動・接続状態であることを前提にし、スリープ/通信断/アプリ終了でremote接続が止まると説明する。実行場所を別hostへ移すhandoffは、同じhostを他端末から操作することとは別の変更。handoff発生時はsource generation/記録位置を再照合するまで新規確定を止める設計とし、本便で移行操作はしない。仕様上の候補があることを、PO端末の接続済みや全入力の保存保証としない。

#### 観測手順の具体化（A1、カード未発行）

1. 実端末の種類と同じ会話への既存接続を確認する。接続設定の変更/QR認証/別アプリへの切替は読み取り確認に含めない。現在の端末とアプリ名はPOへ1問で確認中。
2. 実在する入口集合をEとして固定する。同一端末でも別アプリ/別経路なら区別し、要求した経路を未検証のまま除外しない。現時点でEの実数は未確定。
3. 各入口について、待機中1回・実行中1回の固有合成マーカーを送る計画とする（基本入力数2×|E|）。端末側の送信操作、同会話への反映、受信hook/event、永続保存、取消判定、送信側拒否を別列で記録する。試験担当が手動でメタデータを補って一致させない。
4. 合成通信障害は、診断専用環境でのみ、端末→host未到達とhost→writer保存応答不明を分ける。POの実PCのネットワーク断/スリープを自動で発生させない。遅延分の再配送で受信ID/元時刻/取消状態を改変しない。
5. 同文を別端末から送る別入力、同じ元入力の再配送を区別する。前者は別イベント、後者は重複処理なし。入力本文hashだけで同一性を決めない。
6. 受信と操作確定の先後を正規記録で証明できないケースは未確認停止。無応答/ログなしを「取消なし」としない。host到達確認のないスマホ送信を「取消完了」と表示しない。画面反映やモデルの返事だけで取消保存済みとはしない。

端末が通信できず取消がまだ送られていない間に、他の主体がその意思を知る保証は今回の公式仕様から確立していない。したがって「送信ボタンを押した瞬間に全世界で停止」とは約束しない。P2の入力受信と操作確定を同一順序で扱う条件は維持し、端末側の未配送まで検知する具体的な仕組みは未確立として残す。単にhostの生存通知が新しいだけではこの問題を解消しない。

#### 判定と次の実物

A0はPO承認済み。Plannerは3入口分類、2×|E|の通常/実行中観測、2区間の通信障害、重複と別入力の区別を設計した。Architect同一AI自己審査はA1/P2全体REVISE。実端末一覧/既存接続/host受信監査/通常の診断設定手順が揃っていないため正式カードを発行しない。実機試験0、リモート設定変更0、GO発行0。次はPOから既存接続状態を受け取り、対応host/会話の実物を確認してA1カードの具体手順を埋める。接続範囲の合意を取り直さない。
...
今回の依頼: 設計担当が「GO委任モードを実際に有効化するための設計・承認経路の整備を進める、という理解で合っていますか？」と確認し、PO原文「yes」、続いて「進める」を受領。これは調査・設計の継続であり、検証CIの実装担当への切替や本番の権限変更承認ではない。

本節は以下の履歴を消さず、現在の残件と次工程を示す。根拠: docs/handoff/go-record-transcription/recon.md「2026-09-12 有効化設計の再開調査」。基点66b417665c013fdb354d5bda63226d07a1b2182c。関連ADRはADR-113・ADR-121・ADR-135・ADR-136。

### 承認済みの方針と今回の状態

- 親設計で合意済みの「チャットで委任が成立し、追加のGitHub本人操作は要求しない」を維持する。登録は成立済みの委任を検証可能にする作業であり、再承認のための入口にしない。
- PR #3406はMERGED、merge SHA4afb81c398d26f4c9b1321a4c70f21c50e8211fb、Deploy run34657046594はsuccess。これらは文書PRの記録で、代理GO経路の完成ではない。
- LINE追補の9月10日記録と、親設計の別セッション記録を合成しない。各元発話に固定された開始・対象・失効を保持する。今回のyesや文書保存日時を新しい委任の開始時刻にしない。現在の有効grantを本便では作成しない。
- 次のP1は既に限定設計APPROVE・実行承認後の403停止という履歴を持つ。追加の意思確認を要求するために「未承認」へ戻さない。ただし新しい実装役への委任や役割切替は自動実施しない。

### 目的・成功条件と変更前後

目的は、POが合意した範囲で離席中も検証後の作業を進め、帰席後に委任・GO・対象HEAD・結果を追えること。現在は原文転記の本人GO検査だけで、代理発行を独立に照合する経路がない。導入後は本人GOと代理GOを区別し、代理経路の入力が不足した場合の変更要求を0回にする。POの通常のチャット操作は増やさない。

今回の成果物は既存文書・P1カードの再開条件・証拠JSON。製品、DB、CI、scripts、secrets、App、Ruleset、実行用grantは変更対象外。

| 完成条件 | 判定方法 | 今回の事実 |
|---|---|---|
| 登録済み本人発話を受付け、偽装入力は受理0件 | P1実機6ケースと下記権限試験 | オフライン18/18、GitHub実機0件 |
| 同じ元発話の二重登録で期限・範囲の変化0件 | P2の正常・否定試験 | 設計案のみ |
| 期限・取消・対象・HEAD・CI不一致で変更要求0回 | P3/P4と既存V01〜V13 | 実装・実機未確認 |
| 専用主体以外のmain更新0件 | 専用検証repoの対照試験後、本番設定を照合 | App/本番権限未確認 |
| 委任→GO→変更→反映の参照欠落0件 | 各イベントの記録と実物を全件照合 | 運用経路未実装 |

### 工程と通過条件（名称P1と既存V01〜V13を維持）

| 工程 | 今回確定する範囲 | 次へ進む条件 | 実行担当・現在地 |
|---|---|---|---|
| P1準備 | 既存sandboxに合成受付workflow1件のPR。実grant/GOなし | 指定branch・1ファイル・hash・PRのGET一致 | 検証実装担当。旧403停止、再開カードを更新 |
| P1実機 | 別カードでそのPRを確認・マージ後、合成Issue6件 | 正常1件ACCEPTED_PROBE、拒否5件、全件authorization_issued:false | 検証実装担当。未着手。準備PR作成だけでは実行しない |
| P2 | 元委任・取消・固定版の判定と追記記録 | 下記14ケース合格、状態の同時更新と破損復旧を実機確認 | 設計案。制御repo/主体固定後に正式カード化 |
| P3 | 既存V01〜V13、予約・緊急・復旧を統合 | 正常/否定/応答消失/競合の各証拠。省略・未実施0件 | 別の検証用実装。App・鍵・保護設定は具体差分の承認後 |
| P4導入 | 合格した制御版を固定し、文書・検査・入口・運用を同時に接続 | POの導入承認、実権限の対照試験、実反映証拠、取消演習 | 本番導入は未承認・未着手 |

P1の書込みが拒否されたときに、CLIの別資格やブラウザへ切り替えない。現在はコネクタのcollaborator GETがreadを返せるようになったが、branch/workflowを書ける証明ではない。実装担当の再開直前確認で実権限を確定する。恒久的な権限変更が必要なら対象repo・操作・差分をPOに示す。

### P2の記録契約案（実装カードではない）

1. 元発話の識別をorigin_key=(対象repo_id, source_session_id, source_event_id)に固定する。スコープや内容hashは識別キーに含めない。同じ発話でscopeを変えた二重登録を、新しい委任と誤認しないためである。event_idや時刻を確認できない場合は登録不能を返す。
2. 受付記録はorigin_key、原文の参照/完全性hash、確認担当の数値ID、受付event/run、scope_revision/hash、許可操作、対象tenant/接続ID、受任主体、元発話の受信日時を持つ。セッション原文を読める担当の転記証明を信頼する部分を明示し、hashだけでPO本人性を証明したと称さない。
3. startは確認済み元委任の時刻。expires=start+86400秒。登録が遅れた場合も期限を後ろへずらさない。親設計のチャット成立方針に合わせ、LINE追補の旧「登録時点から開始」案を新方式へそのまま流用しない。既存記録を遡及変更せず、新制度の契約として別審査する。
4. 同じorigin_key・同じ正規化内容は同じgrant_id/開始/失効を返す。内容が異なる再送はORIGIN_CONFLICT。新しいユーザー発話を、古いgrantの上書きに使用しない。元発話の対象に関する意味判断が不足していればSCOPE_UNVERIFIEDとする。
5. 取消は既存grant_idを指す独立要求。取消は単調で、後から元登録を再送しても戻らない。取消の取得失敗・保存結果不明は新規GO/変更要求0回。取消と送信権取得は同じ状態先端を使う排他更新で順序を確定する。送信確定後の取消について、その送信を取り消せたと表示しない。
6. 受任AIは要求を提出できるが、確定grantと制御コードを直接更新できない。受付担当、状態記録担当、製品マージ担当、制御コード保守担当の権限を分ける。自己のscope/期限/認可根を変更するPRはこの委任の対象外。制度実装の改善と自己認可の変更は別判定にする。
7. 各GOはgrant_id/版、repo/PR/head/base、承認済み設計・カードhash、全差分、指定Reviewerの記録、CI結果、発行時刻、操作種別を参照する。パスだけの一致を意味上のscope適合としない。指定Reviewer未確定・差分未照合・CI未完了はGOを発行しない。
8. 実行直前にnow < expires、取消なし、grant/認可版一致、最新head/baseと品質証拠を再取得する。GO後にhead/baseが変われば前のGOを流用しない。各deploy/再解析/配信にも新しい直前照合を要求する。3接続は件数だけでなく固定された接続ID集合で照合する。
9. 状態更新は既存案の唯一の親＋force=falseを維持し、初期先端を別に固定する。保存結果不明で送信しない。IN_FLIGHTの経過時間や新GOを根拠とする解除は設けない。自動デプロイの開始境界を含む最終保証は実機試験で確認する。

| 基準 | 検証方法 |
|---|---|
| P2-01 正常な未失効委任の登録1件 | 確認済み元発話・固定認可版の入力を登録し、1grant/1履歴を照合 |
| P2-02 同じ要求の再送で変化0件 | 同origin/contentを2回送り、ID・時刻・範囲の一致を検査 |
| P2-03 同じ発話で範囲を変えた再送の受理0件 | origin同一・scopeだけ変更しORIGIN_CONFLICT |
| P2-04 元発話ID/時刻不明の登録0件 | 欠落を個別注入して拒否 |
| P2-05 遅い登録で失効時刻の延長0秒 | 受信から23時間後の登録でも残り1時間 |
| P2-06 失効時刻ちょうど以後の要求0回 | expires-1秒/一致/+1秒を時刻入力で照合 |
| P2-07 取消後の再送で復活0件 | revoke→元要求再送の順に実行 |
| P2-08 取消取得/保存不明で変更0回 | 403/timeout/破損と保存応答消失を個別注入 |
| P2-09 未登録担当の受付0件 | repo/eventの正常条件を保ちsender/authorだけ変更 |
| P2-10 自分の認可根・範囲・期限変更を受理0件 | 正常な制度実装PRと自己拡張PRを別fixtureで対照 |
| P2-11 PR側だけの偽grantを受理0件 | trusted state未登録・PR本文だけにgrantを追加 |
| P2-12 HEAD/base/レビュー/CI不足で発行0件 | 各条件を1つずつ壊す。理由コードを記録 |
| P2-13 取消と送信権の同時成立0件 | 同じ親stateで競合させ、先行更新だけを受理 |
| P2-14 件数3だが接続IDが違う配信0件 | 同件数・別ID集合を入力して拒否 |

上記14は計画件数であり、今回の18/18ローカル試験と合算しない。P1の成功はP2の安全性を証明しない。

### 未解決の実装入力と停止範囲

制御repoの実ID/固定コード版、状態書込Appの実ID/installation/権限、鍵受渡し、source_event_idを正規に受け渡す実装、取消の実配送、P3の同時更新/復旧、実配備SHAの読取口は未確定または未実測。仮の成功・IDを埋めない。このためP2以降の正式実装カードと本番有効化判定は出さない。P1はこれらを必要とせず、既存の限定カードで準備PRまで独立して進められる。

### トレードオフ・接触面・維持

公開sandboxには合成データだけを置く。実委任原文やセッションログを公開Issueへ送らない。制御の状態を増やすことで障害時に進行が止まりやすくなるが、結果不明のまま再送する危険を避ける。grant登録遅延によって残り時間は減る。これは再登録で期限を延長しない契約の帰結である。

人: POは既存のチャット委任を維持。エージェント: 新担当にはカード/固定版/停止点を渡し、自動起動しない。機械: 検査と状態更新は未実装で、P1合成受付と区別。データ: 本番DB/tenantは非接触。実行時のtenant/接続IDは正式記録が必要。本番: 予約とdeploy結果の照合、実反映確認が未完了。外部: GitHubの権限・eventを実測する。資料だけで到達性を合格にしない。

守り手: 設計担当は本節とrecon・カードの整合、実装担当は指定差分と検証結果、最終Reviewer/EvaluatorはP3/P4の実機証拠、POは認可根/本番設定変更の判断を担う。自分の限定設計審査を独立した第二者承認としない。

### Architect自己審査

判定: P1準備PRの限定設計はAPPROVE維持。根拠は変更なしのworkflow hash、18/18の直接再検証、actionlint成功、対象repo/main/既存物のGET、カード再開点の具体化。書込権限を断定せず、拒否時に止まる契約を維持する。

全体はREVISE。P2記録型・14否定/正常条件と依存関係は具体化したが、上記未解決入力とP3/P4の実機証拠がない。PR #3406のGOはこの文書PR・実装・有効化へ流用しない。今回の審査は同一AIによる自己審査。

## 実機検証P1の準備PR（作成当時の記録・現状は2026-09-12節）
...
| 対象 | 保存/受付 | 更新できる主体 | 検査 |
|---|---|---|---|
| 制御コードと認可版 | PO管理の制御repoの固定版 | 別の本人承認を受けた保守担当 | 委任AI/製品merge App/実行時GITHUB_TOKENはコード書込不可 |
| 元委任の転記要求 | 制御repoのIssue作成イベント案 | 登録済み転記担当の数値アカウントID | eventのsenderとIssue author両方、対象repo ID、schema、固定要求IDを照合 |
| 委任・GO・予約の確定記録 | 既存案release/go-merge-stateのJSONと追記履歴 | 制御処理だけ。製品側からの直接編集を拒否 | 単一親CAS、固定認可版、受付証明、取消・期限・対象・品質根拠 |
| 製品main | 専用merge App | 制御処理から検査済みの1回送信 | 通常資格の直接更新拒否と既存CI/レビューを維持 |

受付用Issueは承認正本ではない。`issues: opened`で受け取った本文のsnapshotを検証後に記録し、同じ要求IDを異なる内容で再登録したら拒否する。Issue編集を期限/範囲の変更に使わず、取消は別の追記要求にする。取消要求が不明な間は新規GO/送信を停止する。元のセッションを見た転記担当の証明を信用する部分は明示する。Issue authorの数値IDはGitHub側から読み、本文の自己申告IDを信用しない。すべてのコメント/一般参加者の要求を受理する入口にはしない。

制御workflow自身のGITHUB_TOKENはcontents:read、issues:readを基準とし、その他はnone。製品Appのinstallationは製品/状態保存先に限定し、制御repoへのwriteを与えない。受付内容をshellへ展開せず構造化データとしてパースする。入力からローカルpath/URLを読み込む機能を作らない。任意repo、任意ref、任意コマンドを指定できる汎用実行口は作らない。

この分離の実機合格条件は、通常の転記担当が受付可能でも制御コードを更新できないこと、製品Appが制御コードを更新できないこと、PR変更コードがApp鍵へ到達できないこと、状態branchに直接書いた偽記録を採用しないこと。固定したID/権限/版と各拒否結果を記録する。合成JSONの検査だけで権限分離合格にしない。

本番repoを変更できるAppに同repoの状態branch更新も許す構成では、盗まれたApp資格が状態更新可能な点は残る。鍵を保持する制御処理を信頼境界に含め、資格漏洩時は停止/失効と監査を行う。rulesetやJSON hashで資格漏洩そのものを防げるとは主張しない。

### 実測した既存処理との差

scripts/gh-pr-merge-safe.sh:19-22はGITHUB_ACTIONS時にexit0、:93以降のmerge_with_retryはmain追従と再送を行う。GO/委任/予約を検査していない。このwrapperをActionsで呼ぶだけではマージも新制度の検査も実行されない。コメントに残るsquash/adminの使用例は現行規則と異なるため採用しない。新制度の実装時にはローカル入口を中央受付へ接続し、暗黙のmain追従再送で承認HEADを置き換えない。

検証repo1363676622は存在し公開、今回のCLI主体のpermissionsはpull:true/push:false/admin:false。読取は可能、設定や試験workflowを書けると推測しない。この便で設定/実装を変更した事実は0件。

### 復旧枠の到達性検査

合成モデルは8状態/9遷移、通常待機2件を保持し、確定失敗→復旧枠取得→送信→merge確定→実反映確認→再開判断→通常先頭の取得という7操作の経路を確認した。通常列を途中で解放する変異版はunknown_operation_releasedを検出。通常の失敗ticketを成功へ書き換えず、実行枠を復旧へ移す案で循環待ちを避ける。

この結果は承認/CI/実反映と原子的状態更新を正しく取得できる前提。1障害/1復旧主体の限定モデルであり、複数障害・再試行ID・実CAS/APIは対象外。モデルのソースと結果はreconへ保存。全体自己審査はREVISE、検証用実装/実機権限試験/正式導入は未着手。判定と実行の境界は具体化済みだが、実機権限を推測で合格にしない。

## 委任の一次記録・承認処理の分離・復旧経路（2026-09-11追補）

### 委任記録の確認結果

同一セッションのローカルJSONLからuser roleの発話を抽出した。最初に直接確認できた委任は2026-09-10T12:11:22.302Z（JST21:11:22.302）、後続の同条件委任は14:33:02.751Z、成立時点の明確化は22:33:22.965Z。元発話と各行hashはreconへ保存。JSONLのtimestampは受信記録でありPOの端末で送信した瞬間と断定しない。

このセッションで直接確認した元委任の記録IDを go-session-01a088b5-751b-7981-8c33-881468480fdf-2648 とする（文書上の識別子で、実サービスへ登録済みという意味ではない）。受信記録基準の24時間は2026-09-11T12:11:22.302Z（JST21:11:22.302）まで。後続発話や今回の保存時刻で延長しない。本セッション外の別委任へ流用しない。期限確認時刻を毎回取得し、期限ちょうど以降の新しいGO発行・変更操作は拒否する。期限が来ても送信済み処理の結果照合は続け、期限切れを根拠に不明な予約を解放しない。

ここまでで「委任原文と受信時刻が全く確認できない」という調査残件は解消した。ローカルログとhashは改変検出の補助であり、GitHubがPO本人性を独立認証した証明ではない。

### 承認処理の保護案（方式案・設定未実施）

委任されたAIがsalesanchorの制度実装を変更できても、その変更を許可する処理と鍵は同じ変更から隔離する。候補はPO管理の専用制御リポジトリで独立起動する承認処理。salesanchorから再利用workflowを呼ぶだけの構成ではcallerの実行情報/資格に依存するため、隔離の達成とは扱わない。検証は既存sandbox上の合成対象で先に行い、本番用リポジトリの新設・鍵・保護設定はこの文書保存便で実施しない。

役割の境界:

| 主体 | 読取/申請 | 許可しないこと |
|---|---|---|
| 委任を受けたセッション担当 | 元発話を確認し、原文・受信時刻・対象・期限・照合担当を転記した要求を提出 | 自己申告だけで承認済み状態へ直接書く、期限延長 |
| 制御側の受付処理 | 許可された提出主体、登録済み対象repo、要求重複と元記録参照を照合 | PRのスクリプト実行、依頼入力による任意repo/任意コマンド選択 |
| 制御側の承認/マージ処理 | 固定された認可版で期限/取消/対象/レビュー/CIを再取得し、状態保存後に限定Appで送信 | 対象PRの変更後コードで自分を許可する、App鍵を対象repoへ渡す |
| PO管理の保守担当 | 制御側コード/設定/認可版の変更を別の本人承認経路で更新 | 同じ委任で自分の認可の根を更新する |

GO制度変更を含む委任範囲は維持する。上の制御側に固定する最小の認可処理（元委任の照合主体・期限・取消・許可範囲・鍵の受渡し）は、既に合意した自己拡張禁止を守るため別管理にする案。通常のGO制度の改善すべてをPO本人作業へ戻す案ではない。

機械が元セッションへアクセスできない構成では、受付は「元発話を見た担当の転記証明」を信頼する。この信頼をhashによる本人認証と言い換えない。提出資格を持つ担当による虚偽転記まで自動で見破る保証はない。PO追加操作不要と完全な独立本人認証を同時に達成したとはしない。運用上許される転記主体/資格、保存先の更新制限、実機否定試験が揃うまで実装全体APPROVEを出さない。

### 障害修正PRの復旧枠案（通常列を止めたまま修正を通す）

1. 本番デプロイの確定失敗は通常列をBLOCKEDのまま保持する。失敗ticket・merge SHA・run/attempt・実配備とhealth・失敗原因をincident IDで束ねる。
2. 復旧対象PRをincident IDへ結び、通常の緊急優先と区別した復旧承認を取得する。対象修正・HEAD・バックアップ/戻し方・必要CIの証拠を含める。今回この具体的な復旧承認を受領したと創作しない。
3. 復旧枠を取得できるのは、マージ応答不明0件・進行中deploy0件と確認できた時だけ。通常列は動かさず、1つの共有実行枠を復旧ticketへ原子的に割り当てる。独立した2本目のマージ実行権は作らない。
4. 復旧PRにも対象HEADのGO・レビュー・CI・送信前状態保存・不明時の二重送信禁止を適用。通常待機ticketを削除/並べ替えない。
5. 復旧PRのデプロイ成功と実配備証拠、原因が現mainから除去されたこと、再開判断を記録してincidentを閉じる。元失敗ticketを成功へ書き換えず、復旧ticketへの関連を保存し、その後だけ通常先頭を再開する。
6. 復旧も失敗/不明なら通常列を止めたまま照合する。時計切れ/緊急ラベルだけで解放しない。

これは「失敗時に最後尾へ戻す」合意のマージ前確定失敗と別の、本番失敗後の復旧案。POの緊急優先承認を復旧承認へ流用しない。実装状態機械/競合試験への統合は未実施。

### 限定検証と全体審査

委任判定モデルで19ケースが期待結果と一致。重複受付4条件も成功。期限の比較を誤って境界込みにする変異版は、期限ちょうどを許可する誤りとして検出。元記録・時刻・CI・レビューを正しく取得済みという前提付きで、実API・入力パーサー・永続化競合・資格の検証ではない。モデルsourceと結果はrecon参照。

同一AI自己審査REVISE。時刻証拠と限定判定の根拠は補完済み。承認処理の独立配置、転記主体の信頼境界と復旧枠は具体案となったが、実機の権限・正常/拒否試験は未実施。ここを推測でAPPROVEにしない。POの委任は成立済みであり、未完了なのは設計/検査実装と導入検証である。
...
- 観測時刻2026-09-10T22:35:50Z（JST翌07:35:50）はclock取得時刻であり、元の委任発話の時刻ではない。この時点では未確認だった受信時刻を、上の2026-09-11追補で一次記録から確認した。今回の確認や保存時刻で開始を上書きし、24時間を延長しない。元の時刻を一次記録から確認する作業は追加承認を求めることとは異なる。
