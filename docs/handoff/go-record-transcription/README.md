...
次の一便: POが必要とする取消の入口を確定し、接続方法を読取照合する。最小検証をA入力観測/B権限対照/C統合/D配備へ整理。A1は3観測、B1は6行・最小7変更要求の計画。正式カード未発行、実機追加0、全体自己審査REVISE。根拠はdesign.md「最小検証の依存関係」とrecon.md「2026-09-13 最小検証の依存関係整理」。
