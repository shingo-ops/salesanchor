...
次の一便: 制御/配備主体の配置と権限差分を具体化する。H1のtimeout不合格を踏まえ、送信側の5主体/8手順/S01〜S10を設計。main自動配備・可変SHA・専用権限制限の未確認を残す。全体自己審査REVISE、実装カード未発行、代理GO0。根拠はdesign.md「送信側で拒否する契約」とrecon.md「2026-09-13 送信側拒否条件の照合」。
