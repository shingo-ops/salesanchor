...
- フロント変更のみで期間切り替え可能
- Tabs金型: variant="pill" size="sm" がページ内で既に使用済み（カードゲームタブ）
- historyDays i18nキー: `frontend/src/locales/ja.json`:314 / `frontend/src/locales/en.json`:314 に固定文字列が存在
- 既存ADR参照: ADR-027（i18n強制）、ADR-144（UIガバナンス）
