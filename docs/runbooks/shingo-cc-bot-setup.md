# Runbook: shingo-cc ボットアカウント セットアップ
...
### Step 1: shingo-cc アカウント作成

1. **ブラウザで `https://github.com/join` を開く**
2. メールアドレスを入力（例: shingo+shingo-cc@your-domain.com）
3. ユーザー名に `shingo-cc` と入力
...
### Step 2: salesanchor リポジトリに shingo-cc を招待
...
3. 検索欄に `shingo-cc` と入力 → ユーザーを選択
4. 権限を **「Write」** に設定
5. 「Add shingo-cc to this repository」をクリック
6. shingo-cc のメール受信箱で招待メールを確認 → 「Accept invitation」リンクをクリック

---

### Step 3: Fine-grained PAT 発行（shingo-cc アカウントで実施）

1. **shingo-cc アカウントでログイン**（Step 1 で作ったアカウント）
...
| 項目 | 値 |
|------|-----|
| Token name | `shingo-cc-2026` |
| Expiration | **1 year**（最長）|
| Resource owner | `shingo-cc`（デフォルト） |
...
git checkout -b test/shingo-cc-auth-check
git commit --allow-empty -m "test: shingo-cc 認証チェック用テストPR（クローズ予定）"
git push origin test/shingo-cc-auth-check

gh pr create \
  --base develop \
  --title "[TEST] shingo-cc 認証確認 — クローズ予定" \
  --body "shingo-cc アカウントでの PR 作成確認テスト。Shingo が shingo-ops で Approve できることを確認後クローズ。"
```
...
3. テストブランチ削除: `git push origin --delete test/shingo-cc-auth-check`
4. 確認結果を Shingo に報告
