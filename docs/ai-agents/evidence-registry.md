...
### マスタ整備と並行する数量・価格・注意事項監査（2026-09-13）

POはワンピース商品マスタ・検索語の別セッション整備中に進められる作業を希望。商品辞書を編集せず、保存2投稿/67明細の原文から数量/価格を独立に読み直し、抽出値・保存解析値の双方とDecimal比較した。原文数量価格行67、抽出67、欠落0/重複0、抽出数量67/67・価格67/67、保存解析数量67/67・価格67/67一致。内訳SIG66/別投稿1。入力SHA256 5e165443af0812b5f2c45c8b82e2c7682514bfd1cffdbcbbd005821174961693。単位の抽出欄はSIG66件空、別投稿1件box。状態欄非空2/備考非空3。原文省略をBox/Sealedの事実確認とみなさず、PO既決の限定推定規則と区別する。数量・価格の一致は全項目正答率ではない。

注意事項: SIG原文L109の「MEGA スタートデッキ100 バトルコレクション※買取品のためサーチ済みの可能性有」は備考へ保持済み。保存解析はnote_ja=サーチ済の可能性,買取品、pid_resolved=true、needs_review=false、exclusion=null、unit_resolved=false、condition=FLAG_SINGLE。単位不明のため現行配信SQLの条件を満たさないが、実配信実績は未照会。

文書作業木HEADcf20241aの実関数build_review_reasons（backend/app/services/tcg_analyzer_svc.py:976）をAST抽出し同入力で局所実行、要確認理由[]を再現。注意書きが既存ラベルへ変換された場合、note_unmatchedにはならない。配信SQL同tcg_distribution_svc.py:238以降は商品特定/要確認false/非excluded/単位特定等が必要、FLAG_SINGLEはオプション依存。現行全解析/現在DBの状態マスタ・ステータスマスタ・配信設定は再照会していないため、現在本番で配信されるという断定はしない。

次の調査は商品辞書に依存せず、(1)サーチ済み可能性/断定/未サーチ/否定の4区分について状態・注記・配信抑止の現行契約を照合、(2)商品と単位が解決した場合でも意図した抑止が維持されるか匿名試験、(3)SIG限定の単位省略既定と特殊セット/プロモの境界検証。価格単独でBox/Caseの正解を作らない。過去accuracy-followupの価格検証は正解付き2件だけで、一般判定の証明にならない。

今回完了は読み取り調査・保存原文の再測定・次の検証対象特定。製品修正/新規委任/本番接続/Gemini/配信0。新しい安全判定の正式設計・PO承認・実装カードは未作成。現行マスタ整備を待たずに進める検証の根拠であり、別担当の完了や本番正答率を先取りしない。非公開証拠: private-research/sig-nonproduct-audit-20260913（audit.py/summary.json/rows-private.json/review-reason.json）。外部事例は保存原文の転記一致監査に不要のため使用しない。
