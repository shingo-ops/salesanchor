...
EV-20260913-PRODUCT-CSV-ROUNDTRIP-DESIGN: PO原文と合意3条件をdesign§21へ保存。main10212686/10一次情報SHA/旧10列新規専用・非可逆strip/split/既存履歴型を実物確認。12列更新専用・revision/全行同時確定・13製品ファイル/R1–R11を同一AI自己審査APPROVE。Context7不在、公式資料代替確認。実装未着手、番号付きGO原文なし。根拠roundtrip-design-evidence.json/recon。
