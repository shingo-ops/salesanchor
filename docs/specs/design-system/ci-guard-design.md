...
画面移行後の全件検査案では、旧22テストのui-allow免除/table非検出の期待は維持できない。本書の過去の自己審査APPROVEは差分比較を維持する当時の狭い設計に対するもので、新しい全件検査の合格を意味しない。最後のCIでは列挙/読取不能exit2の契約を継承し、差分比較だけの修復便は先行実装しない。詳細は[全体設計§Z](design.md#z-実物照合後の共通部品契約2026-09-10設計担当案)。設計自己審査は全体設計§AAで合格。ADR改訂のPO承認・製品実装・試験は未実施。
