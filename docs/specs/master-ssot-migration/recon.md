...
| ファイル | 行 | 参照内容 |
|---------|-----|---------|
| `tcg_work_reference.py` | 55 | FROM {schema}.tcg_series（抽出の核） |
| `tcg_analyzer_svc.py` | 425 | SELECT id, display_name, alt_name |
| `tcg_distribution_svc.py` | 238 | LEFT JOIN tcg_series |
| `tcg_analysis_review_svc.py` | 41, 303 | JOIN + SELECT |
| `tcg_product_import.py` | 135, 318 | SELECT + カラムマッピング |
| `tcg_product_master_svc.py` | 85, 357, 360 | SELECT（3箇所） |
| `tcg_product_detail_svc.py` | 17 | マッピング定義 |
| `tcg_product_import_svc.py` | 65 | マッピング定義 |
| `tcg_work_comparison_svc.py` | 32 | リスト定義 |

`public.tcg_series_master`を参照している箇所:

| ファイル | 行 | 参照内容 |
|---------|-----|---------|
| `super_admin_tcg.py` | 63,85,121,140,252 | CRUD（5箇所） |
| `inventory_search.py` | 279-280 | 検索JOIN |
| `central_masters.py` | 7,109 | スキーマ定義 |
| `TcgSeriesTab.tsx` | 5 | フロント管理画面 |
