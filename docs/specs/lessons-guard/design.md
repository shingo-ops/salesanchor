...
- （2026-07-22 範囲限定化）検知を scripts/check-lessons-section.sh に分離し、対象を §6開始行〜§7開始行の手前に限定。§0〜§5・§7・§8への箇条書き追記は素通しになる。行番号は毎回 grep で動的取得するため、他セクションの増減でずれない。手元検証5試験(§6内=1 / §3=0 / §7=0 / §8=0 / §6+§7=1)で実測。
