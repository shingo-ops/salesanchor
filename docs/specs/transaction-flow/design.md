...
- ON DELETE挙動: RESTRICT で確定（PO しんご 2026-07-15）。理由: 取引データ（lead・会社・deal・会話ログ）は永久保存の方針（§11 ソフトデリート予約）であり、leadを物理削除しない運用。RESTRICTは物理削除自体をDBが拒否するため、永久保存の思想と一致し、万一の誤削除で会話ログが巻き添えに消えることを防ぐ。lead_channels=CASCADE・meta_messages=SET NULL とは意図的に異なる（会話ログは分析材料として保護優先）
...
### 予約①: CSVインポートの連番自動採番
便1完了後の次テーマとして予約する（PO しんご 2026-07-15）:
...
### 予約②: ソフトデリート（非表示扱い・論理削除）
便1のON DELETE=RESTRICT の前提となる方針。本体実装は独立テーマとして予約（PO しんご 2026-07-15）:
- あるべき姿（PO自筆）: 「フロントからは完全に消すがDB側では残してほしい。データを消すのはもったいないので非表示扱いにする」「画面からは非表示にしてクライアントには見えないようにするが全ての分析材料として記録は残しておく」
