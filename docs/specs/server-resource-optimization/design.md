...
## 2.5 実証で確定した実装条件(2026-07-21 手動発火実証・実測)
- 黒板ディレクトリはnode-exporter起動より前にubuntuユーザーの直接mkdirで作成する(compose upが先行するとroot所有で作られ書き込み不能。実測で確認)。
- 黒板ファイルはchmod 644で公開する(mktemp既定の600ではnode-exporter実行ユーザーnobodyが読めずnode_textfile_scrape_error=1。実測で確認)。
- 棚を作り直した場合はnode-exporterを--force-recreateで再作成する(マウントは起動時固定。実測で確認)。
- 確認プローブは到達性を先に検証する(9100はホストlocalhostに非公開・公開IP側のみbind。localhost応答なし=コンテナ死ではない。実測で確認)。
- 全線開通の実測記録: 黒板epoch 1784568278がprod1ファイル→node-exporter出口→prod2 Prometheusの3地点で同値一致(2026-07-21)。
