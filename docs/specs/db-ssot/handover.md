...
## 次回の入り口（着手はここから）
- POが既存スプレッドシートで整理済みの「どんなデータをどう持ちたいか」を材料に、各テーブルを設計する。
- ただしスプレッドシートの形をそのままDBにしない。同じ事実の重複はSSOT観点で整理し直し（1か所に集約・背番号で参照）てから設計図にする。
- 意味のまとまりごとに1つずつ「recon→設計→保存」を回す（会話便と同じ型）。
