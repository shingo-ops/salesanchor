...
- 成長回路の原則（あるべき姿4対応）: 不具合・違反・停止は「開発中」「関所」「本番」の
  3箇所で捕捉し、track-record と5W2Hで蓄積層（リポジトリの文書と関所）に記録する。
  エージェント本人はセッション毎にリセットされるため、育てる対象は文書と関所である。
  同じ指標で×が続いた教訓は、文書ルールから機械の関所へ昇格させる（読み落とし対策）。
  逆に、関所は定期に棚卸しして過剰な検査を枝刈りする（重すぎる検査は迂回を生み腐る）。
  図解: [growth-loop.svg](./growth-loop.svg) ／ [theme-structure.svg](./theme-structure.svg)
- 資源配分の原則（あるべき姿7・KGI⑧）: Fable（設計パートナー）は翻訳と検品に専念し、
  事実収集・機械的照合・組立は実装役が担う。高価な頭脳は判断に、量産は安価な手に。
